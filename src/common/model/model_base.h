...
public:
  ...
  void end() {
    ...
    cost_ = (int32_t)((end_.tv_sec - start_.tv_sec) * 1000000 + end_.tv_usec -
                      start_.tv_usec);
  }
...
class Model {
public:
  Model(const string &name, const string &model_type,
  ...
  static string gen_name(const string &model_type, const string &name) {
  ...
  bool predict(shared_ptr<Input> input, shared_ptr<Output> output) {
    if (!is_init_) {
      LOG_ERROR << "Local Model predict() model not init, model name "
                << name_.c_str();
      return false;
    }
