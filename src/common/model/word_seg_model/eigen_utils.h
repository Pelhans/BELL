...
public:
    ...
    static void get_max_index(Eigen::MatrixXf& input, Eigen::VectorXi& indexs,
                              int axis = 0);
    ...
    // load
    static bool genWord2id(std::string& path,
                           std::map<std::string, int>& word2id);
    static MatrixXf loadEmbeddingParam(std::string& path, int vocab_size,
                                       int embedding_dim);
    static conv1DLayer loadConv1dParam(std::string& path, int in_channels,
                                       int out_channels, int kernel_size);
    static fnnConnectLayer loadFullConnectParam(std::string& path,
                                                int in_channels,
                                                int out_channels);
