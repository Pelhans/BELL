...
void ResourceInit::async_init(const BellConfig &config) { init_func(config); }
...
bool ResourceInit::init_func(const bell::BellConfig &config) {
  ...
  if (!config.m_word_seg_model_config.empty() &&
      !initWordSeg("word_seg_cnn_crf", config.m_word_seg_model_config)) {
...
bool ResourceInit::initWordSeg(const std::string &model_name,
  ...
  auto model_mgr = boost::serialization::singleton<
