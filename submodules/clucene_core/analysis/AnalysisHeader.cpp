...
struct Analyzer::Internal {
    CL_NS(util)::ThreadLocal<
        TokenStream*, CL_NS(util)::Deletor::Object<TokenStream>>* tokenStreams;
};
Analyzer::Analyzer() {
    _internal = new Internal;
    _internal->tokenStreams = _CLNEW CL_NS(
        util)::ThreadLocal<TokenStream*,
...
class OrderCompare : LUCENE_BASE,
                     public CL_NS(util)::Compare::_base //<Token*>
{
public:
    bool operator()(Token* t1, Token* t2) const {
        if (t1->startOffset() > t2->startOffset()) return false;
        if (t1->startOffset() < t2->startOffset()) return true;
        return true;
    }
};
...
Token::Token(const TCHAR* text, const int32_t start, const int32_t end,
             const TCHAR* typ)
    : _startOffset(start),
      _endOffset(end),
...
size_t Token::bufferLength() { return bufferTextLen; }

int32_t Token::startOffset() const { return _startOffset; }

void Token::setStartOffset(const int32_t val) { _startOffset = val; }
int32_t Token::endOffset() const { return _endOffset; }
const TCHAR* Token::getDefaultType() { return _T("word"); }

void Token::setEndOffset(const int32_t val) { _endOffset = val; }
const TCHAR* Token::type() const { return _type; }

void Token::setType(const TCHAR* val) { _type = val; }

void Token::set(const TCHAR* text, const int32_t start, const int32_t end,
                const TCHAR* typ) {
    _startOffset = start;
    _endOffset = end;
    _type = (typ == NULL ? getDefaultType() : typ);
...
#ifndef LUCENE_TOKEN_WORD_LENGTH
    if (bufferTextLen < l + 1) growBuffer(l + 1);
#else
...
void Token::setTermLength(int32_t len) {
    if (bufferTextLen < len) this->growBuffer(len);
    this->_termTextLen = len;
}
TCHAR* Token::resizeTermBuffer(size_t size) {
    if (bufferTextLen < size) growBuffer(size);
    return this->_buffer;
}
void Token::growBuffer(size_t size) {
    if (bufferTextLen >= size) return;
#ifndef LUCENE_TOKEN_WORD_LENGTH
    if (_buffer == NULL) {
...
const TCHAR* Token::termText() const { return (const TCHAR*)_buffer; }
TCHAR* Token::termBuffer() const { return _buffer; }
...
void Token::resetTermTextLen() { _termTextLen = -1; }
...
Tokenizer::Tokenizer(CL_NS(util)::Reader* _input) : input(_input) {}
