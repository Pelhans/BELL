...
typedef CL_NS(util)::CLSetList<TCHAR*, CL_NS(util)::Compare::TChar,
                               CL_NS(util)::Deletor::tcArray> CLTCSetList;
...
/** A Token is an occurence of a term from the text of a field.  It consists of
  ...
  lucene::index::TermPositions#getPayloadLength()} and
  {@link lucene::index::TermPositions#getPayload(byte[], int)} to retrieve the
...
public:
    ...
    Token(const TCHAR* text, const int32_t start, const int32_t end,
          const TCHAR* typ = NULL);
    void set(const TCHAR* text, const int32_t start, const int32_t end,
             const TCHAR* typ = NULL);
    ...
    void setText(const TCHAR* txt,
                 int32_t len = -1); //< Sets the termBuffer. See #termBuffer
...
public:
    ...
    /** This is for backwards compatibility only. You should pass the token you
     * want to fill to next(), this will save a lot of object construction and
     * destructions.
