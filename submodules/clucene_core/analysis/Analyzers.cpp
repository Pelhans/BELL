...
TCHAR CharTokenizer::normalize(const TCHAR c) const { return c; }
...
LetterTokenizer::LetterTokenizer(CL_NS(util)::Reader* in) : CharTokenizer(in) {}
...
WhitespaceTokenizer::WhitespaceTokenizer(CL_NS(util)::Reader* in)
    : CharTokenizer(in) {}
...
TokenStream* WhitespaceAnalyzer::tokenStream(const TCHAR* /*fieldName*/,
                                             Reader* reader) {
    return _CLNEW WhitespaceTokenizer(reader);
}
TokenStream* WhitespaceAnalyzer::reusableTokenStream(
    const TCHAR* /*fieldName*/, CL_NS(util)::Reader* reader) {
...
SimpleAnalyzer::~SimpleAnalyzer() {}
TokenStream* SimpleAnalyzer::tokenStream(const TCHAR* /*fieldName*/,
                                         Reader* reader) {
    return _CLNEW LowerCaseTokenizer(reader);
}
TokenStream* SimpleAnalyzer::reusableTokenStream(const TCHAR* /*fieldName*/,
...
Token* LowerCaseFilter::next(Token* t) {
    if (input->next(t) == NULL) return NULL;
 ...
 */
StopFilter::StopFilter(TokenStream* in, bool deleteTokenStream,
                       CLTCSetList* stopTable, bool _deleteStopTable)
    : TokenFilter(in, deleteTokenStream),
      stopWords(stopTable),
...
StopFilter::StopFilter(TokenStream* in, bool deleteTokenStream,
                       const TCHAR** _stopWords, const bool _ignoreCase)
    : TokenFilter(in, deleteTokenStream),
      enablePositionIncrements(ENABLE_POSITION_INCREMENTS_DEFAULT),
...
StopFilter::~StopFilter() {
    if (deleteStopTable) _CLLDELETE(stopWords);
}
// static
...
StopAnalyzer::StopAnalyzer(const char* stopwordsFile, const char* enc)
    ...
    if (enc == NULL) enc = "ASCII";
    WordlistLoader::getWordSet(stopwordsFile, enc, stopTable);
}

StopAnalyzer::StopAnalyzer(CL_NS(util)::Reader* stopwordsReader,
                           const bool _bDeleteReader)
    : stopTable(_CLNEW CLTCSetList(true)) {
    WordlistLoader::getWordSet(stopwordsReader, stopTable, _bDeleteReader);
}

StopAnalyzer::StopAnalyzer() : stopTable(_CLNEW CLTCSetList(true)) {
...
TokenStream* StopAnalyzer::tokenStream(const TCHAR* /*fieldName*/,
                                       Reader* reader) {
    return _CLNEW StopFilter(_CLNEW LowerCaseTokenizer(reader), true,
                             stopTable);
}

/** Filters LowerCaseTokenizer with StopFilter. */
TokenStream* StopAnalyzer::reusableTokenStream(const TCHAR* fieldName,
                                               Reader* reader) {
...
const TCHAR* StopAnalyzer::ENGLISH_STOP_WORDS[] = {
    _T("a"),    _T("an"),    _T("and"),   _T("are"),  _T("as"),   _T("at"),
    _T("be"),   _T("but"),   _T("by"),    _T("for"),  _T("if"),   _T("in"),
    _T("into"), _T("is"),    _T("it"),    _T("no"),   _T("not"),  _T("of"),
    _T("on"),   _T("or"),    _T("such"),  _T("that"), _T("the"),  _T("their"),
    _T("then"), _T("there"), _T("these"), _T("they"), _T("this"), _T("to"),
    _T("was"),  _T("will"),  _T("with"),  NULL};
...
void PerFieldAnalyzerWrapper::addAnalyzer(const TCHAR* fieldName,
                                          Analyzer* analyzer) {
    analyzerMap->put(STRDUP_TtoT(fieldName), analyzer);
}

TokenStream* PerFieldAnalyzerWrapper::tokenStream(const TCHAR* fieldName,
                                                  Reader* reader) {
...
TokenStream* PerFieldAnalyzerWrapper::reusableTokenStream(
    const TCHAR* fieldName, CL_NS(util)::Reader* reader) {
    Analyzer* analyzer = analyzerMap->get(const_cast<TCHAR*>(fieldName));
    if (analyzer == NULL) {
        analyzer = defaultAnalyzer;
...
int32_t PerFieldAnalyzerWrapper::getPositionIncrementGap(
        ...
        for (int32_t j = 0; j < l; j++) {
            ...
            switch (c) {
                ...
                case 0x178: // �
                    ...
                    break;
#endif
                default: output.appendChar(c); break;
            }
        }
...
KeywordAnalyzer::~KeywordAnalyzer() {}
TokenStream* KeywordAnalyzer::tokenStream(const TCHAR* /*fieldName*/,
                                          CL_NS(util)::Reader* reader) {
    return _CLNEW KeywordTokenizer(reader);
}
TokenStream* KeywordAnalyzer::reusableTokenStream(const TCHAR* /*fieldName*/,
                                                  CL_NS(util)::Reader* reader) {
    Tokenizer* tokenizer = static_cast<Tokenizer*>(getPreviousTokenStream());
    if (tokenizer == NULL) {
...
KeywordTokenizer::KeywordTokenizer(CL_NS(util)::Reader* input, int bufferSize)
    ...
    if (bufferSize < 1) this->bufferSize = DEFAULT_BUFFER_SIZE;
}
...
Token* KeywordTokenizer::next(Token* token) {
    if (!done) {
        ...
        while (true) {
            rd = input->read(readBuffer, 1,
                             cl_min(bufferSize, token->bufferLength() - upto));
            if (rd == -1) break;
...
LengthFilter::LengthFilter(TokenStream* in, const size_t _min,
                           const size_t _max)
    : TokenFilter(in) {
    this->_min = _min;
    this->_max = _max;
...
Token* LengthFilter::next(Token* token) {
    ...
    while (input->next(token)) {
        ...
        if (len >= _min && len <= _max) return token;
...
CLTCSetList* WordlistLoader::getWordSet(const char* wordfilePath,
                                        const char* enc,
                                        CLTCSetList* stopTable) {
    if (enc == NULL) enc = "ASCII";
    CL_NS(util)::FileReader* reader = NULL;
    try {
        reader = _CLNEW CL_NS(util)::FileReader(
            wordfilePath, enc, LUCENE_DEFAULT_TOKEN_BUFFER_SIZE);
...
// static
CLTCSetList* WordlistLoader::getWordSet(CL_NS(util)::Reader* reader,
                                        CLTCSetList* stopTable,
                                        const bool bDeleteReader) {
    if (!stopTable) stopTable = _CLNEW CLTCSetList(true);
    ...
    _CLFINALLY(if (bDeleteReader && reader != NULL) {
        // reader->close();
        _CLDELETE(reader);
