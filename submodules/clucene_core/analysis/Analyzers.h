...
private:
    // bvk: i found this to work faster with a non-hash table. the number of
    // items in the stop table is not like to make it worth having hashing. ish:
    // implement a radix/patricia tree for this?
...
public:
    ...
    StopFilter(TokenStream* in, bool deleteTokenStream,
               const TCHAR** _stopWords, const bool _ignoreCase = false);
     ...
     */
    StopFilter(TokenStream* in, bool deleteTokenStream, CLTCSetList* stopTable,
     ...
     * Note: the stopWords list must be a static list because the strings are
     * not copied
     */
    static void fillStopTable(CLTCSetList* stopTable, const TCHAR** stopWords,
     ...
     * from now on. <p> Note: behavior of a single StopFilter instance can be
     * modified with {@link #setEnablePositionIncrements(boolean)}. This static
     * method allows control over behavior of classes using StopFilters
     * internally, for example {@link
     * lucene::analysis::standard::StandardAnalyzer StandardAnalyzer}. <p>
     * Default : false.
     ...
     * increments to result tokens. <p> When set, when a token is stopped
     * (omitted), the position increment of the following token is incremented.
     * <p>
...
public:
     ...
     */
    static CLTCSetList* getWordSet(const char* wordfilePath,
                                   const char* enc = NULL,
     ...
     */
    static CLTCSetList* getWordSet(CL_NS(util)::Reader* reader,
                                   CLTCSetList* stopTable = NULL,
...
public:
     ...
     */
    StopAnalyzer(CL_NS(util)::Reader* stopwordsReader,
...
private:
    ...
    typedef CL_NS(util)::CLHashMap<
        TCHAR*, Analyzer*, CL_NS(util)::Compare::TChar,
        CL_NS(util)::Equals::TChar, CL_NS(util)::Deletor::tcArray,
        CL_NS(util)::Deletor::Void<Analyzer>> AnalyzerMapType;
