...
Token* CachingTokenFilter::next(Token* t) {
    ...
    if (itCache == cache.end()) return NULL;

    t->set((*itCache)->termBuffer(), (*itCache)->startOffset(),
           (*itCache)->endOffset(), (*itCache)->type());
...
void CachingTokenFilter::reset() { itCache = cache.begin(); }
...
void CachingTokenFilter::fillCache() {
    ...
    while (input->next(&token)) {
        pCopy = new Token(token.termBuffer(), token.startOffset(),
