...
#include "clucene_core/analysis/AnalysisHeader.h"
#include "clucene_core/analysis/Analyzers.h"
...
StandardAnalyzer::StandardAnalyzer(CL_NS(util)::Reader* stopwordsReader,
...
TokenStream* StandardAnalyzer::tokenStream(const TCHAR* /*fieldName*/,
                                           Reader* reader) {
    ...
    if (bufferedReader == NULL)
        ret = _CLNEW StandardTokenizer(
            _CLNEW FilteredBufferedReader(reader, false), true);
    else
...
TokenStream* StandardAnalyzer::reusableTokenStream(const TCHAR* fieldName,
                                                   Reader* reader) {
    ...
    if (streams == NULL) {
        ...
        streams->filteredTokenStream =
            _CLNEW StandardFilter(streams->tokenStream, true);
        streams->filteredTokenStream =
