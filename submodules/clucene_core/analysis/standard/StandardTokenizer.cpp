...
------------------------------------------------------------------------------*/
#include "clucene_core/_ApiHeader.h"
#include "StandardTokenizer.h"
#include "clucene_core/shared/util/StringBuffer.h"
#include "clucene_core/util/_FastCharStream.h"
#include "clucene_core/util/CLStreams.h"
...
const TCHAR* tokenImageArray[] = {
    _T("<EOF>"),     _T("<UNKNOWN>"), _T("<ALPHANUM>"), _T("<APOSTROPHE>"),
    _T("<ACRONYM>"), _T("<COMPANY>"), _T("<EMAIL>"),    _T("<HOST>"),
    _T("<NUM>"),     _T("<CJK>")};
...
#define _CJK                                                             \
    ((ch >= 0x3040 && ch <= 0x318f) || (ch >= 0x3300 && ch <= 0x337f) || \
     (ch >= 0x3400 && ch <= 0x3d2d) || (ch >= 0x4e00 && ch <= 0x9fff) || \
     (ch >= 0xf900 && ch <= 0xfaff) ||                                   \
...
// freebsd seems to have a problem with defines over multiple lines, so this has
// to be one long line
#define _CONSUME_AS_LONG_AS(conditionFails)                          \
    while (true) {                                                   \
        ch = readChar();                                             \
        if (ch == -1 ||                                              \
            (!(conditionFails) || str.len >= LUCENE_MAX_WORD_LEN)) { \
            break;                                                   \
        }                                                            \
        str.appendChar(ch);                                          \
    }
...
#define CONSUMED_NOTHING_OF_VALUE    \
    (rdPos == specialCharPos ||      \
     (rdPos == specialCharPos + 1 && \
...
//#define REMOVE_TRAILING_CHARS(sb, charMatchesCondition) { TCHAR* sbBuf =
// sb.getBuffer(); for (int32_t i = sb.len-1; i >= 0; i--) { TCHAR c = sbBuf[i];
// if (charMatchesCondition) { sbBuf[--sb.len] = '\0'; } else {break;}}}
...
StandardTokenizer::~StandardTokenizer() {
    _CLDELETE(rd);
    if (this->deleteReader) _CLDELETE(reader)
}
...
inline Token* StandardTokenizer::setToken(Token* t, StringBuffer* sb,
    ...
    while (!EOS) {
        ...
        if (ch == 0 || ch == -1) {
            ...
            ** that case, we just continue. */
            if (ReadNumber(NULL, ch, t)) return t;
        } else if (_CJK) {
            t = ReadCJK(ch, t);
            if (t != NULL) return t;
        }
...
Token* StandardTokenizer::ReadNumber(const TCHAR* previousNumber,
                                     const TCHAR prev, Token* t) {
    ...
    ** generating a single HOST token "192.168.1.3". */
    t->growBuffer(LUCENE_MAX_WORD_LEN +
                  1); // make sure token can hold the next word
    StringBuffer str(t->termBuffer(), t->bufferLength(),
                     true); // use stringbuffer to read data onto the termText
    ...
    {
        /* We have either:
        **   a) a negative sign that's not followed by either digit(s) or a
        *decimal
    ...
    ** implying that there might be another group of digits available? */
    if (!EOS) {
        ...
        if (!DIGIT && !DECIMAL) {
            ...
            /* We just read the fractional digit group, but it's also followed
             *by
...
Token* StandardTokenizer::ReadAlphaNum(const TCHAR prev, Token* t) {
    ...
    if (str.len < LUCENE_MAX_WORD_LEN) {
        ...
        CONSUME_WORD;
        if (!EOS && str.len < LUCENE_MAX_WORD_LEN -
                                  1) { // still have space for 1 more character?
            switch (ch) { /* What follows the first alphanum segment? */
                case '.':
                    str.appendChar('.');
                    return ReadDotted(&str, CL_NS2(analysis, standard)::UNKNOWN,
                                      t);
                case '\'': str.appendChar('\''); return ReadApostrophe(&str, t);
                case '@': str.appendChar('@'); return ReadAt(&str, t);
                case '&':
                    str.appendChar('&');
...
Token* StandardTokenizer::ReadDotted(StringBuffer* _str, TokenTypes forcedType,
                                     Token* t) {
    const int32_t specialCharPos = rdPos;
    StringBuffer& str = *_str;

    /* A segment of a "dotted" is not allowed to begin with another dot or a
     *dash.
        ...
        while (!EOS && str.len < LUCENE_MAX_WORD_LEN - 1) {
            ...
            **   "In the U.S.A.--yes, even there!"                 */
            if ((dot || dash) && (prevWasDot || prevWasDash)) {
                /* We're not going to append the character we just read, in any
                 *case.
                 ** As to the character before it (which is currently
                 *RIGHTMOST(str)):
                 ** Unless RIGHTMOST(str) is a dot, in which we need to save it
                 *so the
                 ** acronym-versus-host detection can work, we want to get rid
                 *of it. */
                if (!prevWasDot) {
                    SHAVE_RIGHTMOST(str);
                }
    ...
    ** that would potentially invalidate it. */
    { /* Begin block-guard of strBuf */
        ...
        if (CONSUMED_NOTHING_OF_VALUE) {
            /* No more alphanums available for this token; shave trailing dot,
             * if any. */
            if (rightmostIsDot) {
                SHAVE_RIGHTMOST(str);
            }
            ...
            /* Check the token to see if it's an acronym.  An acronym must have
             *a
             ** letter in every even slot and a dot in every odd slot, including
             *the
             ** last slot (for example, "U.S.A."). */
        } else if (rightmostIsDot) {
            bool isAcronym = true;
            const int32_t upperCheckLimit =
                str.len - 1; /* -1 b/c we already checked the last slot. */
    ...
    return setToken(t, &str,
                    CL_NS2(analysis, standard)::UNKNOWN
                        ? forcedType
                        : CL_NS2(analysis, standard)::HOST);
}
