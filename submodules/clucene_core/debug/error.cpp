...
------------------------------------------------------------------------------*/
#include "../shared/_SharedHeader.h"
#include "error.h"
...
CLuceneError::CLuceneError(int num, const char* str, bool ownstr)
    ...
    _twhat = NULL;
#endif
    if (ownstr) _CLDELETE_LCaARRAY(const_cast<char*>(str));
}
...
CLuceneError::CLuceneError(int num, const TCHAR* str, bool ownstr) {
    ...
    if (ownstr) _CLDELETE_LCARRAY((TCHAR*)str);
}
#endif
...
#ifndef _ASCII
    this->_awhat = NULL;
    if (clone._awhat != NULL) this->_awhat = STRDUP_AtoA(clone._awhat);
#endif
    this->_twhat = NULL;
    if (clone._twhat != NULL) this->_twhat = STRDUP_TtoT(clone._twhat);
}
...
char* CLuceneError::what() {
    ...
    return _twhat;
#else
    if (_awhat == NULL) _awhat = STRDUP_TtoA(_twhat);
    return _awhat;
#endif
}
TCHAR* CLuceneError::twhat() {
    ...
    return _twhat;
#else
    if (_twhat == NULL) _twhat = STRDUP_AtoT(_awhat);
    return _twhat;
#endif
}
...
void CLuceneError::set(int num, const TCHAR* str, bool ownstr) {
    ...
    _twhat = STRDUP_TtoT(str);
    error_number = num;
    if (ownstr) _CLDELETE_LCARRAY(const_cast<TCHAR*>(str));
}
