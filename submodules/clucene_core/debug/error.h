...
#define CL_ERR_Merge \
    21 //< Exception thrown if there are any problems while executing a merge.
...
#define _TRY try
#define _CLCATCH_ERR(err_num, cleanup, x) \
    catch (CLuceneError & err) {          \
        if (err.number() != err_num) {    \
            cleanup;                      \
            throw err;                    \
        } else {                          \
            x;                            \
...
#define _CLFINALLY(x) \
    ...
    x // note: code x is not run if return is called
...
#define _CLTHROWA_DEL(number, str)  \
    throw CLuceneError(number, str, \
                       true) // throw a string ensures the value is deleted
#define _CLTHROWT_DEL(number, str)  \
    throw CLuceneError(number, str, \
                       true) // throw a string ensures the value is deleted
