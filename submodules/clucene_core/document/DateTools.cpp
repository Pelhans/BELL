...
TCHAR* DateTools::timeToString(const int64_t time,
...
void DateTools::timeToString(const int64_t time, Resolution resolution,
                             TCHAR* buf, size_t bufLength) {
    // Take into account TZ and DST differences which may appear when using
    // gmtime below
...
int64_t DateTools::stringToTime(const TCHAR* dateString) {
    ...
    if (t == -1) _CLTHROWA(CL_ERR_Parse, "Input is not valid date string");
...
int64_t DateTools::timeMakeInclusive(const int64_t time) {
    time_t secs = time / 1000;
    tm* ptm = localtime(&secs); // use localtime since mktime below will convert
                                // the time to GMT before returning
    ptm->tm_hour = 23;
    ptm->tm_min = 59;
    ptm->tm_sec = 59;

    time_t t = mktime(ptm);
    if (t == -1) _CLTHROWA(CL_ERR_Parse, "Input is not a valid date");

    return (static_cast<int64_t>(t) * 1000) + 999;
}

int64_t DateTools::getTime(unsigned short year, uint8_t month, uint8_t mday,
    ...
    time_t rawtime;
    time(&rawtime);
    s_time = localtime(&rawtime); // use localtime, since mktime will take into
                                  // account TZ differences
    s_time->tm_isdst =
        0; // since we are using gmtime all around, make sure DST is off
...
TCHAR* DateTools::getISOFormat(const int64_t time) {
    ...
    tm* ptm = gmtime(&secs);
    return getISOFormat(ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday,
                        ptm->tm_hour, ptm->tm_min, ptm->tm_sec, ms);
}

TCHAR* DateTools::getISOFormat(unsigned short year, uint8_t month, uint8_t mday,
                               uint8_t hours, uint8_t minutes, uint8_t seconds,
                               unsigned short ms) {
    TCHAR* ISOString = _CL_NEWARRAY(TCHAR, 24);
    cl_stprintf(ISOString, 24, _T("%04d-%02d-%02d %02d:%02d:%02d:%03d"), year,
                month, mday, hours, minutes, seconds, ms);
    ISOString[23] = '\0';
    return ISOString;
}
