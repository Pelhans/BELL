...
DocumentFieldEnumeration::DocumentFieldEnumeration(
    Document::FieldsType::iterator itr, Document::FieldsType::iterator end) {
...
Document::Document() : _fields(_CLNEW FieldsType(true)) {
    // Func - Constructor
    // Pre  - true
...
void Document::add(Field& field) { _fields->push_back(&field); }

void Document::setBoost(const float_t boost) { this->boost = boost; }

float_t Document::getBoost() const { return boost; }
...
Field* Document::getField(const TCHAR* name) const {
    ...
    for (FieldsType::const_iterator itr = _fields->begin();
         itr != _fields->end(); itr++) {
        if (_tcscmp((*itr)->name(), name) == 0) return *itr;
    }
    return NULL;
}
...
const Document::FieldsType* Document::getFields() const { return _fields; }
...
void Document::removeField(const TCHAR* name) {
    ...
    for (FieldsType::iterator itr = _fields->begin(); itr != _fields->end();
         itr++) {
...
TCHAR** Document::getValues(const TCHAR* name) {
    ...
    // todo: there must be a better way of doing this, we are doing two
