 ...
 * Hits#doc(int32_t, Document*)}, {@link Searcher#doc(int32_t, Document*)} or
 * {@link IndexReader#document(int32_t, Document*)}.
 */
class CLUCENE_EXPORT Document : LUCENE_BASE {
public:
    typedef CL_NS(util)::CLArrayList<
        Field*, CL_NS(util)::Deletor::Object<Field>> FieldsType;
...
public:
    ...
    /** Returns, at indexing time, the boost factor as set by {@link
     * #setBoost(float_t)}.
     *
     ...
     * an index. These methods cannot be used to change the content of an
     * existing index! In order to achieve this, a document has to be deleted
     * from an index and a new changed version of that document has to be
     * added.</p> Note: name is case sensitive
     */
     ...
     * unchanged.</p> <p> Note that the removeField(s) methods like the add
     * method only make sense prior to adding a document to an index. These
     * methods cannot be used to change the content of an existing index! In
     * order to achieve this, a document has to be deleted from an index and a
     * new changed version of that document has to be added.</p> Note: name is
     * case sensitive
     */
     ...
     * in this document, or null.  If multiple fields exist with this name, this
     * method returns the first value added. If only binary fields with this
     * name exist, returns null. Note: name is case sensitive
     */
     ...
     * <p>Note that fields which are <i>not</i> {@link Field#isStored() stored}
     ...
     * parameter. This method can return <code>null</code>. Note: name is case
     * sensitive
     *
     * @param name the name of the field
...
public:
    DocumentFieldEnumeration(Document::FieldsType::iterator itr,
