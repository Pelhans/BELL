...
Field::Field(const TCHAR* Name, Reader* reader, int config) : lazy(false) {
...
Field::Field(const TCHAR* Name, const TCHAR* Value, int _config,
             const bool duplicateValue)
    : lazy(false) {
    CND_PRECONDITION(Name != NULL, "Name cannot be NULL");
    CND_PRECONDITION(Value != NULL, "value cannot be NULL");
    CND_PRECONDITION(_tcslen(Value) > 0 || _tcslen(Name) > 0,
                     "name and value cannot both be empty");

    /*
    if (_config & INDEX_NO && _config & STORE_NO)
        _CLTHROWA(CL_ERR_IllegalArgument,"it doesn't make sense to have a field
    that is neither indexed nor stored"); if (_config & INDEX_NO && _config &
    TERMVECTOR_YES) _CLTHROWA(CL_ERR_IllegalArgument,"cannot store term vector
    information for a field that is not indexed");
    */
...
Field::Field(const TCHAR* Name, ValueArray<uint8_t>* Value, int config,
             bool duplicateValue)
    : lazy(false) {
    CND_PRECONDITION(Name != NULL, "Name cannot be NULL");
    CND_PRECONDITION(Value != NULL, "value cannot be NULL");
...
Field::Field(const TCHAR* Name, int config) : lazy(false) {
    CND_PRECONDITION(Name != NULL, "Name cannot be NULL");
...
Reader* Field::readerValue() {
    return (valueType & VALUE_READER) ? static_cast<Reader*>(fieldsData) : NULL;
} ///< returns reference
...
bool Field::isStoreOffsetWithTermVector() const {
    return (config & TERMVECTOR_YES) != 0 &&
           (config & TERMVECTOR_WITH_OFFSETS) != 0 &&
           ((config & TERMVECTOR_WITH_OFFSETS) != TERMVECTOR_YES);
}
bool Field::isStorePositionWithTermVector() const {
    return (config & TERMVECTOR_YES) != 0 &&
           (config & TERMVECTOR_WITH_POSITIONS) != 0 &&
           ((config & TERMVECTOR_WITH_POSITIONS) != TERMVECTOR_YES);
}
...
void Field::setConfig(const uint32_t x) {
    ...
    if ((x & STORE_YES) || (x & STORE_COMPRESS)) {
        newConfig |= STORE_YES;
        if (x & STORE_COMPRESS) newConfig |= STORE_COMPRESS;
    } else
        newConfig |= STORE_NO;
    ...
    if ((x & INDEX_NO) == 0) {
        ...
        if (x & INDEX_TOKENIZED && x & INDEX_UNTOKENIZED)
            _CLTHROWA(CL_ERR_IllegalArgument,
                      "it doesn't make sense to have an untokenised and "
                      "tokenised field");
        ...
        if (!index) newConfig |= INDEX_NO;
    } else
        newConfig |= INDEX_NO;

    if (newConfig & INDEX_NO && newConfig & STORE_NO)
        _CLTHROWA(CL_ERR_IllegalArgument,
                  "it doesn't make sense to have a field that is neither "
                  "indexed nor stored");
    ...
    if ((x & TERMVECTOR_NO) == 0) {
        ...
        if ((x & TERMVECTOR_WITH_POSITIONS) &&
            ((x & TERMVECTOR_WITH_POSITIONS) != TERMVECTOR_YES)) {
            newConfig |= TERMVECTOR_WITH_POSITIONS;
            termVector = true;
        }
        if (x & TERMVECTOR_WITH_OFFSETS &&
            ((x & TERMVECTOR_WITH_OFFSETS) != TERMVECTOR_YES)) {
        ...
        if (termVector) {
            if (newConfig & INDEX_NO)
                _CLTHROWA(CL_ERR_IllegalArgument,
                          "cannot store a term vector for fields that are not "
                          "indexed.");
...
TCHAR* Field::toString() {
    ...
    if (isStoreOffsetWithTermVector()) {
        if (result.length() > 0) result.appendChar(',');
...
const char* Field::getObjectName() const { return getClassName(); }
