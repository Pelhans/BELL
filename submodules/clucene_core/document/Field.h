...
TODO: - Solve some inconsistencies between CL and JL - mainly in the
constructors area.
      - Write some more tests to make sure we conform with JL - mainly in the
tokenizing and omitNorms area
      - Is there a bug in JL when calling setOmitNorms after a Tokenized field
was created?
*/
...
public:
    ...
    enum TermVector {
        ...
        TERMVECTOR_WITH_POSITIONS_OFFSETS =
            TERMVECTOR_WITH_OFFSETS | TERMVECTOR_WITH_POSITIONS
    };
     ...
     * @memory Set duplicateValue to false to save on memory allocations when
     * possible
     */
    Field(const TCHAR* name, const TCHAR* value, int _config,
          const bool duplicateValue = true);
     ...
     * possible
     */
    Field(const TCHAR* name, CL_NS(util)::ValueArray<uint8_t>* data,
          int _config, const bool duplicateValue = true);

    Field(const TCHAR* name,
          int _config); ///< No value, for lazy loading support
    virtual ~Field();
    ...
    //	indexing.  Un-tokenized fields are indexed as a single word and may not
    // be 	Reader-valued.
     ...
     *  vector, available from {@link
     * IndexReader#getTermFreqVector(int32_t,TCHAR*)}. These methods do not
     * provide access to the original content of the field, only to terms used
     * to index it. If the original content must be preserved, use the
     * <code>stored</code> attribute instead.
     *
     ...
     *
     * <p>Note: this value is not stored directly with the document in the
     * index. Documents returned from {@link IndexReader#document(int)} and
     ...
     * rounded by {@link Similarity#encodeNorm(float_t)} before it is stored in
     * the index.  One should attempt to ensure that this product does not
     * overflow the range of that encoding.
     *
     ...
     * the {@link org.apache.lucene.index.IndexReader} that retrieved the {@link
     ...
     *  for details.</p>
     *
     * @memory Caller is responsible for releasing value if duplicateValue ==
     * false */
