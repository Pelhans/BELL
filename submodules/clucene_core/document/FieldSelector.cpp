...
FieldSelector::FieldSelectorResult LoadFirstFieldSelector::accept(
    const TCHAR* /*fieldName*/) const {
    return LOAD_AND_BREAK;
}

MapFieldSelector::~MapFieldSelector() { _CLDELETE(fieldSelections); }
...
FieldSelector::FieldSelectorResult MapFieldSelector::accept(
...
void MapFieldSelector::add(const TCHAR* field,
                           FieldSelector::FieldSelectorResult action) {
    fieldSelections->insert(
        fieldSelections->end(),
        std::pair<TCHAR*, FieldSelectorResult>(STRDUP_TtoT(field), action));
}
