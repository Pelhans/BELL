...
/**
 * Similar to a {@link java.io.FileFilter}, the FieldSelector allows one to make
 *decisions about what Fields get loaded on a {@link Document} by {@link
 *org.apache.lucene.index.IndexReader#document(int,org.apache.lucene.document.FieldSelector)}
 *
 **/
...
public:
    ...
    enum FieldSelectorResult {
         ...
         *  {@link Document#getField(String)} and {@link
         *Document#getField(String)} should not return null. <p/>
         * {@link Document#add(Field)} should be called by the Reader.
         */
        LOAD = 0,

        /**
         * Lazily load this {@link Field}.  This means the {@link Field} is
         *valid, but it may not actually contain its data until invoked.  {@link
         *Document#getField(String)} SHOULD NOT BE USED.  {@link
         *Document#getField(String)} is safe to use and should return a valid
         *instance of a {@link Field}. <p/>
         * {@link Document#add(Field)} should be called by the Reader.
         */
        LAZY_LOAD = 1,
         ...
         * {@link Document#getField(String)} should return null.
        ...
        /**
         * Load this field as in the {@link #LOAD} case, but immediately return
         * from {@link Field} loading for the {@link Document}.  Thus, the
         * Document may not have its complete set of Fields.  {@link
         * Document#getField(String)} and {@link Document#getField(String)}
         * should both be valid for this {@link Field} <p/>
         * {@link Document#add(Field)} should be called by the Reader.
         */
        LOAD_AND_BREAK = 3,

        /**
         * Behaves much like {@link #LOAD} but does not uncompress any
         * compressed data.  This is used for internal purposes.
         * {@link Document#getField(String)} and {@link
         * Document#getField(String)} should not return null. <p/>
         * {@link Document#add(Field)} should be called by the Reader.
         */
         ...
         * Size is measured as number of bytes required to store the field ==
         * bytes for a binary or any compressed value, and 2*chars for a String
         * value. The size is stored as a binary value, represented as an int in
         * a byte[], with the higher order byte first in [0]
         */
        SIZE = 5,

        /** Expert: Like {@link #SIZE} but immediately break from the field
           loading loop, i.e., stop loading further fields, after the size is
           loaded 		*/
        SIZE_AND_BREAK = 6
    };
...
public:
    typedef CL_NS(util)::CLHashMap<
        TCHAR*, FieldSelectorResult, CL_NS(util)::Compare::TChar,
        CL_NS(util)::Equals::TChar, CL_NS(util)::Deletor::tcArray,
    ...
    void add(const TCHAR*,
             FieldSelector::FieldSelectorResult action = FieldSelector::LOAD);
};
