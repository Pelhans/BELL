...
const int32_t DocumentsWriter::nextLevelArray[10] = {1, 2, 3, 4, 5,
                                                     6, 7, 8, 9, 9};
const int32_t DocumentsWriter::levelSizeArray[10] = {5,  14, 20, 30,  40,
                                                     40, 80, 80, 120, 200};
const int32_t DocumentsWriter::POSTING_NUM_BYTE =
    OBJECT_HEADER_BYTES + 9 * INT_NUM_BYTE + 5 * POINTER_NUM_BYTE;

const int32_t DocumentsWriter::BYTE_BLOCK_SHIFT = 15;
const int32_t DocumentsWriter::BYTE_BLOCK_SIZE =
    (int32_t)pow(2.0, BYTE_BLOCK_SHIFT);
...
const int32_t DocumentsWriter::CHAR_BLOCK_SIZE =
    (int32_t)pow(2.0, CHAR_BLOCK_SHIFT);
const int32_t DocumentsWriter::CHAR_BLOCK_MASK = CHAR_BLOCK_SIZE - 1;

int32_t DocumentsWriter::OBJECT_HEADER_BYTES = 8;
int32_t DocumentsWriter::OBJECT_POINTER_BYTES =
    4; // TODO: should be 8 on 64-bit platform
...
const int32_t DocumentsWriter::MAX_TERM_LENGTH =
    DocumentsWriter::CHAR_BLOCK_SIZE - 1;
...
DocumentsWriter::DocumentsWriter(CL_NS(store)::Directory* directory,
                                 IndexWriter* writer)
    : bufferedDeleteTerms(_CLNEW TermNumMapType(true, true)),
      ...
      waitingThreadStates(
          CL_NS(util)::ValueArray<ThreadState*>(MAX_THREAD_STATE)) {
    numBytesAlloc = 0;
    numBytesUsed = 0;
    ...
    ramBufferSize =
        (int64_t)(IndexWriter::DEFAULT_RAM_BUFFER_SIZE_MB * 1024 * 1024);
    ...
    *copyByteBuffer = 0;
    ...
    fieldsWriter = NULL;
    tvx = tvf = tvd = NULL;
    postingsFreeCountDW = postingsAllocCountDW = numWaiting = pauseThreads =
        abortCount = 0;
...
int32_t DocumentsWriter::getMaxBufferedDocs() { return maxBufferedDocs; }

std::string DocumentsWriter::getSegment() { return segment; }

int32_t DocumentsWriter::getNumDocsInRAM() { return numDocsInRAM; }

const std::string& DocumentsWriter::getDocStoreSegment() {
    return docStoreSegment;
}

int32_t DocumentsWriter::getDocStoreOffset() { return docStoreOffset; }
...
std::string DocumentsWriter::closeDocStore() {
    ...
    if (infoStream != NULL)
        (*infoStream) << string("\ncloseDocStore: ")
                      << Misc::toString((int32_t)flushedFiles.size())
                      << string(" files to flush to segment ")
                      << docStoreSegment << string(" numDocs=")
                      << Misc::toString(numDocsInStore) << string("\n");
                ...
                directory->fileLength(
                    (docStoreSegment + "." +
                     IndexFileNames::VECTORS_INDEX_EXTENSION)
                        .c_str())); // "after flush: tvx size mismatch: " +
                                    // numDocsInStore + " docs vs " +
                ...
                directory->fileLength(
                    (docStoreSegment + "." +
                     IndexFileNames::FIELDS_INDEX_EXTENSION)
                        .c_str())); // "after flush: fdx size mismatch: " +
                                    // numDocsInStore + " docs vs " +
                                    // directory->fileLength(docStoreSegment +
                                    // "." +
                                    // IndexFileNames::FIELDS_INDEX_EXTENSION) +
                                    // " length in bytes of " + docStoreSegment
                                    // + "." +
                                    // IndexFileNames::FIELDS_INDEX_EXTENSION;
        }
...
void DocumentsWriter::abort(AbortException* ae) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                for (std::vector<string>::const_iterator itr =
                         __abortedFiles.begin();
    ...
    _CLFINALLY(if (ae != NULL) abortCount--;
...
bool DocumentsWriter::allThreadsIdle() {
    ...
    for (size_t i = 0; i < threadStates.length; i++)
        if (!threadStates[i]->isIdle) return false;
    return true;
}
...
int32_t DocumentsWriter::flush(bool _closeDocStore) {
    ...
    if (infoStream != NULL)
        (*infoStream) << string("\nflush postings as segment ") << segment
                      << string(" numDocs=") << Misc::toString(numDocsInRAM)
                      << string("\n");
    ...
    _CLFINALLY(if (!success) abort(NULL);)

    return docCount;
}

void DocumentsWriter::createCompoundFile(const std::string& segment) {
    CompoundFileWriter* cfsWriter = _CLNEW CompoundFileWriter(
        directory,
        (segment + "." + IndexFileNames::COMPOUND_FILE_EXTENSION).c_str());
...
void DocumentsWriter::writeNorms(const std::string& segmentName,
                                 int32_t totalNumDoc) {
    IndexOutput* normsOut = directory->createOutput(
        (segmentName + "." + IndexFileNames::NORMS_EXTENSION).c_str());

    try {
        normsOut->writeBytes(SegmentMerger::NORMS_HEADER,
            ...
            if (fi->isIndexed && !fi->omitNorms) {
                ...
                if (v < totalNumDoc)
                    fillBytes(normsOut, defaultNorm,
                              (int32_t)(totalNumDoc - v));
            }
        }
    }
    _CLFINALLY(normsOut->close(); _CLDELETE(normsOut);)
}
...
void DocumentsWriter::writeSegment(std::vector<std::string>& flushedFiles) {
    ...
    TermInfosWriter* termsOut =
        _CLNEW TermInfosWriter(directory, segmentName.c_str(), fieldInfos,
                               writer->getTermIndexInterval());

    IndexOutput* freqOut =
        directory->createOutput((segmentName + ".frq").c_str());
    IndexOutput* proxOut =
    ...
    for (size_t i = 0; i < threadStates.length; i++) {
        ...
        for (int32_t j = 0; j < numFields; j++) {
            ...
            if (fp->numPostings > 0) allFields.push_back(fp);
        }
    }
    ...
    skipListWriter = _CLNEW DefaultSkipListWriter(
        termsOut->skipInterval, termsOut->maxSkipLevels, numDocsInRAM, freqOut,
        proxOut);
    ...
    while (start < numAllFields) {
        ...
        while (end < numAllFields &&
               _tcscmp(allFields[end]->fieldInfo->name, fieldName) == 0)
            end++;
    ...
    flushedFiles.push_back(
        segmentFileName(IndexFileNames::FIELD_INFOS_EXTENSION));
    ...
    flushedFiles.push_back(
        segmentFileName(IndexFileNames::TERMS_INDEX_EXTENSION));

    if (hasNorms) {
        writeNorms(segmentName, numDocsInRAM);
        flushedFiles.push_back(
            segmentFileName(IndexFileNames::NORMS_EXTENSION));
    }
    ...
    if (infoStream != NULL) {
        ...
        (*infoStream) << string("  oldRAMSize=") << Misc::toString(numBytesUsed)
                      << string(" newFlushedSize=")
                      << Misc::toString(newSegmentSize) << string(" docs/MB=")
                      << Misc::toString((float_t)(
                             numDocsInRAM / (newSegmentSize / 1024.0 / 1024.0)))
                      << string(" new/old=")
                      << Misc::toString(
                             (float_t)(100.0 * newSegmentSize / numBytesUsed))
                      << string("%\n");
    }
...
void DocumentsWriter::appendPostings(ArrayBase<ThreadState::FieldData*>* fields,
    ...
    for (int32_t i = 0; i < numFields; i++) {
        FieldMergeState* fms = mergeStatesData.values[i] =
            _CLNEW FieldMergeState();
        fms->field = (*fields)[i];
    ...
    memcpy(mergeStates.values, mergeStatesData.values,
           sizeof(FieldMergeState*) * numFields);
    ...
    while (numFields > 0) {
        ...
        termInfo.set(df, freqPointer, proxPointer,
                     (int32_t)(skipPointer - freqPointer));
...
DocumentsWriter::ThreadState* DocumentsWriter::getThreadState(Document* doc,
                                                              Term* delTerm) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    ...
    if (threadBindings.find(_LUCENE_CURRTHREADID) == threadBindings.end()) {
        ...
        if (minThreadState != NULL &&
            (minThreadState->numThreads == 0 ||
             threadStates.length == MAX_THREAD_STATE)) {
            ...
            state = threadStates.values[threadStates.length - 1] =
                _CLNEW ThreadState(this);
        }
    ...
    while (!closed && (!state->isIdle || pauseThreads != 0 || flushPending ||
                       abortCount > 0))
        CONDITION_WAIT(THIS_LOCK, THIS_WAIT_CONDITION)

    if (closed) _CLTHROWA(CL_ERR_AlreadyClosed, "this IndexWriter is closed");

    if (segment.empty()) segment = writer->newSegmentName();
...
bool DocumentsWriter::updateDocument(Term* t, Document* doc,
                                     Analyzer* analyzer) {
    return updateDocument(doc, analyzer, t);
}

bool DocumentsWriter::updateDocument(Document* doc, Analyzer* analyzer,
                                     Term* delTerm) {
...
const DocumentsWriter::TermNumMapType&
DocumentsWriter::getBufferedDeleteTerms() {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
...
void DocumentsWriter::clearBufferedDeletes() {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    DocumentsWriter::TermNumMapType::iterator term =
        bufferedDeleteTerms->begin();
    ...
    bufferedDeleteDocIDs.clear();
    numBufferedDeleteTerms = 0;
    if (numBytesUsed > 0) resetPostingsData();
}
...
void DocumentsWriter::setMaxBufferedDeleteTerms(
    int32_t _maxBufferedDeleteTerms) {
...
void DocumentsWriter::addDeleteTerm(Term* term, int32_t docCount) {
    ...
    if (num == NULL) {
        ...
        numBytesUsed +=
            (_tcslen(term->field()) + term->textLength()) * BYTES_PER_CHAR + 4 +
            5 * OBJECT_HEADER_BYTES + 5 * OBJECT_POINTER_BYTES;
        if (ramBufferSize != IndexWriter::DISABLE_AUTO_FLUSH &&
                ...
                for (int32_t i = 0; i < numWaiting;) {
                    ...
                    if (s->docID == nextWriteDocID) {
                        ...
                        if (numWaiting > i + 1)
                            ...
                            // inconsistent state:
                            waitingThreadStates.values[i] =
                                waitingThreadStates[numWaiting - 1];
                        numWaiting--;
                    } else {
                        assert(!s->isIdle);
                        i++;
...
int64_t DocumentsWriter::getRAMUsed() { return numBytesUsed; }

void DocumentsWriter::fillBytes(IndexOutput* out, uint8_t b, int32_t numBytes) {
    for (int32_t i = 0; i < numBytes; i++) out->writeByte(b);
}

void DocumentsWriter::copyBytes(IndexInput* srcIn, IndexOutput* destIn,
                                int64_t numBytes) {
...
void DocumentsWriter::getPostings(ValueArray<Posting*>& postings) {
    ...
    if (numToCopy > 0) {
        memcpy(postings.values, this->postingsFreeListDW.values + start,
               sizeof(Posting*) * numToCopy);
    }
    ...
    if (numToCopy < postings.length) {
        ...
        const int32_t newPostingsAllocCount =
            this->postingsAllocCountDW + extra;
        if (newPostingsAllocCount > this->postingsFreeListDW.length)
            this->postingsFreeListDW.resize(
                (int32_t)(1.25 * newPostingsAllocCount));
...
void DocumentsWriter::recyclePostings(ValueArray<Posting*>& postings,
                                      int32_t numPostings) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    // Move all Postings from this ThreadState back to our
    // free list.  We pre-allocated this array while we were
    // creating Postings to make sure it's large enough
    assert(this->postingsFreeCountDW + numPostings <=
           this->postingsFreeListDW.length);
    if (numPostings > 0)
        memcpy(this->postingsFreeListDW.values + this->postingsFreeCountDW,
               postings.values, numPostings * sizeof(Posting*));
...
void DocumentsWriter::recycleBlocks(ArrayBase<uint8_t*>& blocks, int32_t start,
                                    int32_t end) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    for (int32_t i = start; i < end; i++) {
...
void DocumentsWriter::recycleBlocks(ArrayBase<TCHAR*>& blocks, int32_t start,
                                    int32_t numBlocks) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    ...
    if (numBytesAlloc > freeTrigger) {
        if (infoStream != NULL)
            (*infoStream) << string("  RAM: now balance allocations: usedMB=")
                          << toMB(numBytesUsed) + string(" vs trigger=")
                          << toMB(flushTrigger) << string(" allocMB=")
                          << toMB(numBytesAlloc) << string(" vs trigger=")
                          << toMB(freeTrigger) << string(" postingsFree=")
                          << toMB(this->postingsFreeCountDW * POSTING_NUM_BYTE)
                          << string(" byteBlockFree=")
                          << toMB(freeByteBlocks.size() * BYTE_BLOCK_SIZE)
                          << string(" charBlockFree=")
        ...
        const int32_t postingsFreeChunk =
            (int32_t)(BYTE_BLOCK_SIZE / POSTING_NUM_BYTE);
        ...
        while (numBytesAlloc > freeLevel) {
            ...
            if ((2 == iter % 3) && this->postingsFreeCountDW > 0) {
                ...
                for (size_t i = this->postingsFreeCountDW - numToFree;
                     i < this->postingsFreeCountDW; i++) {
        ...
        if (infoStream != NULL) {
            (*infoStream) << "    after free: freedMB=" +
                                 Misc::toString((float_t)(
                                     (startBytesAlloc - numBytesAlloc) /
                                     1024.0 / 1024.0)) +
                                 " usedMB=" +
                                 Misc::toString((float_t)(numBytesUsed /
                                                          1024.0 / 1024.0)) +
                                 " allocMB=" +
                                 Misc::toString(
                                     (float_t)(numBytesAlloc / 1024.0 / 1024.0))
                          << string("\n");
        }
        ...
        if (numBytesUsed > flushTrigger) {
            if (infoStream != NULL) {
                (*infoStream)
                    << string("  RAM: now flush @ usedMB=")
                    << Misc::toString((float_t)(numBytesUsed / 1024.0 / 1024.0))
                    << string(" allocMB=")
                    << Misc::toString(
                           (float_t)(numBytesAlloc / 1024.0 / 1024.0))
                    << string(" triggerMB=")
                    << Misc::toString((float_t)(flushTrigger / 1024.0 / 1024.0))
                    << string("\n");
            }
...
DocumentsWriter::BufferedNorms::BufferedNorms() { this->upto = 0; }
...
bool DocumentsWriter::FieldMergeState::nextTerm() {
    ...
    text =
        field->threadState->charPool->buffers[p->textStart >> CHAR_BLOCK_SHIFT];
...
void DocumentsWriter::ByteSliceReader::init(ByteBlockPool* _pool,
                                            int32_t _startIndex,
                                            int32_t _endIndex) {
...
DocumentsWriter::ByteBlockPool::ByteBlockPool(bool _trackAllocations,
...
int32_t DocumentsWriter::ByteBlockPool::newSlice(const int32_t size) {
    if (tUpto > BYTE_BLOCK_SIZE - size) nextBuffer();
...
int32_t DocumentsWriter::ByteBlockPool::allocSlice(uint8_t* slice,
                                                   const int32_t upto) {
    ...
    if (tUpto > BYTE_BLOCK_SIZE - newSize) nextBuffer();
