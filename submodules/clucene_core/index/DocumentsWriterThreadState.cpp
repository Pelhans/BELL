...
DocumentsWriter::ThreadState::ThreadState(DocumentsWriter* __parent)
    ...
    this->docBoost = 0.0;
    this->fieldGen = this->posUpto = this->maxPostingsVectors =
        this->numStoredFields = 0;
    this->numAllFieldData = this->docID = 0;
    this->numFieldData = numVectorFields = this->proxUpto = this->freqUpto =
        this->offsetUpto = 0;
...
void DocumentsWriter::ThreadState::resetPostings() {
    ...
    postingsPool->reset();
    charPool->reset();
    _parent->recyclePostings(this->postingsFreeListTS,
                             this->postingsFreeCountTS);
    ...
    try {
        ...
        for (int32_t i = 0; i < numFieldData; i++) {
            ...
            if (fp->doNorms) {
                ...
                bn->fill(docID);
                float_t norm =
                    fp->boost * _parent->writer->getSimilarity()->lengthNorm(
...
void DocumentsWriter::ThreadState::init(Document* doc, int32_t docID) {
    assert(!isIdle);
    assert(
        _parent->writer->testPoint("DocumentsWriter.ThreadState.init start"));
    ...
    for (int32_t i = 0; i < numDocFields; i++) {
        ...
        FieldInfo* fi = _parent->fieldInfos->add(
            field->name(), field->isIndexed(), field->isTermVectorStored(),
            field->isStorePositionWithTermVector(),
            field->isStoreOffsetWithTermVector(), field->getOmitNorms(), false);
        ...
        int32_t hashPos = Misc::thashCode(fi->name) &
                          fieldDataHashMask; // TODO: put hash in fieldinfo
        ...
        if (fp == NULL) {
            ...
            if (numAllFieldData == allFieldDataArray.length) {
                allFieldDataArray.resize(
                    (int32_t)(allFieldDataArray.length * 1.5));
                ...
                for (size_t j = 0; j < fieldDataHash.length; j++) {
                    ...
                    while (fp0 != NULL) {
                        // todo: put hash code into fieldinfo to reduce number
                        // of hashes necessary
                        hashPos = Misc::thashCode(fp0->fieldInfo->name) &
                                  fieldDataHashMask;
                ...
                memcpy(fieldDataHash.values, newHashArray.values,
                       newHashArray.length * sizeof(FieldData*));
            }
    ...
    if (localFieldsWriter == NULL) {
        if (_parent->fieldsWriter == NULL) {
            ...
            // state:
            try {
                _parent->fieldsWriter = _CLNEW FieldsWriter(
                    _parent->directory, _parent->docStoreSegment.c_str(),
                    _parent->fieldInfos);
            } catch (CLuceneError& t) {
                throw AbortException(t, _parent);
            }
        ...
        localFieldsWriter =
            _CLNEW FieldsWriter(NULL, fdtLocal, _parent->fieldInfos);
    }
...
void DocumentsWriter::ThreadState::doPostingSort(Posting** postings,
                                                 int32_t numPosting) {
    quickSort(postings, 0, numPosting - 1);
}

void DocumentsWriter::ThreadState::quickSort(Posting** postings, int32_t lo,
                                             int32_t hi) {
    if (lo >= hi) return;
    ...
    for (;;) {
        while (comparePostings(postings[right], partition) > 0) --right;
...
void DocumentsWriter::ThreadState::doVectorSort(
    ArrayBase<PostingVector*>& postings, int32_t numPosting) {
    quickSort(postings, 0, numPosting - 1);
}

void DocumentsWriter::ThreadState::quickSort(
    ArrayBase<PostingVector*>& postings, int32_t lo, int32_t hi) {
    if (lo >= hi) return;
...
void DocumentsWriter::ThreadState::trimFields() {
    ...
    for (int32_t i = 0; i < numAllFieldData; i++) {
        ...
        if (fp->lastGen == -1) {
            ...
            // Unhash
            const int32_t hashPos =
                Misc::thashCode(fp->fieldInfo->name) & fieldDataHashMask;
            FieldData* last = NULL;
            ...
            if (_parent->infoStream != NULL)
                (*_parent->infoStream)
                    << "  remove field=" << fp->fieldInfo->name << "\n";
            ...
            if (fp->numPostings > 0 &&
                ((float_t)fp->numPostings) / fp->postingsHashSize < 0.2) {
...
void DocumentsWriter::ThreadState::processDocument(Analyzer* analyzer) {
    ...
    if (_parent->tvx != NULL) {
        ...
        // here.
        Arrays<FieldData*>::sort(fieldDataArray.values, fieldDataArray.length,
                                 0, numFields);
    }
    ...
    if (maxTermPrefix != NULL && _parent->infoStream != NULL)
        (*_parent->infoStream)
            << "WARNING: document contains at least one immense term (longer "
               "than the max length "
            << MAX_TERM_LENGTH
            << "), all of which were skipped.  Please correct the analyzer to "
               "not produce such terms.  The prefix of the first immense term "
               "is: '"
            << maxTermPrefix << "...'\n";
...
bool DocumentsWriter::ThreadState::postingEquals(const TCHAR* tokenText,
                                                 const int32_t tokenTextLen) {
...
int32_t DocumentsWriter::ThreadState::comparePostings(Posting* p1,
                                                      Posting* p2) {
    const TCHAR* pos1 = charPool->buffers[p1->textStart >> CHAR_BLOCK_SHIFT] +
                        (p1->textStart & CHAR_BLOCK_MASK);
    const TCHAR* pos2 = charPool->buffers[p2->textStart >> CHAR_BLOCK_SHIFT] +
...
void DocumentsWriter::ThreadState::writeProxBytes(uint8_t* b, int32_t offset,
                                                  int32_t len) {
...
DocumentsWriter::ThreadState::FieldData::FieldData(DocumentsWriter* __parent,
                                                   ThreadState* __threadState,
                                                   FieldInfo* fieldInfo)
    : docFields(ValueArray<Field*>(1)),
      _parent(__parent),
      localToken(_CLNEW Token),
      vectorSliceReader(_CLNEW ByteSliceReader()) {
    this->fieldCount = this->postingsHashSize = this->postingsHashHalfSize =
        this->postingsVectorsUpto = 0;
    this->postingsHashMask = this->offsetEnd = 0;
    this->offsetStartCode = this->offsetStart = this->numPostings =
        this->position = this->length = this->offset = 0;
    this->boost = 0.0;
    this->next = NULL;
    this->lastGen = -1;
...
int32_t DocumentsWriter::ThreadState::FieldData::compareTo(NamedObject* o) {
    if (o->getObjectName() != FieldData::getClassName()) return -1;
...
CL_NS(util)::ValueArray<DocumentsWriter::Posting*>* DocumentsWriter::
    ThreadState::FieldData::sortPostings() {
    compactPostings();
...
void DocumentsWriter::ThreadState::FieldData::processField(Analyzer* analyzer) {
    ...
    try {
        for (int32_t j = 0; j < limit; j++) {
            ...
            if (field->isStored()) {
                ...
                bool success = false;
                try {
                    threadState->localFieldsWriter->writeField(fieldInfo,
                                                               field);
                    success = true;
                }
                _CLFINALLY(
    ...
    _CLFINALLY(if (postingsVectorsUpto > 0) {
        try {
            if (doWriteVectors) {
        ...
        _CLFINALLY(if (postingsVectorsUpto > threadState->maxPostingsVectors)
                       threadState->maxPostingsVectors = postingsVectorsUpto;
                   postingsVectorsUpto = 0; threadState->vectorsPool->reset();)
    })
}
void DocumentsWriter::ThreadState::FieldData::invertField(
    Field* field, Analyzer* analyzer, const int32_t maxFieldLength) {
    if (length > 0)
    ...
    if (!field->isTokenized()) { // un-tokenized field
            ...
            if (readerValue != NULL)
                ...
                if (stringValue == NULL)
                    _CLTHROWA(CL_ERR_IllegalArgument,
                              "field must have either TokenStream, String or "
                              "Reader value");
            ...
            for (;;) {
                ...
                if (maxFieldLength != IndexWriter::FIELD_TRUNC_POLICY__WARN) {
                    ...
                    const TCHAR* errMsgBase =
                        ...
                        _T("LUCENE_INT32_MAX")
                        _T(" to disable truncation, or a value to specify ")
                        _T("maximum number of fields).");
                    ...
                    _sntprintf(errMsg, errMsgLen, errMsgBase, fieldInfo->name,
                               defaultMaxAsChar);
        ...
        _CLFINALLY(stream->close(); // don't delete, this stream is re-used
        )
    }
...
DocumentsWriter::PostingVector*
DocumentsWriter::ThreadState::FieldData::addNewVector() {
    ...
    if (threadState->p->vector == NULL)
        threadState->p->vector =
            threadState->postingsVectors.values[postingsVectorsUpto] =
                _CLNEW PostingVector();
    ...
    if (doVectorOffsets) {
        const int32_t upto = threadState->vectorsPool->newSlice(firstSize);
        v->offsetStart = v->offsetUpto =
            threadState->vectorsPool->tOffset + upto;
    }
...
void DocumentsWriter::ThreadState::FieldData::addPosition(Token* token) {
    ...
    while (downto > 0) code = (code * 31) + tokenText[--downto];
    /*
      std::cout << "  addPosition: buffer=" <<
      Misc::toString(tokenText).substr(0,tokenTextLen) << " pos=" << position
                << " offsetStart=" << (offset+token->startOffset()) << "
      offsetEnd=" << (offset + token->endOffset())
                << " docID=" << threadState->docID << " doPos=" <<
      (doVectorPositions?"true":"false") << " doOffset=" <<
    ...
    */
    ...
    try {
        if (threadState->p != NULL) { // term seen since last flush
            ...
            if (threadState->docID !=
                threadState->p->lastDocID) { // term not yet seen in this doc
                                             /*
                                                     std::cout << "    seen before (new docID=" <<
                                                threadState->docID << ") freqUpto=" <<
                                                threadState->p->freqUpto
                ...
                threadState->freqUpto =
                    threadState->p->freqUpto & BYTE_BLOCK_MASK;
                threadState->freq =
                    threadState->postingsPool
                        ->buffers[threadState->p->freqUpto >> BYTE_BLOCK_SHIFT];
                ...
                threadState->p->freqUpto =
                    threadState->freqUpto +
                    (threadState->p->freqUpto & BYTE_BLOCK_NOT_MASK);
                ...
                threadState->p->lastDocCode =
                    (threadState->docID - threadState->p->lastDocID) << 1;
                threadState->p->lastDocID = threadState->docID;

            } else { // term already seen in this doc
                     // std::cout << "    seen before (same docID=" <<
                // threadState->docID << ") proxUpto=" <<
                // threadState->p->proxUpto << "\n";
                ...
                if (doVectors) {
                    ...
                    if (doVectorOffsets) {
                        ...
                        offsetEnd = offset + token->endOffset();
                        offsetStartCode =
                            offsetStart - threadState->vector->lastOffset;
                    }
                }
            }
            ...
            if (0 == threadState->postingsFreeCountTS) {
                ...
                threadState->postingsFreeCountTS =
                    threadState->postingsFreeListTS.length;
            }
            ...
            threadState->p =
                threadState
                    ->postingsFreeListTS[--threadState->postingsFreeCountTS];
            assert(threadState->p != NULL);
            threadState->p->textStart =
                textUpto + threadState->charPool->tOffset - text;
            ...
            const int32_t upto1 =
                threadState->postingsPool->newSlice(firstSize);
            threadState->p->freqStart = threadState->p->freqUpto =
                threadState->postingsPool->tOffset + upto1;

            const int32_t upto2 =
                threadState->postingsPool->newSlice(firstSize);
            threadState->p->proxStart = threadState->p->proxUpto =
                threadState->postingsPool->tOffset + upto2;
        ...
        threadState->p->proxUpto =
            threadState->proxUpto +
            (threadState->p->proxUpto & BYTE_BLOCK_NOT_MASK);

        threadState->p->lastPosition = position++;

        if (doVectorPositions) {
            threadState->posUpto =
                threadState->vector->posUpto & BYTE_BLOCK_MASK;
            threadState->pos =
                threadState->vectorsPool
                    ->buffers[threadState->vector->posUpto >> BYTE_BLOCK_SHIFT];
            threadState->writePosVInt(proxCode);
            threadState->vector->posUpto =
                threadState->posUpto +
                (threadState->vector->posUpto & BYTE_BLOCK_NOT_MASK);
        }

        if (doVectorOffsets) {
            threadState->offsetUpto =
                threadState->vector->offsetUpto & BYTE_BLOCK_MASK;
            threadState->offsets =
                threadState->vectorsPool
                    ->buffers[threadState->vector->offsetUpto >>
                              BYTE_BLOCK_SHIFT];
            ...
            threadState->vector->offsetUpto =
                threadState->offsetUpto +
                (threadState->vector->offsetUpto & BYTE_BLOCK_NOT_MASK);
        }
    } catch (CLuceneError& t) {
        throw AbortException(t, _parent);
    }
...
void DocumentsWriter::ThreadState::FieldData::rehashPostings(
    ...
    for (int32_t i = 0; i < postingsHashSize; i++) {
        p0 = postingsHash[i];
        if (p0 != NULL) {
            start = threadState->charPool
                        ->buffers[p0->textStart >> CHAR_BLOCK_SHIFT] +
                    (p0->textStart & CHAR_BLOCK_MASK);
            pos = start;
            while (*pos != CLUCENE_END_OF_WORD) pos++;
            code = 0;
            while (pos > start) code = (code * 31) + *--pos;
...
void DocumentsWriter::ThreadState::FieldData::writeVectors(
    FieldInfo* fieldInfo) {
    assert(fieldInfo->storeTermVector);

    threadState->vectorFieldNumbers.values[threadState->numVectorFields] =
        fieldInfo->number;
    threadState->vectorFieldPointers.values[threadState->numVectorFields] =
        threadState->tvfLocal->getFilePointer();
    ...
    for (int32_t j = 0; j < numPostingsVectors; j++) {
        ...
        const TCHAR* text2 =
            threadState->charPool
                ->buffers[posting->textStart >> CHAR_BLOCK_SHIFT];
        ...
        if (lastPosting == NULL)
            ...
            const TCHAR* text1 =
                threadState->charPool
                    ->buffers[lastPosting->textStart >> CHAR_BLOCK_SHIFT];
            const TCHAR* start1 =
                text1 + (lastPosting->textStart & CHAR_BLOCK_MASK);
        ...
        // Compute length
        while (*pos2 != CLUCENE_END_OF_WORD) pos2++;
        ...
        if (doVectorPositions) {
            reader->init(threadState->vectorsPool, vector->posStart,
                         vector->posUpto);
            reader->writeTo(threadState->tvfLocal);
        }

        if (doVectorOffsets) {
            reader->init(threadState->vectorsPool, vector->offsetStart,
                         vector->offsetUpto);
            reader->writeTo(threadState->tvfLocal);
        }
    }
