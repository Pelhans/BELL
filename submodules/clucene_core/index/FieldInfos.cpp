...
FieldInfo::FieldInfo(const TCHAR* _fieldName, const bool _isIndexed,
                     const int32_t _fieldNumber, const bool _storeTermVector,
                     const bool _storeOffsetWithTermVector,
                     const bool _storePositionWithTermVector,
                     const bool _omitNorms, const bool _storePayloads)
...
FieldInfo::~FieldInfo() { CL_NS(util)::CLStringIntern::unintern(name); }

FieldInfo* FieldInfo::clone() {
    return _CLNEW FieldInfo(
        name, isIndexed, number, storeTermVector, storePositionWithTermVector,
...
void FieldInfos::addIndexed(const TCHAR** names, const bool storeTermVectors,
    ...
    size_t i = 0;
    while (names[i]) {
        add(names[i], true, storeTermVectors, storePositionWithTermVector,
            storeOffsetWithTermVector);
        ++i;
    }
}

void FieldInfos::add(const TCHAR** names, const bool isIndexed,
                     const bool storeTermVectors,
                     const bool storePositionWithTermVector,
...
FieldInfo* FieldInfos::add(const TCHAR* name, const bool isIndexed,
...
FieldInfo* FieldInfos::fieldInfo(const int32_t fieldNumber) const {
    if (fieldNumber < 0 || (size_t)fieldNumber >= byNumber.size()) return NULL;
    return byNumber[fieldNumber];
}

size_t FieldInfos::size() const { return byNumber.size(); }

bool FieldInfos::hasVectors() const {
    for (size_t i = 0; i < size(); i++) {
        if (fieldInfo(i)->storeTermVector) return true;
    }
    return false;
}
...
void FieldInfos::read(IndexInput* input) {
    ...
    uint8_t bits;
    bool isIndexed, storeTermVector, storePositionsWithTermVector,
        storeOffsetWithTermVector, omitNorms, storePayloads;
    for (int32_t i = 0; i < size; ++i) {
        TCHAR* name = input->readString(); // we could read name into a string
                                           // buffer, but we can't be sure what
