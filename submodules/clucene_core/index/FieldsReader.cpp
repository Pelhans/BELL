...
FieldsReader::FieldsReader(Directory* d, const char* segment, FieldInfos* fn,
                           int32_t _readBufferSize, int32_t _docStoreOffset,
                           int32_t size)
    : fieldInfos(fn),
    ...
    try {
        cloneableFieldsStream = d->openInput(
            Misc::segmentname(segment, ".fdt").c_str(), _readBufferSize);
        fieldsStream = cloneableFieldsStream->clone();

        indexStream = d->openInput(Misc::segmentname(segment, ".fdx").c_str(),
                                   _readBufferSize);
...
int32_t FieldsReader::size() const { return _size; }

bool FieldsReader::doc(int32_t n, Document& doc,
                       const CL_NS(document)::FieldSelector* fieldSelector) {
    if ((n + docStoreOffset) * 8L > indexStream->length()) return false;
    ...
    for (int32_t i = 0; i < numFields; i++) {
        ...
        FieldSelector::FieldSelectorResult acceptField =
            (fieldSelector == NULL) ? FieldSelector::LOAD
                                    : fieldSelector->accept(fi->name);

        uint8_t bits = fieldsStream->readByte();
        CND_CONDITION(bits <= FieldsWriter::FIELD_IS_COMPRESSED +
        ...
        // grow beyond the list of 5 or 6 currently here.  See Lucene 762 for
        // discussion
...
CL_NS(store)::IndexInput* FieldsReader::rawDocs(int32_t* lengths,
                                                const int32_t startDocID,
                                                const int32_t numDocs) {
...
void FieldsReader::addFieldLazy(CL_NS(document)::Document& doc,
                                const FieldInfo* fi, const bool binary,
                                const bool compressed, const bool tokenize) {
    if (binary) {
        int32_t toRead = fieldsStream->readVInt();
        int64_t pointer = fieldsStream->getFilePointer();
        if (compressed) {
            doc.add(*_CLNEW LazyField(this, fi->name, Field::STORE_COMPRESS,
                                      toRead, pointer));
        } else {
            doc.add(*_CLNEW LazyField(this, fi->name, Field::STORE_YES, toRead,
                                      pointer));
        }
        ...
        if (compressed) {
            int32_t toRead = fieldsStream->readVInt();
            int64_t pointer = fieldsStream->getFilePointer();
            f = _CLNEW LazyField(this, fi->name, Field::STORE_COMPRESS, toRead,
                                 pointer);
            ...
            f = _CLNEW LazyField(this, fi->name,
                                 Field::STORE_YES | getIndexType(fi, tokenize) |
                                     getTermVectorType(fi),
                                 length, pointer);
            f->setOmitNorms(fi->omitNorms);
        }
        doc.add(*f);
...
void FieldsReader::addFieldForMerge(CL_NS(document)::Document& doc,
                                    const FieldInfo* fi, const bool binary,
...
void FieldsReader::addField(CL_NS(document)::Document& doc, const FieldInfo* fi,
                            const bool binary, const bool compressed,
                            const bool tokenize) {
    ...
    if (binary) {
        ...
        if (compressed) {
            ...
            _CLDELETE(b);
            doc.add(
                *_CLNEW Field(fi->name, data, Field::STORE_COMPRESS, false));
        } else {
        ...
        if (compressed) {
            ...
            size_t l =
                lucene_utf8towcs(result, (const char*)data.values, data.length);
            result[l] = 0;

            // if we were a bit too pesimistic with the size, then shrink the
            // memory...
            if (l < data.length / 2) {
                TCHAR* tmp = result;
            ...
            f = _CLNEW Field(
                ...
                reinterpret_cast<char*>(
                    data.values), // uncompress the value and add as string
                bits, false);
#endif
            f->setOmitNorms(fi->omitNorms);
        } else {
...
int32_t FieldsReader::addFieldSize(CL_NS(document)::Document& doc,
                                   const FieldInfo* fi, const bool binary,
                                   const bool compressed) {
...
CL_NS(document)::Field::TermVector FieldsReader::getTermVectorType(
    const FieldInfo* fi) {
...
CL_NS(document)::Field::Index FieldsReader::getIndexType(const FieldInfo* fi,
                                                         const bool tokenize) {
...
FieldsReader::LazyField::LazyField(FieldsReader* _parent, const TCHAR* _name,
                                   int config, const int32_t _toRead,
                                   const int64_t _pointer)
    : Field(_name, config),
      parent(_parent) {
    // todo: need to allow for auto setting Field::INDEX_NO |
    // Field::TERMVECTOR_NO so only Store is required
    this->toRead = _toRead;
...
const ValueArray<uint8_t>* FieldsReader::LazyField::binaryValue() {
    ...
    if (fieldsData == NULL) {
        ...
        // Throw this IO Exception since IndexREader.document does so anyway, so
        // probably not that big of a change for people
        // since they are already handling this exception when getting the
        // document
        try {
...
CL_NS(util)::Reader* FieldsReader::LazyField::readerValue() {
    parent->ensureOpen();
    return (valueType & VALUE_READER)
               ? static_cast<CL_NS(util)::Reader*>(fieldsData)
               : NULL;
}

CL_NS(analysis)::TokenStream* FieldsReader::LazyField::tokenStreamValue() {
    parent->ensureOpen();
    return (valueType & VALUE_TOKENSTREAM)
               ? static_cast<CL_NS(analysis)::TokenStream*>(fieldsData)
               : NULL;
}
...
const TCHAR* FieldsReader::LazyField::stringValue() {
    ...
    if (fieldsData == NULL) {
        ...
        if (isCompressed()) {
            ...
            _resetValue();
            uncompress(
                b, uncompressed); // no need to catch error, memory all in frame

#ifndef _ASCII
            TCHAR* str = _CL_NEWARRAY(TCHAR, uncompressed.length);
            size_t l = lucene_utf8towcs(str, (const char*)uncompressed.values,
                                        uncompressed.length);
            str[l] = 0;
    ...
    return static_cast<const TCHAR*>(
        fieldsData); // instanceof String ? (String) fieldsData : null;
}
...
const CL_NS(
    util)::ValueArray<uint8_t>* FieldsReader::FieldForMerge::binaryValue() {
    return (valueType & VALUE_BINARY)
               ? static_cast<CL_NS(util)::ValueArray<uint8_t>*>(fieldsData)
               : NULL;
}

CL_NS(analysis)::TokenStream* FieldsReader::FieldForMerge::tokenStreamValue()
    const {
    // not needed for merge
    return NULL;
}

FieldsReader::FieldForMerge::FieldForMerge(void* _value, ValueType _type,
                                           const FieldInfo* fi,
                                           const bool binary,
                                           const bool compressed,
                                           const bool tokenize)
...
void FieldsReader::uncompress(const CL_NS(util)::ValueArray<uint8_t>& input,
                              CL_NS(util)::ValueArray<uint8_t>& output) {
    stringstream out;
    string err;
    ...
    output.values[length] =
        0; // null-terminate in case we want to use it as utf8
}
