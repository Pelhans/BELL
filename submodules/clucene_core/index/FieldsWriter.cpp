...
FieldsWriter::FieldsWriter(CL_NS(store)::IndexOutput* fdx,
                           CL_NS(store)::IndexOutput* fdt, FieldInfos* fn)
    : fieldInfos(fn) {
    fieldsStream = fdt;
    CND_CONDITION(fieldsStream != NULL, "fieldsStream is NULL");
    indexStream = fdx;
...
void FieldsWriter::writeField(FieldInfo* fi, CL_NS(document)::Field* field) {
    // if the field as an instanceof FieldsReader.FieldForMerge, we're in merge
    // mode and field.binaryValue() already returns the compressed value for a
    // field with isCompressed()==true, so we disable compression in that case
    bool disableCompression =
        (field->instanceOf(FieldsReader::FieldForMerge::getClassName()));
    ...
    if (field->isCompressed()) {
        ...
        if (disableCompression) {
            ...
            if (field->isBinary()) {
                compress(*field->binaryValue(), dataB);
                       ...
                       NULL) { // we must be using readerValue
                ...
                int32_t sz = r->size();
                if (sz < 0)
                    sz = 10000000; // todo: we should warn the developer
                                   // here....
        ...
        // FEATURE: this problem in Java Lucene too, if using Reader, data is
        // not stored. todo: this is a logic bug... if the field is stored, and
        // indexed, and is using a reader the field wont get indexed
        //
        // if we could write zero prefixed vints (therefore static length), then
        // we could write a reader directly to the field indexoutput and then go
        // back and write the data length. however this is not supported in
        // lucene yet... if this is ever implemented, then it would make sense
        // to also be able to combine the FieldsWriter and
        // DocumentWriter::invertDocument process, and use a streamfilter to
        // write the field data while the documentwrite analyses the document!
        // how cool would that be! it would cut out all these buffers!!!
...
void FieldsWriter::flushDocument(int32_t numStoredFields,
                                 CL_NS(store)::RAMOutputStream* buffer) {
    indexStream->writeLong(fieldsStream->getFilePointer());
...
void FieldsWriter::addRawDocuments(CL_NS(store)::IndexInput* stream,
    ...
    CND_CONDITION(fieldsStream->getFilePointer() == position,
                  "fieldsStream->getFilePointer() != position");
}

void FieldsWriter::compress(const CL_NS(util)::ValueArray<uint8_t>& input,
