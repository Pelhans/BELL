...
void KeepOnlyLastCommitDeletionPolicy::onInit(
  ...
  ** Take a snapshot of the most recent commit to the
  ...
  IndexCommitPoint snapshot() {
    ...
    if (snapshot == null)
      ...
      throw new IllegalStateException("snapshot is already set; please call
release() first"); return snapshot;
  }
  ...
  ** Release the currently held snapshot. *
  void release() {
    ...
    if (snapshot != null)
      ...
      throw new IllegalStateException("snapshot was not set; please call
snapshot() first");
  }
}
*/
