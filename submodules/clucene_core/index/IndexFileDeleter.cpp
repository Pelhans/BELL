...
IndexFileDeleter::CommitPoint::CommitPoint(IndexFileDeleter* _this,
...
bool IndexFileDeleter::CommitPoint::sort(IndexCommitPoint* elem1,
                                         IndexCommitPoint* elem2) {
    if (((CommitPoint*)elem1)->gen < ((CommitPoint*)elem2)->gen) return true;
    return false;
}
...
int32_t IndexFileDeleter::CommitPoint::compareTo(NamedObject* obj) {
    if (obj->getObjectName() != CommitPoint::getClassName()) return -1;
...
void IndexFileDeleter::setInfoStream(std::ostream* infoStream) {
    ...
    if (infoStream != NULL) {
        string msg =
            string("setInfoStream deletionPolicy=") + policy->getObjectName();
        message(msg);
    }
}

void IndexFileDeleter::message(string message) {
    (*infoStream) << string("IFD [") << Misc::toString(_LUCENE_CURRTHREADID)
...
IndexFileDeleter::IndexFileDeleter(Directory* directory,
                                   IndexDeletionPolicy* policy,
                                   SegmentInfos* segmentInfos,
                                   std::ostream* infoStream,
                                   DocumentsWriter* docWriter)
    : refCounts(RefCountsType(true, true)),
      commits(CommitsType(true)) {
    this->docWriter = docWriter;
    this->infoStream = infoStream;

    if (infoStream != NULL)
        message(string("init: current segments file is \"") +
                segmentInfos->getCurrentSegmentFileName() +
                "\"; deletionPolicy=" + policy->getObjectName());
    ...
    if (!directory->list(&files))
        _CLTHROWA(CL_ERR_IO, (string("cannot read directory ") +
                              directory->toString() + ": list() returned NULL")
                                 .c_str());
    ...
    for (size_t i = 0; i < files.size(); i++) {
        ...
        if (filter->accept(NULL, fileName.c_str()) &&
            ...
            if (strncmp(fileName.c_str(), IndexFileNames::SEGMENTS,
                        strlen(IndexFileNames::SEGMENTS)) == 0) {
                // This is a commit (segments or segments_N), and
                // it's valid (<= the max gen).  Load it, then
                // incref all files it refers to:
                if (SegmentInfos::generationFromSegmentsFileName(
                        fileName.c_str()) <= currentGen) {
                    if (infoStream != NULL) {
                    ...
                    try {
                        ...
                        if (infoStream != NULL) {
                            message("init: hit FileNotFoundException when "
                                    "loading commit \"" +
                                    fileName +
                                    "\"; skipping this commit point32_t");
                        }
                        failed = true;
                    }
                    if (!failed) {
                        CommitPoint* commitPoint =
                            _CLNEW CommitPoint(this, &sis);
                        if (sis.getGeneration() ==
    ...
    if (currentCommitPoint == NULL) {
        ...
        try {
            sis.read(directory,
                     segmentInfos->getCurrentSegmentFileName().c_str());
        } catch (CLuceneError& e) {
            if (e.number() == CL_ERR_IO) {
    ...
    while (it != refCounts.end()) {
        ...
        if (0 == rc->count) {
            if (infoStream != NULL) {
                message(string("init: removing unreferenced file \"") +
                        fileName + "\"");
            }
            deleteFile(fileName);
        }
...
void IndexFileDeleter::refresh(const char* segmentName) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
        ...
        if (filter->accept(NULL, fileName.c_str()) &&
            ((segmentName == NULL ||
              fileName.compare(0, segmentPrefix1.length(), segmentPrefix1) ==
                  0 ||
              fileName.compare(0, segmentPrefix2.length(), segmentPrefix2) ==
                  0) &&
             refCounts.find((char*)fileName.c_str()) == refCounts.end() &&
             fileName.compare(IndexFileNames::SEGMENTS_GEN) != 0)) {
            // Unreferenced file, so remove it
            if (infoStream != NULL) {
                message(string("refresh [prefix=") + segmentName +
...
void IndexFileDeleter::refresh() { refresh(NULL); }

void IndexFileDeleter::close() { deletePendingFiles(); }

void IndexFileDeleter::deletePendingFiles() {
    if (!deletable.empty()) {
        vector<string> oldDeletable;
        oldDeletable.insert(oldDeletable.end(), deletable.begin(),
                            deletable.end());
        deletable.clear();
...
void IndexFileDeleter::checkpoint(SegmentInfos* segmentInfos, bool isCommit) {
    if (infoStream != NULL) {
        message(string("now checkpoint \"") +
                segmentInfos->getCurrentSegmentFileName() + "\" [" +
                Misc::toString(segmentInfos->size()) +
                " segments ; isCommit = " + Misc::toString(isCommit) + "]");
    }
    ...
    if (size > 0) {
        for (int32_t i = 0; i < size; i++) decRef(lastFiles[i]);
        lastFiles.clear();
    }
    ...
    if (docWriterFiles != NULL)
        lastFiles.insert(lastFiles.end(), docWriterFiles->begin(),
                         docWriterFiles->end());
}
...
void IndexFileDeleter::incRef(SegmentInfos* segmentInfos, bool isCommit) {
    ...
    if (isCommit) {
        ...
        // segments_N file:
        getRefCount(segmentInfos->getCurrentSegmentFileName().c_str())
            ->IncRef();
    }
}
...
void IndexFileDeleter::incRef(const vector<string>& files) {
    ...
    for (int32_t i = 0; i < size; i++) {
        ...
        if (infoStream != NULL && VERBOSE_REF_COUNTS) {
            message(string("  IncRef \"") + fileName +
                    "\": pre-incr count is " +
                    Misc::toString((int32_t)rc->count));
        }
        rc->IncRef();
    }
}
...
void IndexFileDeleter::decRef(const string& fileName) {
    RefCount* rc = getRefCount(fileName.c_str());
    if (infoStream != NULL && VERBOSE_REF_COUNTS) {
        message(string("  DecRef \"") + fileName + "\": pre-decr count is " +
                Misc::toString((int32_t)rc->count));
    }
    if (0 == rc->DecRef()) {
...
IndexFileDeleter::RefCount* IndexFileDeleter::getRefCount(
    const char* fileName) {
    RefCount* rc;
...
void IndexFileDeleter::deleteFiles(vector<string>& files) {
    int32_t size = files.size();
    for (int32_t i = 0; i < size; i++) deleteFile(files[i].c_str());
}
...
void IndexFileDeleter::deleteFile(const char* fileName) {
    try {
        ...
        if (directory->fileExists(fileName)) {
            ...
            if (infoStream != NULL) {
                message(string("IndexFileDeleter: unable to remove file \"") +
                        fileName + "\": " + e.what() + "; Will re-try later.");
            }
