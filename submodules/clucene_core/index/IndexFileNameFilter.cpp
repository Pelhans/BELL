...
IndexFileNameFilter* IndexFileNameFilter::singleton =
    _CLNEW IndexFileNameFilter();

void IndexFileNameFilter::_shutdown() { _CLDELETE(singleton); }
...
IndexFileNameFilter::IndexFileNameFilter() {
    ...
    for (i = 0; i < IndexFileNames::INDEX_EXTENSIONS_IN_COMPOUND_FILE.length;
         ++i) {
        extensionsInCFS.insert(
            IndexFileNames::INDEX_EXTENSIONS_IN_COMPOUND_FILE[i]);
    }
}
IndexFileNameFilter::~IndexFileNameFilter() {}
bool IndexFileNameFilter::accept(const char* /*dir*/, const char* name) const {
    ...
    if (i != string::npos) {
        ...
        if (*extension == 'f' && strtol(extension + 1, &tmp, 10) >= 0 &&
            tmp == (extension + l)) { // check for f001
            return true;
        } else if (*extension == 's' && strtol(extension + 1, &tmp, 10) >= 0 &&
                   tmp == (extension + l)) {
            return true;
        }
    } else {
        if (strcmp(name, IndexFileNames::DELETABLE) == 0)
            return true;
        else if (strncmp(name, IndexFileNames::SEGMENTS,
