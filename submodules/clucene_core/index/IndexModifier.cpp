...
IndexModifier::IndexModifier(const char* dirName, Analyzer* analyzer,
                             bool create) {
...
void IndexModifier::setUseCompoundFile(bool useCompoundFile) {
    SCOPED_LOCK_MUTEX(directory->THIS_LOCK)
    assureOpen();
    if (indexWriter != NULL) indexWriter->setUseCompoundFile(useCompoundFile);
...
void IndexModifier::setMaxFieldLength(int32_t maxFieldLength) {
    SCOPED_LOCK_MUTEX(directory->THIS_LOCK)
    assureOpen();
    if (indexWriter != NULL) indexWriter->setMaxFieldLength(maxFieldLength);
...
void IndexModifier::setMaxBufferedDocs(int32_t maxBufferedDocs) {
    SCOPED_LOCK_MUTEX(directory->THIS_LOCK)
    assureOpen();
    if (indexWriter != NULL) indexWriter->setMaxBufferedDocs(maxBufferedDocs);
...
void IndexModifier::setMergeFactor(int32_t mergeFactor) {
    SCOPED_LOCK_MUTEX(directory->THIS_LOCK)
    assureOpen();
    if (indexWriter != NULL) indexWriter->setMergeFactor(mergeFactor);
...
void IndexModifier::close() {
    if (!open) _CLTHROWA(CL_ERR_IllegalState, "Index is closed already");
    SCOPED_LOCK_MUTEX(directory->THIS_LOCK)
    if (indexWriter != NULL) {
        indexWriter->close();
        _CLDELETE(indexWriter);
...
CL_NS(store)::Directory* IndexModifier::getDirectory() { return directory; }
