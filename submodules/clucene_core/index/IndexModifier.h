 ...
 *
 * <p>Note that you cannot create more than one <code>IndexModifier</code>
 ...
 *     IndexModifier* indexModifier = <b>new</b> IndexModifier("/tmp/index",
 * analyzer, <b>true</b>);<br/> Document* doc = <b>new </b>Document*();<br/>
 *     doc->add(*<b>new </b>Field("id", "1", Field::STORE_YES|
 * Field::INDEX_UNTOKENIZED));<br/> doc->add(*<b>new </b>Field("body", "a simple
 * test", Field::STORE_YES, Field::INDEX_TOKENIZED));<br/>
 *     indexModifier->addDocument(doc);<br/>
 *     <b>int32_t </b>deleted = indexModifier->deleteDocuments(<b>new
 * </b>Term("id", "1"));<br/> printf("Deleted %d document", deleted);<br/>
...
public:
     ...
     * @param create <code>true</code> to create the index or overwrite the
     * existing one; <code>false</code> to append to the existing index
     */
     ...
     * existing one; <code>false</code> to append to the existing index
     */
    IndexModifier(const char* dirName, CL_NS(analysis)::Analyzer* analyzer,
                  bool create);
...
public:
    ...
    void addDocument(CL_NS(document)::Document* doc,
                     CL_NS(analysis)::Analyzer* docAnalyzer = NULL);

    /**
     * Deletes all documents containing <code>term</code>.
     * This is useful if one uses a document field to hold a unique ID string
     * for the document.  Then to delete such a document, one merely constructs
     * a term with the appropriate field and the unique ID string as its text
     * and passes it to this method.  Returns the number of documents deleted.
     ...
     * process by running out of memory.<p/> Note that this effectively
     * truncates large documents, excluding from the index terms that occur
     * further in the document.  If you know your source documents are large, be
     * sure to set this value high enough to accomodate the expected size.  If
     * you set it to Integer.MAX_VALUE, then the only limit is your memory, but
     * you should anticipate an OutOfMemoryError.<p/> By default, no more than
     * 10,000 terms will be indexed for a field.
     ...
     * for indices that are interactively maintained. <p>This must never be less
     * than 2.  The default value is 10.
     *
