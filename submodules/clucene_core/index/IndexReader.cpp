...
class IndexReaderFindSegmentsFile
    : public SegmentInfos::FindSegmentsFile<uint64_t> {
public:
...
public:
    bool operator()(IndexReader::CloseCallback t1,
                    IndexReader::CloseCallback t2) const {
        return t1 > t2;
    }
...
public:
    ...
    typedef CL_NS(
        util)::CLSet<IndexReader::CloseCallback, void*, CloseCallbackCompare,
...
IndexReader::~IndexReader() {
    ...
    // Pre  - true
    // Post - The instance has been destroyed if pre(writeLock) exists is has
    // been released
    _CLLDELETE(_internal);
}

IndexReader* IndexReader::open(const char* path, bool closeDirectoryOnCleanup,
                               IndexDeletionPolicy* deletionPolicy) {
    // Func - Static method.
    //       Returns an IndexReader reading the index in an FSDirectory in the
    //       named path.
    // Pre  - path != NULL and contains the path of the index for which an
    // IndexReader must be
    //       instantiated
    //       closeDir indicates if the directory needs to be closed
    // Post - An IndexReader has been returned that reads tnhe index located at
    // path
...
IndexReader* IndexReader::open(Directory* directory, bool closeDirectory,
    ...
    //       closeDir indicates if the directory needs to be closed
    // Post - An IndexReader has been returned that reads the index located at
    // directory
...
CL_NS(document)::Document* IndexReader::document(const int32_t n) {
    ...
    if (!document(n, *ret)) _CLDELETE(ret);
    return ret;
}
...
uint64_t IndexReader::lastModified(const char* directory2) {
    ...
    //       modified.
    // Pre  - directory != NULL and contains the path name of the directory to
    // check Post - The last modified time of the index has been returned
...
TermDocs* IndexReader::termDocs(Term* term) {
    ...
    //       scoring. Thus, this method implements the mapping:
    //
    //       Term => <docNum, freq>*
    //	   The enumeration is ordered by document number.  Each document number
    //       is greater than all that precede it in the enumeration.
    // Pre  - term != NULL
    // Post - A reference to TermDocs containing an enumeration of all found
    // documents
    //       has been returned
...
TermPositions* IndexReader::termPositions(Term* term) {
    ...
    //       term in that document, a list of all of the ordinal positions of
    //       the term in the document is available.  Thus, this method
    //       implements the mapping:
    //
    //       Term => <docNum, freq,<pos 1, pos 2, ...pos freq-1>>*
    //
    //       This positional information faciliates phrase and proximity
    //       searching. The enumeration is ordered by document number.  Each
    //       document number is greater than all that precede it in the
    //       enumeration.
    // Pre  - term != NULL
    // Post - A reference to TermPositions containing an enumeration of all
    // found documents
    //       has been returned
...
void IndexReader::deleteDoc(const int32_t docNum) { deleteDocument(docNum); }
int32_t IndexReader::deleteTerm(Term* term) { return deleteDocuments(term); }

void IndexReader::deleteDocument(const int32_t docNum) {
    // Func - Deletes the document numbered docNum.  Once a document is deleted
    // it will not appear
    //       in TermDocs or TermPostitions enumerations. Attempts to read its
    //       field with the document method will result in an error.  The
    //       presence of this document may still be reflected in the docFreq
    //       statistic, though this will be corrected eventually as the index is
    //       further modified.
    // Pre  - docNum >= 0
    // Post - If successful the document identified by docNum has been deleted.
    // If no writelock
    //       could be obtained an exception has been thrown stating that the
    //       index was locked or has no write access
...
int32_t IndexReader::deleteDocuments(Term* term) {
    // Func - Deletes all documents containing term. This is useful if one uses
    // a
    //       document field to hold a unique ID string for the document.  Then
    //       to delete such a document, one merely constructs a term with the
    //       appropriate field and the unique ID string as its text and passes
    //       it to this method.
    // Pre  - term != NULL
    // Post - All documents containing term have been deleted. The number of
    // deleted documents
    //       has been returned
...
void IndexReader::close() {
    // Func - Closes files associated with this index and also saves any new
    // deletions to disk.
    //       No other methods should be called after this has been called.
    // Pre  - true
    // Post - All files associated with this index have been deleted and new
    // deletions have been
    //       saved to disk
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    if (!closed) {
        Internal::CloseCallbackMap::iterator iter =
            _internal->closeCallbacks.begin();
...
bool IndexReader::isLocked(Directory* directory) {
    ...
    // Post - Returns true if the index in the named directory is locked
    // otherwise false

    // Check the existence of the file write.lock and return true when it does
...
bool IndexReader::isLuceneFile(const char* filename) {
    ...
    if (strcmp(ext, ".cfs") == 0)
        ...
        const char* n = ext + 2;
        if (*n && _istdigit(*n)) return true;
    }

    return false;
}

CL_NS(store)::Directory* IndexReader::getDirectory() { return directory(); }
