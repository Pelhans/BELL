...
#include "../store/_Lock.h"
#include "../store/_RAMDirectory.h"
...
const int32_t IndexWriter::DEFAULT_MAX_BUFFERED_DELETE_TERMS =
    DISABLE_AUTO_FLUSH;
const int32_t IndexWriter::DEFAULT_MAX_MERGE_DOCS =
    LogDocMergePolicy::DEFAULT_MAX_MERGE_DOCS;
const int32_t IndexWriter::DEFAULT_MERGE_FACTOR =
...
public:
    IndexWriter* _this;
    Internal(IndexWriter* _this) { this->_this = _this; }
    // Apply buffered delete terms to the segment just flushed from ram
    // apply appropriately so that a delete term is only applied to
    // the documents buffered before it, not those buffered after it.
    void applyDeletesSelectively(
        const DocumentsWriter::TermNumMapType& deleteTerms,
        const std::vector<int32_t>& deleteIds, IndexReader* reader);

    // Apply buffered delete terms to this reader.
    void applyDeletes(const DocumentsWriter::TermNumMapType& deleteTerms,
                      IndexReader* reader);
};
...
void IndexWriter::message(string message) {
    if (infoStream != NULL) {
        (*infoStream) << string("IW ") << Misc::toString(messageID)
                      << string(" [") << Misc::toString(_LUCENE_CURRTHREADID)
...
LogMergePolicy* IndexWriter::getLogMergePolicy() const {
    if (mergePolicy->instanceOf(LogMergePolicy::getClassName()))
        ...
        _CLTHROWA(CL_ERR_IllegalArgument,
                  "this method can only be called when the merge policy is the "
                  "default LogMergePolicy");
}
...
IndexWriter::IndexWriter(Directory* d, bool autoCommit, Analyzer* a,
                         IndexDeletionPolicy* deletionPolicy,
                         bool closeDirOnShutdown)
    : bOwnsDirectory(false) {
...
void IndexWriter::init(Directory* d, Analyzer* a, bool closeDir,
                       IndexDeletionPolicy* deletionPolicy, bool autoCommit) {
...
void IndexWriter::init(Directory* d, Analyzer* a, const bool create,
                       const bool closeDir, IndexDeletionPolicy* deletionPolicy,
                       const bool autoCommit) {
    this->_internal = new Internal(this);
    this->termIndexInterval = IndexWriter::DEFAULT_TERM_INDEX_INTERVAL;
    this->mergeScheduler = _CLNEW
    SerialMergeScheduler(); // TODO: implement and use ConcurrentMergeScheduler
    ...
    if (!writeLock->obtain(writeLockTimeout)) // obtain write lock
        _CLTHROWA(CL_ERR_LockObtainFailed, (string("Index locked for write: ") +
                                            writeLock->getObjectName())
                                               .c_str());
    ...
    try {
        ...
        deleter = _CLNEW IndexFileDeleter(
            directory,
            deletionPolicy == NULL ? _CLNEW KeepOnlyLastCommitDeletionPolicy()
                                   : deletionPolicy,
...
void IndexWriter::setMaxBufferedDocs(int32_t maxBufferedDocs) {
    ...
    if (maxBufferedDocs == DISABLE_AUTO_FLUSH &&
        ...
        _CLTHROWA(CL_ERR_IllegalArgument, "at least one of ramBufferSize and "
    ...
    if (docWriter->getMaxBufferedDocs() != DISABLE_AUTO_FLUSH) {
        ...
        if (mp->instanceOf(LogDocMergePolicy::getClassName())) {
            ...
            if (lmp->getMinMergeDocs() != maxBufferedDocs) {
                if (infoStream != NULL) {
                    message(string("now push maxBufferedDocs ") +
                            Misc::toString(maxBufferedDocs) +
...
void IndexWriter::setMaxBufferedDeleteTerms(int32_t maxBufferedDeleteTerms) {
    ...
    if (infoStream != NULL)
        message("setMaxBufferedDeleteTerms " +
                Misc::toString(maxBufferedDeleteTerms));
}
...
void IndexWriter::setInfoStream(std::ostream* infoStream) {
    ...
    if (infoStream != NULL) messageState();
}

void IndexWriter::messageState() {
    message(
        string("setInfoStream: dir=") + directory->toString() + " autoCommit=" +
        (autoCommit ? "true"
                    : "false" + string(" mergePolicy=") +
                          mergePolicy->getObjectName() +
                          " mergeScheduler=" + mergeScheduler->getObjectName() +
                          " ramBufferSizeMB=" +
                          Misc::toString(docWriter->getRAMBufferSizeMB()) +
                          " maxBuffereDocs=" +
                          Misc::toString(docWriter->getMaxBufferedDocs())) +
        " maxBuffereDeleteTerms=" +
        Misc::toString(docWriter->getMaxBufferedDeleteTerms()) +
...
void IndexWriter::closeInternal(bool waitForMerges) {
    try {
        if (infoStream != NULL) message(string("now flush at close"));
...

        {
            ...
            if (commitPending) {
                ...
                _CLFINALLY(if (!success) {
                    if (infoStream != NULL)
                        message(string("hit exception committing segments file "
                                       "during close"));
...
bool IndexWriter::flushDocStores() {
    ...
    if (files.size() > 0) {
        ...
        if (useCompoundDocStore && !docStoreSegment.empty()) {
            ...
            const string compoundFileName =
                docStoreSegment + "." +
                IndexFileNames::COMPOUND_FILE_STORE_EXTENSION;

            try {
                CompoundFileWriter cfsWriter(directory,
                                             compoundFileName.c_str());
            ...
            _CLFINALLY(if (!success) {
                if (infoStream != NULL)
                    message("hit exception building compound file doc store "
                            "for segment " +
                            docStoreSegment);
...
void IndexWriter::optimize(bool doWait) { optimize(1, doWait); }
                ...
                if (mergeExceptions->size() > 0) {
                    ...
                    for (int32_t i = 0; i < size; i++) {
                        ...
                        if (_merge->optimize) {
                            ...
                            CLuceneError err(
                                tmp.number(),
                                (string("background merge hit exception: ") +
                                 _merge->segString(directory) + ":" +
                                 tmp.what())
                                    .c_str(),
                                false);
                            throw err;
...
void IndexWriter::maybeMerge() { maybeMerge(false); }

void IndexWriter::maybeMerge(bool optimize) { maybeMerge(1, optimize); }
...
void IndexWriter::updatePendingMerges(int32_t maxNumSegmentsOptimize,
                                      bool optimize) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    assert(!optimize || maxNumSegmentsOptimize > 0);

    if (stopMerges) return;

    MergePolicy::MergeSpecification* spec;
    if (optimize) {
        spec = mergePolicy->findMergesForOptimize(
            segmentInfos, this, maxNumSegmentsOptimize, *segmentsToOptimize);
...
void IndexWriter::startTransaction() {
    ...
    if (infoStream != NULL) message(string("now start transaction"));

    CND_PRECONDITION(
        docWriter->getNumBufferedDeleteTerms() == 0,
        "calling startTransaction with buffered delete terms not supported");
    CND_PRECONDITION(
        docWriter->getNumDocsInRAM() == 0,
    ...
    if (localAutoCommit) {
        if (infoStream != NULL) message(string("flush at startTransaction"));
...
void IndexWriter::rollbackTransaction() {
    ...
    if (infoStream != NULL) message(string("now rollback transaction"));
...
void IndexWriter::commitTransaction() {
    ...
    if (infoStream != NULL) message(string("now commit transaction"));
...
void IndexWriter::abort() {
    ensureOpen();
    if (autoCommit)
        _CLTHROWA(CL_ERR_IllegalState,
                  "abort() can only be called when IndexWriter was opened with "
                  "autoCommit=false");
...
void IndexWriter::finishMerges(bool waitForMerges) {
    ...
    if (!waitForMerges) {
        ...
        while (runningMerges->size() > 0) {
            if (infoStream != NULL)
                message(string("now wait for ") +
                        Misc::toString((int32_t)runningMerges->size()) +
...
void IndexWriter::addIndexes(
    CL_NS(util)::ArrayBase<CL_NS(store)::Directory*>& dirs) {
    ensureOpen();
    ...
    try {
        if (infoStream != NULL) message(string("flush at addIndexes"));
        flush();
        ...
        _CLFINALLY(
            if (success) { commitTransaction(); } else {
                rollbackTransaction();
            })
    } catch (std::bad_alloc&) {
        hitOOM = true;
    ...
    _CLFINALLY(docWriter->resumeAllThreads();)
}
...
void IndexWriter::addIndexesNoOptimize(
    CL_NS(util)::ArrayBase<CL_NS(store)::Directory*>& dirs) {
    ensureOpen();
    ...
    while (true) {
        ...
        MergePolicy::OneMerge* _merge = NULL;
        {
            ...
            for (int32_t i = 0; i < numSegments; i++) {
                ...
                if (info->dir != directory) {
                    ...
                    _merge = _CLNEW MergePolicy::OneMerge(
                        range, info->getUseCompoundFile());
                    break;
                }
            }
        }
        ...
        if (_merge != NULL) {
            if (registerMerge(_merge)) {
                PendingMergesType::iterator p = std::find(
                    pendingMerges->begin(), pendingMerges->end(), _merge);
                ...
                _CLTHROWA(
                    CL_ERR_Merge,
                    (string("segment \"") + info->name +
                     " exists in external directory yet the MergeScheduler "
                     "executed the merge in a separate thread")
                        .c_str());
        } else
...
void IndexWriter::flush() { flush(true, false); }

void IndexWriter::flush(bool triggerMerge, bool _flushDocStores) {
    ensureOpen();

    if (doFlush(_flushDocStores) && triggerMerge) maybeMerge();
}
...
bool IndexWriter::doFlush(bool _flushDocStores) {
    ...
    try {
        ...
        if (docStoreSegment.empty()) _flushDocStores = false;
        ...
        if (infoStream != NULL) {
            message("  flush: segment=" + docWriter->getSegment() +
                    ...
                    " numDocs=" + Misc::toString(numDocs) + " numBufDelTerms=" +
        ...
        if (_flushDocStores &&
            (!flushDocs || !docWriter->getSegment().compare(
                               docWriter->getDocStoreSegment()) == 0)) {
        ...
        if (flushDocs || flushDeletes) {
            ...
            if (flushDeletes) rollback = segmentInfos->clone();
            ...
            try {
                if (flushDocs) {
                    ...
                    newSegment = _CLNEW SegmentInfo(
                        segment.c_str(), flushedDocCount, directory, false,
                        true, docStoreOffset, docStoreSegment.c_str(),
            ...
            _CLFINALLY(if (!success) {
                ...
                if (flushDocs) docWriter->abort(NULL);
                deletePartialSegmentsFile();
                deleter->checkpoint(segmentInfos, false);

                if (!segment.empty()) deleter->refresh(segment.c_str());
            } else if (flushDeletes) _CLDELETE(rollback);)

            deleter->checkpoint(segmentInfos, autoCommit);

            if (flushDocs &&
                mergePolicy->useCompoundFile(segmentInfos, newSegment)) {
                success = false;
                try {
                ...
                _CLFINALLY(if (!success) {
                    if (infoStream != NULL)
                        message("hit exception creating compound file for "
                                "newly flushed segment " +
                                segment);
    ...
    _CLFINALLY(docWriter->clearFlushPending(); docWriter->resumeAllThreads();)
    return ret;
}
...
int32_t IndexWriter::ensureContiguousMerge(MergePolicy::OneMerge* _merge) {
    ...
    if (first == -1)
        _CLTHROWA(CL_ERR_Merge,
                  (string("could not find segment ") +
                   _merge->segments->info(0)->name + " in current segments")
                      .c_str());
    ...
    for (int32_t i = 0; i < numSegmentsToMerge; i++) {
        ...
        if (first + i >= numSegments ||
            ...
            if (segmentInfos->indexOf(info) == -1)
                _CLTHROWA(CL_ERR_Merge,
                          (string("MergePolicy selected a segment (") +
                           info->name + ") that is not in the index")
                              .c_str());
            else
                _CLTHROWA(CL_ERR_Merge,
                          (string("MergePolicy selected non-contiguous "
                                  "segments to merge (") +
                           _merge->getObjectName() + " vs " + segString() +
...
bool IndexWriter::commitMerge(MergePolicy::OneMerge* _merge) {
    ...
    if (_merge->isAborted()) {
        if (infoStream != NULL)
            message("commitMerge: skipping merge " +
                    _merge->segString(directory) + ": it was aborted");
    ...
    try {
        ...
        for (int32_t i = 0; i < numSegmentsToMerge; i++) {
            ...
            if (previousInfo->hasDeletions()) {
                ...
                BitVector previousDeletes(
                    previousInfo->dir, previousInfo->getDelFileName().c_str());

                if (!currentInfo->getDelFileName().compare(
                        previousInfo->getDelFileName()) == 0) {
                    ...
                    BitVector currentDeletes(
                        currentInfo->dir,
                        currentInfo->getDelFileName().c_str());
                    for (int32_t j = 0; j < docCount; j++) {
                        if (previousDeletes.get(j))
                            assert(currentDeletes.get(j));
                        else {
                            if (currentDeletes.get(j)) deletes->set(docUpto);
                            docUpto++;
                        }
                    }
                } else
                ...
                if (deletes == NULL)
                    deletes = _CLNEW BitVector(_merge->info->docCount);
                BitVector currentDeletes(directory,
                                         currentInfo->getDelFileName().c_str());
    ...
    if (!mergeDocStoreSegment.empty() &&
        !_merge->info->getDocStoreIsCompoundFile()) {
    ...
    _CLFINALLY(if (!success && rollback != NULL) {
        if (infoStream != NULL)
    ...
    if (_merge->optimize) segmentsToOptimize->push_back(_merge->info);
        ...
        _CLFINALLY({
            ...
            try {
                ...
                if (success && !_merge->isAborted() && !closed && !closing)
                    updatePendingMerges(_merge->maxNumSegmentsOptimize,
                                        _merge->optimize);
            }
            _CLFINALLY(
...
bool IndexWriter::registerMerge(MergePolicy::OneMerge* _merge) {
    ...
    if (_merge->registerDone) return true;

    const int32_t count = _merge->segments->size();
    bool isExternal = false;
    for (int32_t i = 0; i < count; i++) {
        SegmentInfo* info = _merge->segments->info(i);
        if (mergingSegments->find(info) != mergingSegments->end()) return false;
        if (segmentInfos->indexOf(info) == -1) return false;
        if (info->dir != directory) isExternal = true;
    }

    pendingMerges->push_back(_merge);

    if (infoStream != NULL)
        message(string("add merge to pendingMerges: ") +
                _merge->segString(directory) + " [total " +
                Misc::toString((int32_t)pendingMerges->size()) + " pending]");
    ...
    // threads, start
    for (int32_t i = 0; i < count; i++)
        mergingSegments->insert(mergingSegments->end(),
                                _merge->segments->info(i));
...
void IndexWriter::_mergeInit(MergePolicy::OneMerge* _merge) {
    ...
    for (int32_t i = 0; i < end; i++) {
        ...
        if (si->getDocStoreOffset() != -1 && !currentDocStoreSegment.empty() &&
            si->getDocStoreSegment().compare(currentDocStoreSegment) == 0)
    ...
    if (mergeDocStores && doFlushDocStore) {
        ...
        if (infoStream != NULL) message(string("flush at merge"));
        flush(false, true);
    }
    ...
    for (int32_t i = 0; i < end; i++) {
        ...
        if (si->dir == directory) deleter->incRef(si->files());
    }
    ...
    // names.
    _merge->info = _CLNEW SegmentInfo(
        newSegmentName().c_str(), 0, directory, false, true, docStoreOffset,
        docStoreSegment.c_str(), docStoreIsCompoundFile);
...
void IndexWriter::mergeFinish(MergePolicy::OneMerge* _merge) {
    ...
    if (_merge->increfDone) decrefMergeSegments(_merge);
    ...
    for (int32_t i = 0; i < end; i++) { // todo: use iterator
        MergingSegmentsType::iterator itr =
            mergingSegments->find(sourceSegments->info(i));
...
int32_t IndexWriter::mergeMiddle(MergePolicy::OneMerge* _merge) {
    ...
    if (infoStream != NULL) message("merging " + _merge->segString(directory));
    ...
    try {
        ...
        for (int32_t i = 0; i < numSegments; i++) {
            ...
            IndexReader* reader = SegmentReader::get(
                si, MERGE_READ_BUFFER_SIZE,
                _merge->mergeDocStores); // no need to set deleter (yet)
            merger.add(reader);
            totDocCount += reader->numDocs();
        }
        if (infoStream != NULL) {
            message(string("merge: total ") + Misc::toString(totDocCount) +
                    " docs");
        }

        _merge->checkAborted(directory);

        mergedDocCount = _merge->info->docCount =
            merger.merge(_merge->mergeDocStores);
    ...
    _CLFINALLY(
        ...
        merger.closeReaders(); if (!success) {
            if (infoStream != NULL)
                message("hit exception during merge; now refresh deleter on "
                        "segment " +
                        mergedName);
            {
                SCOPED_LOCK_MUTEX(this->THIS_LOCK)
    ...
    if (_merge->useCompoundFile) {
                    ...
                    if (segmentInfos->indexOf(_merge->info) == -1) {
                        ...
                        if (infoStream != NULL)
                            message("hit exception creating compound file; "
                                    "ignoring it because our info (segment " +
                                    _merge->info->name +
        ...
        _CLFINALLY(if (!success) {
            if (infoStream != NULL)
                message(string("hit exception creating compound file during "
                               "merge: skip=") +
                        Misc::toString(skip));

            {
                SCOPED_LOCK_MUTEX(this->THIS_LOCK)
                if (!skip) addMergeException(_merge);
                deleter->deleteFile(compoundFileName.c_str());
            }
        })
        ...
        if (!skip) {
            {
                ...
                if (skip || segmentInfos->indexOf(_merge->info) == -1 ||
                    ...
                    _CLFINALLY(if (!success) {
                        if (infoStream != NULL)
                            message(string("hit exception checkpointing "
                                           "compound file during merge"));
...
void IndexWriter::applyDeletes(bool flushedNewSegment) {
    const DocumentsWriter::TermNumMapType& bufferedDeleteTerms =
        docWriter->getBufferedDeleteTerms();
    const vector<int32_t>* bufferedDeleteDocIDs =
        docWriter->getBufferedDeleteDocIDs();

    if (infoStream != NULL)
        message(string("flush ") +
                Misc::toString(docWriter->getNumBufferedDeleteTerms()) +
                " buffered deleted terms and " +
                Misc::toString((int32_t)bufferedDeleteDocIDs->size()) +
                " deleted docIDs on " +
                Misc::toString((int32_t)segmentInfos->size()) + " segments.");
    ...
    if (flushedNewSegment) {
        ...
        try {
            ...
            reader = SegmentReader::get(
                segmentInfos->info(segmentInfos->size() - 1), false);

            // Apply delete terms to the segment just flushed from ram
            // apply appropriately so that a delete term is only applied to
            // the documents buffered before it, not those buffered after it.
            _internal->applyDeletesSelectively(bufferedDeleteTerms,
                                               *bufferedDeleteDocIDs, reader);
        }
        _CLFINALLY(if (reader != NULL) {
            try {
                reader->doCommit();
            }
            _CLFINALLY(reader->doClose(); _CLLDELETE(reader);)
        })
    }
...
void IndexWriter::Internal::applyDeletesSelectively(
    const DocumentsWriter::TermNumMapType& deleteTerms,
    ...
    while (iter != deleteTerms.end()) {
        ...
        if (docs != NULL) {
            ...
            _CLFINALLY(docs->close(); _CLDELETE(docs);)
        }
...
void IndexWriter::Internal::applyDeletes(
    const DocumentsWriter::TermNumMapType& deleteTerms, IndexReader* reader) {
    DocumentsWriter::TermNumMapType::const_iterator iter = deleteTerms.begin();
    while (iter != deleteTerms.end()) {
...
bool IndexWriter::testPoint(const char* name) { return true; }
