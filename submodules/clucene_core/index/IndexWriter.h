...
/**
  ...
  <a href="#IndexWriter(org.apache.lucene.store.Directory,
  ...
  also <a href="#IndexWriter(org.apache.lucene.store.Directory,
  ...
  already an index at the provided path and otherwise open the existing
  index.</p>
  ...
  org.apache.lucene.document.Document)"><b>updateDocument</b></a> (which just
  deletes and then adds the entire document). When finished adding, deleting and
  updating documents, <a href="#close()"><b>close</b></a> should be called.</p>
  ...
  <a href="#IndexWriter(org.apache.lucene.store.Directory, boolean,
  ...
  index. When this is <code>false</code>, changes are not visible until {@link
  #close()} is called. Note that changes will still be flushed to the
  ...
  You can also call {@link #abort()}, which closes the writer without committing
  any changes, and removes any index files that had been flushed but are now
  unreferenced. This mode is useful for preventing readers from refreshing at a
  bad time (for example after you've done all your deletes but before you've
  done your adds). It can also be used to implement simple single-writer
  ...
  IndexReader} or {@link org.apache.lucene.search.IndexSearcher} will only see
  the index as of the "point in time" that it was opened.  Any changes committed
  to the index after the reader was opened are not visible until the reader is
  re-opened.</p>

  <p>If an index will not have more documents added for a while and optimal
  search performance is desired, then the <a
  href="#optimize()"><b>optimize</b></a> method should be called before the
  index is closed.</p>
...
class CLUCENE_EXPORT IndexWriter : LUCENE_BASE {
    bool isOpen; // indicates if the writers is open - this way close can be
    ...
    // merges
    typedef CL_NS(util)::CLHashSet<
        SegmentInfo *,
        CL_NS(util)::Compare::Void<SegmentInfo>> MergingSegmentsType;
    MergingSegmentsType *mergingSegments;
    MergePolicy *mergePolicy;
    MergeScheduler *mergeScheduler;

    typedef CL_NS(util)::CLLinkedList<
    ...
    typedef CL_NS(
        util)::CLArrayList<MergePolicy::OneMerge *> MergeExceptionsType;
    ...
    SegmentInfos *rollbackSegmentInfos; // segmentInfos we will fallback to if
                                        // the commit fails

    SegmentInfos *localRollbackSegmentInfos; // segmentInfos we will fallback to
                                             // if the commit fails
    ...
    SegmentsToOptimizeType *segmentsToOptimize; // used by optimize to note
                                                // those needing optimization

    CL_NS(store)::LuceneLock *writeLock;

    void init(CL_NS(store)::Directory *d, CL_NS(analysis)::Analyzer *a,
              bool closeDir, IndexDeletionPolicy *deletionPolicy,
              bool autoCommit);
    void init(CL_NS(store)::Directory *d, CL_NS(analysis)::Analyzer *a,
              bool create, bool closeDir, IndexDeletionPolicy *deletionPolicy,
              bool autoCommit);
...
public:
     ...
     * process by running out of memory.  This setting refers to the number of
     * running terms, not to the number of different terms.<p/>
     * <strong>Note:</strong> see {@link DEFAULT_MAX_FIELD_LENGTH} for an
     * important note regarding field lengths.
     ...
     * larger values more RAM is used while indexing and searches on unoptimized
     * indices are slower, but indexing is faster.  Thus larger values (> 10)
     * are best for batched index creation, and smaller values (< 10) for
     ...
     * information may be processed.  In a large index with user-entered query
     * terms, query processing time is likely to be dominated not by term lookup
     * but rather by the processing of frequency and positional data.  In a
     * small index or when many uncommon query terms are generated (e.g., by
     * wildcard queries) term lookup may become a dominant cost.
     *
     ...
     *  IO error
     */
    explicit IndexWriter(const char *path, CL_NS(analysis)::Analyzer *a,
                         const bool create);
     ...
     *  IO error
     */
    explicit IndexWriter(CL_NS(store)::Directory *d,
                         CL_NS(analysis)::Analyzer *a, const bool create,
                         const bool closeDirOnShutdown = false);
     ...
     *  IO error
     */
    explicit IndexWriter(CL_NS(store)::Directory *d, const bool autoCommit,
     ...
     * setUseCompoundFile(boolean) or the default. You cannot use this to query
     * the status of an existing index.
    ...
    /**
     * <p>Determines the minimal number of delete terms required before the
     buffered
     ...
     *
     * <p>It is recommended that this method be called upon completion of
     * indexing.  In environments with frequent updates, optimize is best done
     * during low volume times, if at all.
     *
     * </p>
     * <p>See http://www.gossamer-threads.com/lists/lucene/java-dev/47895 for
     * more discussion. </p>
     *
     ...
     * are discarded (depending on the policy, see #FIELD_TRUNC_POLICY__WARN)
     *
    ...
    // addIndexes(CL_NS(util)::ArrayBase<IndexReader*>& readers);

    /** Merges all segments from an array of indexes into this index.
     *
     * <p>This may be used to parallelize batch indexing.  A large document
     * collection can be broken into sub-collections.  Each sub-collection can
     * be indexed in parallel, on a different thread, process or machine.  The
