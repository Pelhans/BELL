...
#define MESSAGE(msg) \
    if (writer != NULL && writer->getInfoStream() != NULL) message(msg)

const int32_t LogMergePolicy::DEFAULT_MAX_MERGE_DOCS =
...
MergePolicy::OneMerge::OneMerge(SegmentInfos* segments, bool _useCompoundFile)
    ...
    aborted = mergeDocStores = optimize = increfDone = registerDone =
        isExternal = false;
}
...
void MergePolicy::OneMerge::checkAborted(CL_NS(store)::Directory* dir) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    if (aborted)
        _CLTHROWA(CL_ERR_MergeAborted,
                  (string("merge is aborted: ") + segString(dir)).c_str());
}

std::string MergePolicy::OneMerge::segString(
    CL_NS(store)::Directory* dir) const {
    std::string b;
...
MergePolicy::MergeSpecification::~MergeSpecification() { _CLDELETE(merges); }
void MergePolicy::MergeSpecification::add(OneMerge* merge) {
    merges->push_back(merge);
}

std::string MergePolicy::MergeSpecification::segString(
...
int32_t LogMergePolicy::getMergeFactor() { return mergeFactor; }
bool LogMergePolicy::isOptimized(
    SegmentInfos* infos, IndexWriter* writer, int32_t maxNumSegments,
    std::vector<SegmentInfo*>& segmentsToOptimize) {
    const int32_t numSegments = infos->size();
    int32_t numToOptimize = 0;
    SegmentInfo* optimizeInfo = NULL;
    for (int32_t i = 0; i < numSegments && numToOptimize <= maxNumSegments;
         i++) {
...
bool LogMergePolicy::useCompoundFile(SegmentInfos* /*infos*/,
                                     SegmentInfo* /*info*/) {
...
bool LogMergePolicy::getUseCompoundFile() { return _useCompoundFile; }
...
bool LogMergePolicy::getUseCompoundDocStore() { return _useCompoundDocStore; }

void LogMergePolicy::close() {}

MergePolicy::MergeSpecification* LogMergePolicy::findMergesForOptimize(
    SegmentInfos* infos, IndexWriter* writer, int32_t maxNumSegments,
    vector<SegmentInfo*>& segmentsToOptimize) {
            ...
            if (0 == spec->merges->size()) {
                if (maxNumSegments == 1) {
                    ...
                    for (int32_t i = 0; i < last - finalMergeSize + 1; i++) {
                        ...
                        if (i == 0 || (sumSize < 2 * size(infos->info(i - 1)) &&
                                       sumSize < bestSize)) {
                            bestStart = i;
                            bestSize = sumSize;
                        }
...
MergePolicy::MergeSpecification* LogMergePolicy::findMerges(
    SegmentInfos* infos, IndexWriter* writer) {
    const int32_t numSegments = infos->size();
    this->writer = writer;
    ...
    while (start < numSegments) {
        ...
        MESSAGE(string("  level ") + Misc::toString(levelBottom) + " to " +
                Misc::toString(maxLevel) + ": " +
                Misc::toString(1 + upto - start) + " segments");
        ...
        while (end <= 1 + upto) {
            ...
            for (int32_t i = start; i < end; i++) {
                ...
                anyTooLarge |= (size(info) >= maxMergeSize ||
                                info->docCount >= maxMergeDocs);
            }

            if (!anyTooLarge) {
                if (spec == NULL) spec = _CLNEW MergeSpecification();
                MESSAGE(string("    ") + Misc::toString(start) + " to " +
                        Misc::toString(end) + ": add this merge");
                SegmentInfos* range = _CLNEW SegmentInfos;
                ...
                MESSAGE(string("    ") + Misc::toString(start) + " to " +
                        Misc::toString(end) +
                        ": contains segment over maxMergeSize or maxMergeDocs; "
                        "skipping");
...
int32_t LogMergePolicy::getMaxMergeDocs() { return maxMergeDocs; }

const char* LogMergePolicy::getClassName() { return "LogMergePolicy"; }
const char* LogMergePolicy::getObjectName() const { return getClassName(); }
...
int32_t LogDocMergePolicy::getMinMergeDocs() { return (int32_t)minMergeSize; }

int64_t LogDocMergePolicy::size(SegmentInfo* info) { return info->docCount; }

const char* LogDocMergePolicy::getClassName() { return "LogDocMergePolicy"; }
const char* LogDocMergePolicy::getObjectName() const { return getClassName(); }
...
const float_t LogByteSizeMergePolicy::DEFAULT_MAX_MERGE_MB =
    (float_t)LUCENE_INT64_MAX_SHOULDBE;
