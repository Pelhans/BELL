...
MultiReader::MultiReader(const CL_NS(util)::ArrayBase<IndexReader*>* subReaders,
                         bool closeSubReaders) {
    this->_internal = _CLNEW Internal();
    this->init(subReaders, closeSubReaders);
}

void MultiReader::init(const CL_NS(util)::ArrayBase<IndexReader*>* _subReaders,
                       bool closeSubReaders) {
    this->subReaders =
        _CLNEW CL_NS(util)::ValueArray<IndexReader*>(_subReaders->length);
    starts =
    ...
    for (size_t i = 0; i < subReaders->length; i++) {
        ...
        if ((*subReaders)[i]->hasDeletions()) _internal->_hasDeletions = true;
    }
...
IndexReader* MultiReader::reopen() {
    ...
    _CLFINALLY(if (!success && reopened) {
...
void MultiReader::getTermFreqVector(int32_t docNumber, const TCHAR* field,
...
void MultiReader::getTermFreqVector(int32_t docNumber,
                                    TermVectorMapper* mapper) {
    ensureOpen();
    int32_t i = readerIndex(docNumber); // find segment num
...
bool MultiReader::isOptimized() { return false; }
...
void MultiReader::doSetNorm(int32_t n, const TCHAR* field, uint8_t value) {
    _internal->normsCache.removeitr(
        _internal->normsCache.find((TCHAR*)field));         // clear cache
    int32_t i = readerIndex(n);                             // find segment num
...
TermPositions* MultiReader::termPositions() {
    ensureOpen();
    TermPositions* ret =
        (TermPositions*)_CLNEW MultiTermPositions(subReaders, starts);
    return ret;
}
...
int32_t MultiReader::readerIndex(
    const int32_t n) const { // find reader for doc n:
...
void MultiReader::doClose() {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    for (size_t i = 0; i < subReaders->length; i++) {
        if ((*subReaders)[i] == NULL) continue; // reopen may take some
                                                // memory...
...
void MultiReader::getFieldNames(FieldOption fieldNames,
                                StringArrayWithDeletor& retarray) {
    ensureOpen();
    return MultiSegmentReader::getFieldNames(fieldNames, retarray,
                                             this->subReaders);
}
...
const char* MultiReader::getClassName() { return "MultiReader"; }
const char* MultiReader::getObjectName() const { return getClassName(); }
CL_NS_END
