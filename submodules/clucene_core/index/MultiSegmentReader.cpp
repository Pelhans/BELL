...
#include "../document/FieldSelector.h"
#include "Term.h"
#include "Terms.h"
...
void MultiSegmentReader::initialize(
    CL_NS(util)::ArrayBase<IndexReader*>* _subReaders) {
    ...
    starts =
        _CL_NEWARRAY(int32_t, subReaders->length + 1); // build starts array
    for (size_t i = 0; i < subReaders->length; i++) {
        starts[i] = _maxDoc;

        // compute maxDocs
        _maxDoc += (*subReaders)[i]->maxDoc();
        if ((*subReaders)[i]->hasDeletions()) _hasDeletions = true;
    }
...
MultiSegmentReader::MultiSegmentReader(CL_NS(store)::Directory* directory,
    ...
    ArrayBase<IndexReader*>* readers =
        _CLNEW ObjectArray<IndexReader>(sis->size());
...
MultiSegmentReader::MultiSegmentReader(
    CL_NS(store)::Directory* directory, SegmentInfos* infos,
    bool closeDirectory, CL_NS(util)::ArrayBase<IndexReader*>* oldReaders,
    int32_t* oldStarts, NormsCacheType* oldNormsCache)
    ...
    if (oldReaders != NULL) {
        ...
        for (size_t i = 0; i < oldReaders->length; i++) {
            segmentReaders[((SegmentReader*)(*oldReaders)[i])
                               ->getSegmentName()] = i;
        }
    }

    ArrayBase<IndexReader*>* newReaders =
        _CLNEW ObjectArray<IndexReader>(infos->size());

    for (int32_t i = infos->size() - 1; i >= 0; i--) {
        // find SegmentReader for this segment
        map<string, size_t>::iterator oldReaderIndex =
            segmentReaders.find(infos->info(i)->name);
    ...
    if (oldNormsCache != NULL) {
        ...
        while (it != oldNormsCache->end()) {
            ...
            for (size_t i = 0; i < subReaders->length; i++) {
                map<string, size_t>::iterator oldReaderIndex =
                    segmentReaders.find(
                        ((SegmentReader*)(*subReaders)[i])->getSegmentName());

                // this SegmentReader was not re-opened, we can copy all of its
                // norms
                if (oldReaderIndex != segmentReaders.end() &&
                    ((*oldReaders)[oldReaderIndex->second] ==
                         (*subReaders)[i] ||
                     ((SegmentReader*)(*oldReaders)[oldReaderIndex->second])
                             ->_norms.get(field) ==
                         ((SegmentReader*)(*subReaders)[i])
                             ->_norms.get(field))) {
                    // we don't have to synchronize here: either this
                    // constructor is called from a SegmentReader, in which case
                    // no old norms cache is present, or it is called from
                    // MultiReader.reopen(), which is synchronized
                    memcpy(bytes + starts[i],
                           oldBytes + oldStarts[oldReaderIndex->second],
                           starts[i + 1] - starts[i]);
                } else {
...
DirectoryIndexReader* MultiSegmentReader::doReopen(SegmentInfos* infos) {
    ...
    if (infos->size() == 1) {
        ...
        // MultiSegmentReader. Return a new SegmentReader instead
...
ArrayBase<TermFreqVector*>* MultiSegmentReader::getTermFreqVectors(int32_t n) {
    ...
    return (*subReaders)[i]->getTermFreqVectors(
        n - starts[i]); // dispatch to segment
}

TermFreqVector* MultiSegmentReader::getTermFreqVector(int32_t n,
                                                      const TCHAR* field) {
    ensureOpen();
    int32_t i = readerIndex(n); // find segment num
    return (*subReaders)[i]->getTermFreqVector(n - starts[i], field);
}
void MultiSegmentReader::getTermFreqVector(int32_t docNumber,
                                           const TCHAR* field,
...
bool MultiSegmentReader::isOptimized() { return false; }
...
bool MultiSegmentReader::document(int32_t n, CL_NS(document)::Document& doc,
    ...
    ensureOpen();
    int32_t i = readerIndex(n); // find segment num
    return (*subReaders)[i]->document(
        n - starts[i], doc, fieldSelector); // dispatch to segment reader
}

bool MultiSegmentReader::isDeleted(const int32_t n) {
    // Don't call ensureOpen() here (it could affect performance)
    int32_t i = readerIndex(n); // find segment num
    return (*subReaders)[i]->isDeleted(n -
                                       starts[i]); // dispatch to segment reader
}
...
uint8_t* MultiSegmentReader::norms(const TCHAR* field) {
    ...
    if (!hasNorms(field)) return fakeNorms();

    bytes = _CL_NEWARRAY(uint8_t, maxDoc());
    for (size_t i = 0; i < subReaders->length; i++)
        (*subReaders)[i]->norms(field, bytes + starts[i]);

    // Unfortunately the data in the normCache can get corrupted, since it's
    // being loaded with string keys that may be deleted while still in use by
    // the map. To prevent this field is duplicated and then stored in the
    // normCache
...
void MultiSegmentReader::doSetNorm(int32_t n, const TCHAR* field,
                                   uint8_t value) {
...
TermPositions* MultiSegmentReader::termPositions() {
    ensureOpen();
    TermPositions* ret = static_cast<TermPositions*>(
        _CLNEW MultiTermPositions(subReaders, starts));
    return ret;
}
...
void MultiSegmentReader::doDelete(const int32_t n) {
    ...
    int32_t i = readerIndex(n); // find segment num
    (*subReaders)[i]->deleteDocument(n -
                                     starts[i]); // dispatch to segment reader
    _hasDeletions = true;
}

int32_t MultiSegmentReader::readerIndex(
    int32_t n) const { // find reader for doc n:
    return readerIndex(n, this->starts, this->subReaders->length);
}

int32_t MultiSegmentReader::readerIndex(
    const int32_t n, int32_t* starts,
    int32_t numSubReaders) {        // find reader for doc n:
...
void MultiSegmentReader::commitChanges() {
    for (size_t i = 0; i < subReaders->length; i++) (*subReaders)[i]->commit();
}
...
void MultiSegmentReader::getFieldNames(
    FieldOption fieldNames, StringArrayWithDeletor& retarray,
...
void MultiSegmentReader::getFieldNames(FieldOption fldOption,
...
const char* MultiSegmentReader::getClassName() { return "MultiSegmentReader"; }
const char* MultiSegmentReader::getObjectName() const { return getClassName(); }
...
MultiTermDocs::MultiTermDocs() {
    ...
    //       This constructor is needed to allow the constructor of
    //       MultiTermPositions initialise the instance by itself
    // Pre  - true
    // Post - An empty
...
TermPositions* MultiTermDocs::__asTermPositions() { return NULL; }

int32_t MultiTermDocs::doc() const {
    CND_PRECONDITION(current != NULL,
                     "current==NULL, check that next() was called");
...
void MultiTermDocs::seek(TermEnum* termEnum) { seek(termEnum->term(false)); }
...
void MultiTermDocs::seek(Term* tterm) {
    ...
    // MultiTermDocs with as argument this->term (seek(this->term)) that the
    // assignment will succeed and all referencecounters represent the correct
    // situation

    // Get a pointer from tterm and increase its reference counter
    Term* TempTerm = _CL_POINTER(tterm);
    // xx
    // Finialize term to ensure we decrease the reference counter of the
    // instance which term points to
    _CLDECDELETE(term);
...
int32_t MultiTermDocs::read(int32_t* docs, int32_t* freqs, int32_t length) {
    while (true) {
        ...
        if (end == 0) { // none left in segment
            ...
            for (int32_t i = 0; i < end; i++) docs[i] += b;
            return end;
        }
    }
}
...
MultiTermEnum::MultiTermEnum(ArrayBase<IndexReader*>* subReaders,
                             const int32_t* starts, const Term* t) {
    // Func - Constructor
    //       Opens all enumerations of all readers
    // Pre  - readers != NULL and contains an array of IndexReader instances
    // each responsible for
    //       reading a single segment
    //       subReaders->length >= 0 and represents the number of readers in the
    //       readers array starts is an array of
    // Post - An instance of has been created

    // Pre  - if readers is NULL then subReaders->length must be 0 else if
    // readers != NULL then subReaders->length > 0
    //       s != NULL
    // Post - The instance has been created
    ...
    for (size_t i = 0; i < subReaders->length; i++) {
        ...
        if (t != NULL) {
            // termEnum is an enumeration of terms starting at or after the
            // named term t
            termEnum = reader->terms(t);
        } else {
            // termEnum is an enumeration of all the Terms and TermInfos in the
            // set.
        ...
        // Note that in the call termEnum->getTerm(false) below false is
        // required because otherwise a reference is leaked. By passing false
        // getTerm is ordered to return an unowned reference instead. (Credits
        // for DSR)
...
bool MultiTermEnum::next() {
    ...
    // Pre  - true
    // Post - Returns true if term has been moved to the next in the set of
    // enumerations
    ...
    // Assign term the term of top and make sure the reference counter is
    // increased
...
void MultiTermEnum::close() {
    ...
    // Post - The queue has been closed all SegmentMergeInfo instance have been
    // deleted by
...
MultiTermPositions::MultiTermPositions(ArrayBase<IndexReader*>* r,
                                       const int32_t* s) {
    // Func - Constructor
    // Pre  - if r is NULL then rLen must be 0 else if r != NULL then rLen > 0
    //       s != NULL
...
TermDocs* MultiTermPositions::__asTermDocs() { return (TermDocs*)this; }
