...
void MultipleTermPositions::seek(Term*) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "Unsupported operation: MultipleTermPositions::seek");
}
...
int32_t MultipleTermPositions::read(int32_t*, int32_t*, int32_t) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "Unsupported operation: MultipleTermPositions::read");
}
...
uint8_t* MultipleTermPositions::getPayload(uint8_t*) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "Unsupported operation: MultipleTermPositions::getPayload");
}

bool MultipleTermPositions::isPayloadAvailable() const { return false; }

TermDocs* MultipleTermPositions::__asTermDocs() { return (TermDocs*)this; }
...
class MultipleTermPositions::TermPositionsQueue
    : public CL_NS(util)::PriorityQueue<
          TermPositions*, CL_NS(util)::Deletor::Object<TermPositions>> {
public:
    TermPositionsQueue(TermPositions** termPositions,
                       size_t termPositionsSize) {
...
public:
    IntQueue()
        : _array(_CLNEW ValueArray<int32_t>(16)),
          _index(0),
          _lastIndex(0) {}
    virtual ~IntQueue() { _CLLDELETE(_array); }

    void add(const int32_t i) {
        if (_lastIndex == _array->length) _array->resize(_array->length * 2);

        _array->values[_lastIndex++] = i;
    }

    int32_t next() { return _array->values[_index++]; }
    ...
    int32_t size() { return (_lastIndex - _index); }
};

MultipleTermPositions::MultipleTermPositions(
    IndexReader* indexReader, const CL_NS(util)::ArrayBase<Term*>* terms)
    ...
    TermPositions** tps =
        _CL_NEWARRAY(TermPositions*, terms->length + 1); // i == tpsSize
...
int32_t MultipleTermPositions::nextPosition() { return _posList->next(); }
...
int32_t MultipleTermPositions::doc() const { return _doc; }

int32_t MultipleTermPositions::freq() const { return _freq; }
