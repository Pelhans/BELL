...
Payload::Payload() : data(*_CLNEW CL_NS(util)::ValueArray<uint8_t>(0)) {
    // nothing to do
    this->deleteData = true;
    this->deleteArray = true;
}
Payload::Payload(CL_NS(util)::ValueArray<uint8_t>& _data, const int32_t offset,
                 const int32_t length, bool deleteData)
    : data(_data) {
...
void Payload::setData(CL_NS(util)::ValueArray<uint8_t>& data,
...
uint8_t Payload::byteAt(int index) const {
    ...
    _CLTHROWA(CL_ERR_IndexOutOfBounds,
              "Array index out of bounds at Payload::byteAt");
}

CL_NS(util)::ValueArray<uint8_t>* Payload::toByteArray() const {
    CL_NS(util)::ValueArray<uint8_t>* ret =
        _CLNEW CL_NS(util)::ValueArray<uint8_t>(this->_length);
    memcpy(ret->values, this->data.values + this->offset,
           this->_length * sizeof(uint8_t));
    return ret;
}
...
void Payload::copyTo(uint8_t* target, const int32_t targetLen) const {
    ...
    memcpy(target, this->data.values + this->offset,
           this->_length * sizeof(uint8_t));
}
