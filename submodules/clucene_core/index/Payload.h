 ...
 *  <p>
 *  Use {@link TermPositions#getPayloadLength()} and {@link
 * TermPositions#getPayload(byte[], int)} to retrieve the payloads from the
 * index.<br>
 *
 */
...
public:
     ...
     * @param deleteData delete data when payload is deleted
     */
    Payload(CL_NS(util)::ValueArray<uint8_t>& data, const int32_t offset = 0,
            const int32_t length = -1, bool deleteData = false);
     ...
     * @param deleteData delete data when payload is deleted
     */
    void setData(CL_NS(util)::ValueArray<uint8_t>& data,
                 const int32_t offset = 0, const int32_t length = -1,
     ...
     * it. Caller is responsible for deleting it later.
