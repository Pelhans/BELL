...
SegmentInfo::SegmentInfo(const char* _name, const int32_t _docCount,
                         CL_NS(store)::Directory* _dir, bool _isCompoundFile,
                         bool _hasSingleNormFile, int32_t _docStoreOffset,
    ...
    CND_PRECONDITION(
        docStoreOffset == -1 || !docStoreSegment.empty(),
        "failed testing for (docStoreOffset == -1 || docStoreSegment != NULL)");
...
string SegmentInfo::segString(Directory* dir) {
    ...
    return string(name) + ":" + cfs + string(this->dir == dir ? "" : "x") +
           Misc::toString(docCount) + docStore;
}
SegmentInfo::SegmentInfo(CL_NS(store)::Directory* _dir, int32_t format,
                         CL_NS(store)::IndexInput* input)
    : _sizeInBytes(-1) {
    this->dir = _dir;
...
void SegmentInfo::reset(const SegmentInfo* src) {
    ...
    if (src->normGen.values == NULL) {
        ...
        // optimized case to allocate new array only if current memory buffer is
        // too small
        ...
        memcpy(this->normGen.values, src->normGen.values,
               sizeof(int64_t) * this->normGen.length);
    }
    isCompoundFile = src->isCompoundFile;
    hasSingleNormFile = src->hasSingleNormFile;
}

SegmentInfo::~SegmentInfo() { normGen.deleteValues(); }
...
void SegmentInfo::setNumFields(const int32_t numFields) {
    if (normGen.values == NULL) {
        ...
        if (preLockless) {
            ...
            // later we know we have to check filesystem for norm files, because
            // this is prelockless.
...
void SegmentInfo::addIfExists(std::vector<std::string>& files,
                              const std::string& fileName) {
    if (dir->fileExists(fileName.c_str())) files.push_back(fileName);
}
...
const vector<string>& SegmentInfo::files() {
    ...
    string delFileName = IndexFileNames::fileNameFromGeneration(
        name.c_str(), (string(".") + IndexFileNames::DELETES_EXTENSION).c_str(),
        delGen);
    if (!delFileName.empty() &&
        (delGen >= YES || dir->fileExists(delFileName.c_str()))) {
    ...
    if (normGen.values != NULL) {
        for (size_t i = 0; i < normGen.length; i++) {
            ...
            if (gen >= YES) {
                ...
                _files.push_back(IndexFileNames::fileNameFromGeneration(
                    name.c_str(), gens.c_str(), gen));
            } else if (NO == gen) {
                // No separate norms but maybe plain norms
                // in the non compound file case:
                if (!hasSingleNormFile && !useCompoundFile) {
                    string fileName =
                        name + "." + IndexFileNames::PLAIN_NORMS_EXTENSION;
                    fileName += i;
                ...
                if (useCompoundFile) {
                    fileName =
                        name + "." + IndexFileNames::SEPARATE_NORMS_EXTENSION;
        ...
        for (size_t i = 0; i < allFiles.size(); i++) {
            ...
            if (fileName.length() > prefixLength &&
                _istdigit(fileName[prefixLength]) &&
...
SegmentInfo* SegmentInfo::clone() {
    ...
    if (this->normGen.values != NULL) {
        ...
        memcpy(si->normGen.values, this->normGen.values,
               sizeof(int64_t) * this->normGen.length);
    }
...
bool SegmentInfo::hasSeparateNorms(const int32_t fieldNumber) const {
    if ((normGen.values == NULL && preLockless) ||
        (normGen.values != NULL && normGen[fieldNumber] == CHECK_DIR)) {
        // Must fallback to directory file exists check:
        return dir->fileExists(
            (name + string(".s") + Misc::toString(fieldNumber)).c_str());
...
bool SegmentInfo::hasSeparateNorms() const {
    if (normGen.values == NULL) {
        if (!preLockless) {
            ...
            if (!dir->list(result)) {
                _CLTHROWA(CL_ERR_IO,
                          (string("cannot read directory: ") + dir->toString() +
                           string(" list() returned NULL"))
                              .c_str());
            }
            ...
            for (vector<string>::iterator itr = result.begin();
                 itr != result.end(); itr++) {
                if (strncmp(itr->c_str(), pattern.c_str(), pattern.length()) ==
                        0 &&
...
string SegmentInfo::getNormFileName(const int32_t number) const {
    ...
    if (hasSeparateNorms(number)) {
        ...
        return IndexFileNames::fileNameFromGeneration(name.c_str(), prefix,
                                                      gen);
    }
...
bool SegmentInfo::getUseCompoundFile() const {
    if (isCompoundFile == NO) {
        ...
        return dir->fileExists(
            ((string)name + "." + IndexFileNames::COMPOUND_FILE_EXTENSION)
                .c_str());
    }
}
 ...
 * @see #setInfoStream
 */
std::ostream* SegmentInfos::getInfoStream() { return infoStream; }
...
SegmentInfos::SegmentInfos(bool deleteMembers, int32_t reserveCount)
    ...
    if (reserveCount > 1) infos.reserve(reserveCount);
}
...
int64_t SegmentInfos::getCurrentSegmentGeneration(
    ...
    while (itr != files.end()) {
        file = itr->c_str();
        if (strncmp(file, IndexFileNames::SEGMENTS, seglen) == 0 &&
            strcmp(file, IndexFileNames::SEGMENTS_GEN) != 0) {
...
int64_t SegmentInfos::getCurrentSegmentGeneration(
    const CL_NS(store)::Directory* directory) {
    vector<string> files;
    if (!directory->list(&files)) {
        _CLTHROWA(CL_ERR_IO,
                  (string("cannot read directory ") + directory->toString() +
                   string(": list() returned NULL"))
                      .c_str());
    }
...
int64_t SegmentInfos::generationFromSegmentsFileName(const char* fileName) {
    if (strcmp(fileName, IndexFileNames::SEGMENTS) == 0) {
        return 0;
    } else if (strncmp(fileName, IndexFileNames::SEGMENTS,
                       strlen(IndexFileNames::SEGMENTS)) == 0) {
        return CL_NS(util)::Misc::base36ToLong(
            fileName + strlen(IndexFileNames::SEGMENTS) + 1);
    } else {
        TCHAR err[CL_MAX_PATH + 35];
        _sntprintf(err, CL_MAX_PATH + 35,
                   _T("fileName \"%s\" is not a segments file"), fileName);
...
void SegmentInfos::clearto(size_t from, size_t end) {
    ...
    if ((infos.size() - from) >=
        range) { // Make sure we actually need to remove
        segmentInfosType::iterator itr, bitr = infos.begin() + from,
                                        eitr = infos.end();
        size_t count = 0;
...
void SegmentInfos::add(SegmentInfo* info, int32_t pos) {
    if (pos == -1) {
        ...
        if (pos < 0 || pos >= (int32_t)infos.size() + 1)
            _CLTHROWA(CL_ERR_IllegalArgument, "pos is out of range");
        infos.insert(infos.begin() + pos, info);
    }
}
int32_t SegmentInfos::size() const { return infos.size(); }
SegmentInfo* SegmentInfos::elementAt(int32_t pos) { return infos.at(pos); }
...
void SegmentInfos::insert(SegmentInfo* info) { infos.push_back(info); }
    ...
    try {
        ...
        if (format >= 0) { // in old format the version number may be at the end
            ...
            if (input->getFilePointer() >= input->length())
                version = CL_NS(
                    util)::Misc::currentTimeMillis(); // old file format without
                                                      // version number
            else
                version = input->readLong(); // read version
        }
...
void SegmentInfos::write(Directory* directory) {
    // Func - Writes a new segments file based upon the SegmentInfo instances it
    // manages Pre  - directory is a valid reference to a Directory Post - The
    // new segment has been written to disk
    ...
    try {
        ...
        _CLFINALLY(output->close(); _CLDELETE(output);)
...
// void SegmentInfos::setDefaultGenFileRetryCount(const int32_t count) {
// defaultGenFileRetryCount = count; }
int32_t SegmentInfos::getDefaultGenFileRetryCount() {
    return defaultGenFileRetryCount;
}

// void SegmentInfos::setDefaultGenFileRetryPauseMsec(const int32_t msec) {
// defaultGenFileRetryPauseMsec = msec; }
int32_t SegmentInfos::getDefaultGenFileRetryPauseMsec() {
    return defaultGenFileRetryPauseMsec;
}

// void SegmentInfos::setDefaultGenLookaheadCount(const int32_t count) {
...
void SegmentInfos::_FindSegmentsFile::doRun() {
    ...
    while (true) {
        if (0 == method) {
            ...
            if (infoStream) {
                (*infoStream)
                    << "[SIS]: directory listing genA=" << genA << "\n";
            }
            ...
            if (directory != NULL) {
                ...
                for (int32_t i = 0; i < defaultGenFileRetryCount; i++) {
                    ...
                    if (!directory->openInput(IndexFileNames::SEGMENTS_GEN,
                                              genInput, e)) {
                        if (e.number() == CL_ERR_IO) {
                            if (infoStream) {
                                (*infoStream)
                                    << "[SIS]: segments.gen open: IOException "
                                    << e.what() << "\n";
                            }
                            break;
                        } else {
                    ...
                    if (genInput != NULL) {
                        try {
                            ...
                            if (version == FORMAT_LOCKLESS) {
                                ...
                                // CL_TRACE("fallback check: %d; %d", gen0,
                                // gen1);
                                if (gen0 == gen1) {
                    ...
                    /*
                    //todo: Wrap the LUCENE_SLEEP call above with the following
                    try/catch block if
                    //	  InterruptedException is implemented
                    try {
                    } catch (CLuceneError &e) {
                    //if (err2.number != CL_ERR_Interrupted) // retry only for
            ...
            // CL_TRACE("%s check: genB=%d", IndexFileNames::SEGMENTS_GEN,
            // genB);
            ...
            if (gen == -1) {
                ...
                _CLTHROWA(CL_ERR_IO, (string("No segments* file found in ") +
                                      directory->toString())
                                         .c_str());
            }
        }
        ...
        // CL_TRACE("primary Exception on '" + segmentFileName + "': " + err +
        // "'; will retry: retry=" + retry + "; gen = " + gen);
        ...
        if (!retry && gen > 1) {
            ...
            // try it if so:
            string prevSegmentFileName = IndexFileNames::fileNameFromGeneration(
            ...
            if (directory != NULL)
                ...
                prevExists = Misc::dir_Exists(
                    (string(fileDirectory) + prevSegmentFileName).c_str());

            if (prevExists) {
                // CL_TRACE("fallback to prior segment file '%s'",
                // prevSegmentFileName);
                CLuceneError saved_error;
                if (tryDoBody(prevSegmentFileName.c_str(), saved_error)) {
                    return;
                }
                // CL_TRACE("secondary Exception on '" + prevSegmentFileName +
                // "': " + err2 + "'; will retry");
            }
        }
    }
...
SegmentInfos::FindSegmentsVersion::FindSegmentsVersion(
...
int64_t SegmentInfos::FindSegmentsVersion::doBody(const char* segmentFileName) {
    ...
    // Therefore we have to read the whole file and cannot simply seek to the
    // version entry.
