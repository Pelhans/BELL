...
SegmentMergeInfo::SegmentMergeInfo(const int32_t b, TermEnum* te,
                                   IndexReader* r)
    : docMap(NULL),
      termEnum(te),
      base(b),
...
int32_t* SegmentMergeInfo::getDocMap() {
    if (docMap == NULL) {
        ...
        if (reader->hasDeletions()) {
            // Get the total number of documents managed by the reader including
            // the deleted ones
...
bool SegmentMergeInfo::next() {
    // Func - Moves the current term of the enumeration termEnum to the next and
    // term
    //       points to this new current term
    // Pre  - true
    // Post - Returns true if the term has been moved to the next otherwise
    // false
    if (termEnum->next()) {
        _CLDECDELETE(term);
        term = termEnum->term();
