...
SegmentMergeQueue::SegmentMergeQueue(const int32_t size) {
    ...
    // Call the initialize method of its superclass. The boolean value  passed
    // here indicates that the superclass PriorityQueue takes the responsibility
    // to have its elements deleted The destructor of SegmentMergInfo will make
    // sure that each intstance it will be closed properly before it is deleted
...
void SegmentMergeQueue::close() {
    ...
    // Pre  - true
    // post - All SegmentMergeInfo Instances in the queue have been closed and
    // deleted
...
bool SegmentMergeQueue::lessThan(SegmentMergeInfo* stiA,
                                 SegmentMergeInfo* stiB) {
    // Func - Overloaded method that implements the lessThan operator for the
    // parent class
    //       This method is used by the parent class Priority queue to reorder
    //       its internal data structures. This implementation check if stiA is
    //       less than the current term of stiB.
    // Pre  - stiA != NULL
    //		stiB != NULL
    ...
    // Check if they match
    if (comparison == 0) { // todo: can we do an optimized compare here? compare
                           // using equals, then compare properly?
        // If the match check if the base of stiA is smaller than the base of
        // stiB Note that different bases means that the terms of stiA an stiB
        // ly in different segments
