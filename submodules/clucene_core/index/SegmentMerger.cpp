...
#include <assert.h>
#include "../index/_IndexFileNames.h"
...
SegmentMerger::SegmentMerger(IndexWriter* writer, const char* name,
    ...
    this->segment = name;
    if (merge != NULL) this->checkAbort = _CLNEW CheckAbort(merge, directory);
...
int32_t SegmentMerger::merge(bool mergeDocStores) {
    ...
    if (mergeDocStores && fieldInfos->hasVectors()) mergeVectors();
...
void SegmentMerger::createCompoundFile(const char* filename,
                                       std::vector<std::string>* files) {
    CompoundFileWriter* cfsWriter =
        _CLNEW CompoundFileWriter(directory, filename, checkAbort);
    ...
    for (int32_t i = 0; i < IndexFileNames::COMPOUND_EXTENSIONS_LENGTH; i++) {
        ...
        if (mergeDocStores ||
            (strcmp(ext, IndexFileNames::FIELDS_EXTENSION) != 0 &&
...
void SegmentMerger::addIndexed(IndexReader* reader, FieldInfos* fieldInfos,
    ...
    while (itr != names.end()) {
        fieldInfos->add(*itr, true, storeTermVectors,
                        storePositionWithTermVector, storeOffsetWithTermVector,
                        !reader->hasNorms(*itr), storePayloads);

        ++itr;
    }
}

// for merging we don't want to compress/uncompress the data, so to tell the
// FieldsReader that we're in  merge mode, we use this FieldSelector
...
int32_t SegmentMerger::mergeFields() {
    ...
    if (!mergeDocStores) {
        ...
        assert(readers[readers.size() - 1]->instanceOf(
            SegmentReader::getClassName()));
    ...
    CND_CONDITION(fieldInfos != NULL,
                  "Memory allocation for fieldInfos failed");
    ...
    for (uint32_t i = 0; i < readers.size(); i++) {
        ...
        if (reader->instanceOf(SegmentReader::getClassName())) {
            ...
            for (size_t j = 0; j < segmentReader->getFieldInfos()->size();
                 j++) {
            ...
            tmp.clear();
            reader->getFieldNames(IndexReader::TERMVECTOR_WITH_POSITION_OFFSET,
                                  tmp);
            ...
            tmp.clear();
            reader->getFieldNames(IndexReader::STORES_PAYLOADS, tmp);
            ...
            if (tmp.size() > 0) {
                ...
                _CLDELETE_ARRAY(arr); // no need to delete the contents, since
                                      // tmp is responsible for it
            }
        }
    }
    ...
    fieldInfos->write(directory,
                      Misc::segmentname(segment.c_str(), ".fnm").c_str());
    ...
    if (mergeDocStores) {
        ...
        for (size_t i = 0; i < readers.size(); i++) {
            ...
            if (reader->instanceOf(SegmentReader::getClassName())) {
                ...
                for (size_t j = 0; same && j < segmentFieldInfos->size(); j++)
                    same = _tcscmp(fieldInfos->fieldName(j),
                                   segmentFieldInfos->fieldName(j)) == 0;
                if (same) {
        ...
        try {
            for (size_t i = 0; i < readers.size(); i++) {
                IndexReader* reader = readers[i];
                SegmentReader* matchingSegmentReader =
                    matchingSegmentReaders[i];
                FieldsReader* matchingFieldsReader;
                if (matchingSegmentReader != NULL)
                    matchingFieldsReader =
                        matchingSegmentReader->getFieldsReader();
                else
                ...
                for (int32_t j = 0; j < maxDoc;) {
                    if (!reader->isDeleted(j)) { // skip deleted docs
                        if (matchingSegmentReader != NULL) {
                            ...
                            IndexInput* stream = matchingFieldsReader->rawDocs(
                                rawDocLengths.values, start, numDocs);
                            fieldsWriter.addRawDocuments(
                                stream, rawDocLengths.values, numDocs);
                            docCount += numDocs;
                            if (checkAbort != NULL)
            ...
            docCount * 8 ==
                directory->fileLength(
                    (segment + "." + IndexFileNames::FIELDS_INDEX_EXTENSION)
                        .c_str()),
            (string("after mergeFields: fdx size mismatch: ") +
             Misc::toString(docCount) + " docs vs " +
             Misc::toString(directory->fileLength(
                 (segment + "." + IndexFileNames::FIELDS_INDEX_EXTENSION)
                     .c_str())) +
...
void SegmentMerger::mergeVectors() {
    ...
    _CLFINALLY(if (termVectorsWriter != NULL) {
        ...
        4 + mergedDocs * 8 ==
            directory->fileLength(
                (segment + "." + IndexFileNames::VECTORS_INDEX_EXTENSION)
                    .c_str()),
        (string("after mergeVectors: tvx size mismatch: ") +
         Misc::toString(mergedDocs) + " docs vs " +
         Misc::toString(directory->fileLength(
             (segment + "." + IndexFileNames::VECTORS_INDEX_EXTENSION)
                 .c_str())) +
         " length in bytes of " + segment + "." +
...
void SegmentMerger::mergeTerms() {
    ...
    try {
        ...
        freqOutput = directory->createOutput(
            Misc::segmentname(segment.c_str(), ".frq").c_str());

        // Open an IndexOutput to the new Prox File
        proxOutput = directory->createOutput(
            Misc::segmentname(segment.c_str(), ".prx").c_str());
        ...
        termInfosWriter = _CLNEW TermInfosWriter(directory, segment.c_str(),
        ...
        skipListWriter = _CLNEW DefaultSkipListWriter(
            skipInterval, maxSkipLevels, mergedDocs, freqOutput, proxOutput);
...
void SegmentMerger::mergeTermInfos() {
    ...
    for (uint32_t i = 0; i < readers.size(); i++) {
        ...
        // Instantiate a new SegmentMerginfo for the current reader and
        // enumeration
        ...
        // Increase the base by the number of documents that have not been
        // marked deleted so base will contain a new value for the first
        // document of the next iteration
        base += reader->numDocs();
        // Get the next current term
        if (smi->next()) {
            // Store the SegmentMergeInfo smi with the initialized
            // SegmentTermEnum TermEnum into the queue
            queue->put(smi);
        } else {
            // Apparently the end of the TermEnum of the SegmentTerm has been
            // reached so close the SegmentMergeInfo smi
            smi->close();
            // And destroy the instance and set smi to NULL (It will be used
    ...
    while (queue->size() > 0) {
        ...
        // Check if term matches the term of the SegmentMergeInfo instances in
        // the queue
        while (top != NULL && term->equals(top->term)) {
            // A match has been found so add the matching SegmentMergeInfo to
            // the match array
            match[matchSize++] = queue->pop();
        ...
        // Restore the SegmentTermInfo instances in the match array back into
        // the queue
        while (matchSize > 0) {
            smi = match[--matchSize];
...
int32_t SegmentMerger::mergeTermInfo(SegmentMergeInfo** smis, int32_t n) {
    ...
    // Pre  - smis != NULL and it contains segments that are positioned at the
    // same term.
    ...
    // df contains the number of documents across all segments where this term
    // was found
    if (df > 0) {
...
int32_t SegmentMerger::appendPostings(SegmentMergeInfo** smis, int32_t n) {
    ...
    //       freqOutput != NULL
    //       proxOutput != NULL
    // Post - Returns number of documents across all segments where this term
    // was found
    ...
    bool storePayloads =
        fieldInfos->fieldInfo(smis[0]->term->field())->storePayloads;
    ...
    for (int32_t i = 0; i < n; i++) {
        ...
        while (postings->next()) {
            ...
            if (doc < 0 || (df > 0 && doc <= lastDoc))
                _CLTHROWA(CL_ERR_CorruptIndex,
                          (string("docs out of order (") + Misc::toString(doc) +
            ...
            for (int32_t j = 0; j < freq; j++) {
                ...
                if (storePayloads) {
                    ...
                    if (payloadLength > 0) {
                        ...
                        proxOutput->writeBytes(payloadBuffer.values,
                                               payloadLength);
                    }
                } else {
            ...
            if (fi->isIndexed && !fi->omitNorms) {
                ...
                if (output == NULL) {
                    output = directory->createOutput(
                        (segment + "." + IndexFileNames::NORMS_EXTENSION)
                            .c_str());
                ...
                for (uint32_t j = 0; j < readers.size(); j++) {
                    ...
                    // Condition check to see if reader points to a valid
                    // instance
                    ...
                    // Get an IndexInput to the norm file for this field in this
                    // segment
                    ...
                    if (checkAbort != NULL) checkAbort->work(maxDoc);
                }
            }
        }
    }
    _CLFINALLY(if (output != NULL) {
        output->close();
        _CLDELETE(output);
    });
}

SegmentMerger::CheckAbort::CheckAbort(MergePolicy::OneMerge* merge,
                                      Directory* dir) {
    this->merge = merge;
    this->dir = dir;
    this->workCount = 0;
