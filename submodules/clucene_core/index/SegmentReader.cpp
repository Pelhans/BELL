...
SegmentReader::Norm::Norm(IndexInput* instrm, bool _useSingleNormStream,
                          int32_t n, int64_t ns, SegmentReader* r,
                          const char* seg)
    : number(n),
      normSeek(ns),
      _this(r),
...
SegmentReader::Norm::~Norm() {
    ...
    // by its destructor. Note that the IndexInput 'in' actually is a
    // pointer!!!!!
    if (in != _this->singleNormStream) _CLDELETE(in);
...
void SegmentReader::Norm::reWrite(SegmentInfo* si) {
    ...
    IndexOutput* out = _this->directory()->createOutput(
        si->getNormFileName(this->number).c_str());
    try {
...
void SegmentReader::initialize(SegmentInfo* si, int32_t readBufferSize,
                               bool doOpenStores, bool doingReopen) {
    ...
    try {
        ...
        if (si->getUseCompoundFile()) {
            cfsReader = _CLNEW CompoundFileReader(
                directory(),
        ...
        if (doOpenStores) {
            if (si->getDocStoreOffset() != -1) {
                if (si->getDocStoreIsCompoundFile()) {
                    storeCFSReader = _CLNEW CompoundFileReader(
                        directory(),
        ...
        if (doOpenStores) {
            fieldsReader = _CLNEW FieldsReader(
                storeDir, fieldsSegment.c_str(), _fieldInfos, readBufferSize,
        ...
        tis = _CLNEW TermInfosReader(cfsDir, segment.c_str(), _fieldInfos,
                                     readBufferSize);
        ...
        // so that if an index update removes them we'll still have them
        freqStream =
            cfsDir->openInput((segment + ".frq").c_str(), readBufferSize);
        proxStream =
            cfsDir->openInput((segment + ".prx").c_str(), readBufferSize);
        openNorms(cfsDir, readBufferSize);

        if (doOpenStores &&
            _fieldInfos
                ->hasVectors()) { // open term vector files only as needed
            string vectorsSegment;
            if (si->getDocStoreOffset() != -1)
                vectorsSegment = si->getDocStoreSegment();
            else
                vectorsSegment = segment;
            termVectorsReaderOrig = _CLNEW TermVectorsReader(
                storeDir, vectorsSegment.c_str(), _fieldInfos, readBufferSize,
                si->getDocStoreOffset(), si->docCount);
        }
        success = true;
    }
...
SegmentReader* SegmentReader::get(Directory* dir, SegmentInfo* si,
                                  SegmentInfos* sis, bool closeDir, bool ownDir,
                                  int32_t readBufferSize, bool doOpenStores) {
    SegmentReader* instance =
        _CLNEW SegmentReader(); // todo: make this configurable...
    instance->init(dir, sis, closeDir);
    instance->initialize(
        si,
        readBufferSize == -1 ? BufferedIndexInput::BUFFER_SIZE : readBufferSize,
        doOpenStores, false);
    return instance;
}

SegmentReader::SegmentReader() : DirectoryIndexReader(), _norms(false, true) {}
SegmentReader::~SegmentReader() {
    // Func - Destructor.
    // Pre  - doClose has been invoked!
    // Post - the instance has been destroyed

    doClose(); // this means that index reader doesn't need to be closed
               // manually
...
bool SegmentReader::hasDeletions(const SegmentInfo* si) {
    ...
    // Post - if the segement contains deleteions true is returned otherwise
    // flas
...
bool SegmentReader::isDeleted(const int32_t n) {
    ...
    // Pre  - n >=0 and identifies the document n
    // Post - true has been returned if document n has been deleted otherwise
    // fralse
...
int32_t SegmentReader::numDocs() {
    ...
    // Get the number of all the documents in the segment including the ones
    // that have been marked deleted
    int32_t n = maxDoc();

    // Check if there any deleted docs
    if (deletedDocs != NULL)
        // Substract the number of deleted docs from the number returned by
        // maxDoc
        n -= deletedDocs->count();
...
void SegmentReader::getFieldNames(FieldOption fldOption,
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (fldOption & IndexReader::ALL) {
            ...
            if (!fi->isIndexed && (fldOption & IndexReader::UNINDEXED))
                ...
                v = true;
                     ...
                     (fldOption & IndexReader::STORES_PAYLOADS))
                v = true;
        ...
        if (v) retarray.push_back(STRDUP_TtoT(fi->name));
    }
}
...
void SegmentReader::norms(const TCHAR* field, uint8_t* bytes) {
    // Func - Reads the Norms for field from disk starting at offset in the
    // inputstream Pre  - field != NULL
    //       bytes != NULL is an array of bytes which is to be used to read the
    //       norms into. it is advisable to have bytes initalized by zeroes!
    // Post - The if an inputstream to the norm file could be retrieved the
    // bytes have been read
    //       You are never sure whether or not the norms have been read into
    //       bytes properly!!!!!!!!!!!!!!!!!
...
uint8_t* SegmentReader::createFakeNorms(int32_t size) {
    ...
    if (size > 0) memset(ones, DefaultSimilarity::encodeNorm(1.0f), size);
    return ones;
}
...
DirectoryIndexReader* SegmentReader::doReopen(SegmentInfos* infos) {
    ...
    if (infos->size() == 1) {
        ...
        if (segment.compare(si->name) == 0 &&
            si->getUseCompoundFile() == this->si->getUseCompoundFile()) {
        ...
        return _CLNEW MultiSegmentReader(_directory, infos, closeDirectory,
                                         &readers, NULL, NULL);
    }
...
uint8_t* SegmentReader::norms(const TCHAR* field) {
    ...
    // Pre  - field != NULL and contains the name of the field for which the
    // norms
    //       must be retrieved
    // Post - If there was norm for the named field then a bytes array has been
    // allocated
...
string SegmentReader::SegmentName(const char* ext, const int32_t x) {
    ...
    //       x contains a number
    // Post - A buffer has been instantiated an when x = -1 buffer contains the
    // concatenation of
    //       segment and ext otherwise buffer contains the contentation of
    //       segment, ext and x
...
void SegmentReader::openNorms(Directory* cfsDir, int32_t readBufferSize) {
    ...
    // Pre  - true
    // Post - For each field a norm instance has been created with an open
    // inputstream to
    //       a corresponding norm file ready to be read
    int64_t nextNormSeek =
        SegmentMerger::NORMS_HEADER_length; // skip header (header unused for
                                            // now)
    int32_t _maxDoc = maxDoc();
    for (size_t i = 0; i < _fieldInfos->size(); i++) {
        FieldInfo* fi = _fieldInfos->fieldInfo(i);
        if (_norms.find(fi->name) != _norms.end()) {
            // in case this SegmentReader is being re-opened, we might be able
            // to reuse some norm instances and skip loading them here
            continue;
        }
        if (fi->isIndexed && !fi->omitNorms) {
            ...
            bool singleNormFile =
                fileName.compare(fileName.length() - ext.length(), ext.length(),
                                 ext) == 0;
            ...
            if (singleNormFile) {
                ...
                if (singleNormStream == NULL) {
                    singleNormStream =
                        d->openInput(fileName.c_str(), readBufferSize);
                }
                // All norms in the .nrm file can share a single IndexInput
                // since they are only used in a synchronized context. If this
                // were to change in the future, a clone could be done here.
            ...
            _norms[fi->name] =
                _CLNEW Norm(normInput, singleNormFile, fi->number, normSeek,
                            this, segment.c_str());
            nextNormSeek +=
...
FieldsReader* SegmentReader::getFieldsReader() { return fieldsReader; }

FieldInfos* SegmentReader::getFieldInfos() { return _fieldInfos; }
...
ArrayBase<TermFreqVector*>* SegmentReader::getTermFreqVectors(
    int32_t docNumber) {
    ensureOpen();
    if (termVectorsReaderOrig == NULL) return NULL;
...
void SegmentReader::getTermFreqVector(int32_t docNumber, const TCHAR* field,
...
void SegmentReader::getTermFreqVector(int32_t docNumber,
                                      TermVectorMapper* mapper) {
    ensureOpen();
    if (termVectorsReaderOrig == NULL) return;

    TermVectorsReader* termVectorsReader = getTermVectorsReader();
    if (termVectorsReader == NULL) return;
...
void SegmentReader::loadDeletedDocs() {
    ...
    if (hasDeletions(si)) {
        deletedDocs =
            _CLNEW BitVector(directory(), si->getDelFileName().c_str());
...
SegmentReader* SegmentReader::reopenSegment(SegmentInfo* si) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    bool deletionsUpToDate =
        (this->si->hasDeletions() == si->hasDeletions()) &&
        (!si->hasDeletions() ||
         this->si->getDelFileName().compare(si->getDelFileName()) == 0);
    bool normsUpToDate = true;

    ValueArray<bool> fieldNormsChanged(_fieldInfos->size());
    if (normsUpToDate) {
        for (size_t i = 0; i < _fieldInfos->size(); i++) {
            if (this->si->getNormFileName(i).compare(si->getNormFileName(i)) !=
                0) {
                normsUpToDate = false;
    ...
    if (normsUpToDate && deletionsUpToDate) {
        this->si = si; // force the result to use the new segment info (the old
                       // one is going to go away!)
        return this;
    }
    ...
    try {
        ...
        if (fieldsReader != NULL) {
            clone->fieldsReader = _CLNEW FieldsReader(
                storeDir, fieldsSegment.c_str(), _fieldInfos, readBufferSize,
                si->getDocStoreOffset(), si->docCount);
        }
        ...
        if (!normsUpToDate) {
            ...
            for (size_t i = 0; i < fieldNormsChanged.length; i++) {
                // copy unchanged norms to the cloned reader and incRef those
                // norms
            ...
            clone->openNorms(si->getUseCompoundFile() ? cfsReader : directory(),
                             readBufferSize);
        } else {
            NormsType::iterator it = _norms.begin();
            while (it != _norms.end()) {
        ...
        if (clone->singleNormStream == NULL) {
            for (size_t i = 0; i < _fieldInfos->size(); i++) {
                ...
                if (fi->isIndexed && !fi->omitNorms) {
                    Directory* d =
                        si->getUseCompoundFile() ? cfsReader : directory();
                    string fileName = si->getNormFileName(fi->number);
                    if (si->hasSeparateNorms(fi->number)) {
                        continue;
                    }
                    ...
                    string ext = string(".") + IndexFileNames::NORMS_EXTENSION;
                    if (fileName.compare(fileName.length() - ext.length(),
                                         ext.length(), ext) == 0) {
                        clone->singleNormStream =
                            d->openInput(fileName.c_str(), readBufferSize);
                        break;
                    }
                }
...
FieldInfos* SegmentReader::fieldInfos() { return _fieldInfos; }

/**
 * Return the name of the segment this reader is reading.
 */
const char* SegmentReader::getSegmentName() { return segment.c_str(); }

/**
 * Return the SegmentInfo of the segment this reader is reading.
 */
SegmentInfo* SegmentReader::getSegmentInfo() { return si; }

void SegmentReader::setSegmentInfo(SegmentInfo* info) { si = info; }
...
const char* SegmentReader::getClassName() { return "SegmentReader"; }
const char* SegmentReader::getObjectName() const { return getClassName(); }
