...
SegmentTermDocs::SegmentTermDocs(const SegmentReader* _parent)
    : parent(_parent),
      freqStream(_parent->freqStream->clone()),
      count(0),
      df(0),
      deletedDocs(_parent->deletedDocs),
      _doc(0),
      _freq(0),
      skipInterval(_parent->tis->getSkipInterval()),
      maxSkipLevels(_parent->tis->getMaxSkipLevels()),
      skipListReader(NULL),
      freqBasePointer(0),
      proxBasePointer(0),
      skipPointer(0),
...
SegmentTermDocs::~SegmentTermDocs() { close(); }

TermPositions* SegmentTermDocs::__asTermPositions() { return NULL; }
...
void SegmentTermDocs::seek(TermEnum* termEnum) {
    ...
    // use comparison of fieldinfos to verify that termEnum belongs to the same
    // segment as this SegmentTermDocs
...
bool SegmentTermDocs::next() {
    while (true) {
        ...
        if ((deletedDocs == NULL) ||
            (_doc >= 0 && deletedDocs->get(_doc) == false))
            break;
        skippingDoc();
    }
    return true;
...
int32_t SegmentTermDocs::read(int32_t* docs, int32_t* freqs, int32_t length) {
    ...
    // dirs and iterate over them instead of using readByte() intensive
    // functions.
...
bool SegmentTermDocs::skipTo(const int32_t target) {
    ...
    if (df >= skipInterval) { // optimized case
        if (skipListReader == NULL)
            skipListReader =
                _CLNEW DefaultSkipListReader(freqStream->clone(), maxSkipLevels,
                                             skipInterval); // lazily clone

        if (!haveSkipped) { // lazily initialize skip stream
            skipListReader->init(skipPointer, freqBasePointer, proxBasePointer,
                                 df, currentFieldStoresPayloads);
            haveSkipped = true;
        }
