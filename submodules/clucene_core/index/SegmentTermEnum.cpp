...
SegmentTermEnum::SegmentTermEnum(IndexInput* i, FieldInfos* fis, const bool isi)
    ...
    if (firstInt >= 0) {
        ...
        indexInterval = 128;
        skipInterval =
            LUCENE_INT32_MAX_SHOULDBE; // switch off skipTo optimization
        ...
        if (format == -1) {
            ...
            // order to avoid a bug in skipTo implementation of these versions
...
SegmentTermEnum::SegmentTermEnum(const SegmentTermEnum& clone)
    ...
    buffer = clone.buffer == NULL
                 ? NULL
                 : (TCHAR*)malloc(sizeof(TCHAR) * (clone.bufferLength + 1));
    bufferLength = clone.bufferLength;
    prev = clone.prev == NULL
               ? NULL
               : _CLNEW Term(clone.prev->field(), clone.prev->text(), false);
    size = clone.size;
...
bool SegmentTermEnum::next() {
    ...
    if (prev != NULL) {
        if (_LUCENE_ATOMIC_INT_GET(prev->__cl_refcount) > 1) {
            _CLDECDELETE(
                prev); // todo: tune other places try and delete its term
        } else
...
void SegmentTermEnum::seek(const int64_t pointer, const int32_t p, Term* t,
    ...
    //       t is a valid reference to a Term and is the new current term in the
    //       enumeration ti is a valid reference to a TermInfo and is
    //       corresponding TermInfo form the new current Term
    ...
    if (_term == NULL || _LUCENE_ATOMIC_INT_GET(_term->__cl_refcount) > 1) {
        ...
        _term = _CLNEW Term; // cannot use reference, because TermInfosReader
                             // uses non ref-counted array
    }
    _term->set(t, t->text());
...
Term* SegmentTermEnum::readTerm(Term* reuse) {
    ...
    // current chars in buffer and the new ones yet to be read
    ...
    // Read a length number of characters into the buffer from position start in
...
void SegmentTermEnum::growBuffer(const uint32_t length, bool force_copy) {
    ...
    //       has been allocated of length length+1 and the text of term has been
    //       copied to buffer
