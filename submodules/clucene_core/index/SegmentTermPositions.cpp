...
SegmentTermPositions::SegmentTermPositions(const SegmentReader* _parent)
    : SegmentTermDocs(_parent),
      proxStream(NULL) // the proxStream will be cloned lazily when
...
SegmentTermPositions::~SegmentTermPositions() { close(); }

TermDocs* SegmentTermPositions::__asTermDocs() { return (TermDocs*)this; }
...
void SegmentTermPositions::seek(const TermInfo* ti, Term* term) {
    ...
    if (ti != NULL) lazySkipPointer = ti->proxPointer;
...
void SegmentTermPositions::skippingDoc() { lazySkipProxCount += _freq; }
...
int32_t SegmentTermPositions::read(int32_t* /*docs*/, int32_t* /*freqs*/,
    ...
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "TermPositions does not support processing multiple documents in "
              "one call. Use TermDocs instead.");
}

void SegmentTermPositions::skipProx(const int64_t proxPointer,
                                    const int32_t _payloadLength) {
...
uint8_t* SegmentTermPositions::getPayload(uint8_t* data) {
    if (!needToLoadPayload) {
        _CLTHROWA(CL_ERR_IO, "Payload cannot be loaded more than once for the "
                             "same term position.");
    }
