...
MultiLevelSkipListReader::MultiLevelSkipListReader(IndexInput* _skipStream,
                                                   const int32_t maxSkipLevels,
                                                   const int32_t _skipInterval)
    : maxNumberOfSkipLevels(maxSkipLevels),
      numberOfLevelsToBuffer(1),
      skipStream(
          CL_NS(util)::ObjectArray<CL_NS(store)::IndexInput>(maxSkipLevels)),
    ...
    this->inputIsBuffered =
        _skipStream->instanceOf(BufferedIndexInput::getClassName());
...
int32_t MultiLevelSkipListReader::getDoc() const { return lastDoc; }
...
void MultiLevelSkipListReader::close() {
    for (int32_t i = 1; i < maxNumberOfSkipLevels; i++) {
        if (skipStream[i] != NULL) {
            // skipStream[i]->close();
            _CLDELETE(skipStream[i]); // ISH: We actually do need to nullify
                                      // pointer here
        }
    }
}

void MultiLevelSkipListReader::init(const int64_t _skipPointer,
                                    const int32_t df) {
...
void MultiLevelSkipListReader::loadSkipLevels() {
    numberOfSkipLevels = (docCount == 0)
                             ? 0
                             : (int32_t)floor(log((double)docCount) /
                                              log((double)skipInterval[0]));
...
MultiLevelSkipListReader::SkipBuffer::SkipBuffer(IndexInput* input,
...
MultiLevelSkipListReader::SkipBuffer::~SkipBuffer() { _CLLDELETE(data); }
...
uint8_t MultiLevelSkipListReader::SkipBuffer::readByte() { return data[pos++]; }

void MultiLevelSkipListReader::SkipBuffer::readBytes(uint8_t* b,
                                                     const int32_t len) {
...
DefaultSkipListReader::DefaultSkipListReader(
    CL_NS(store)::IndexInput* _skipStream, const int32_t maxSkipLevels,
    const int32_t _skipInterval)
...
void DefaultSkipListReader::init(const int64_t _skipPointer,
                                 const int64_t freqBasePointer,
                                 const int64_t proxBasePointer,
                                 const int32_t df, const bool storesPayloads) {
...
int32_t DefaultSkipListReader::readSkipData(
    const int32_t level, CL_NS(store)::IndexInput* _skipStream) {
    int32_t delta;
