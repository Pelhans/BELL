...
void MultiLevelSkipListWriter::bufferSkip(int32_t df) {
    ...
    // determine max level
    for (numLevels = 0;
         (df % skipInterval) == 0 && numLevels < numberOfSkipLevels;
         df /= skipInterval) {
        numLevels++;
    }
...
MultiLevelSkipListWriter::MultiLevelSkipListWriter(int32_t skipInterval,
    ...
    numberOfSkipLevels =
        df == 0 ? 0
                : (int32_t)floor(log((float_t)df) / log((float_t)skipInterval));
...
MultiLevelSkipListWriter::~MultiLevelSkipListWriter() { _CLDELETE(skipBuffer); }

void MultiLevelSkipListWriter::init() {
    skipBuffer = _CLNEW CL_NS(util)::ObjectArray<CL_NS(store)::RAMOutputStream>(
        numberOfSkipLevels);
...
void DefaultSkipListWriter::setSkipData(int32_t doc, bool storePayloads,
...
void DefaultSkipListWriter::resetSkip() {
    ...
    Arrays<int32_t>::fill(
        lastSkipPayloadLength, numberOfSkipLevels,
        -1); // we don't have to write the first length in the skip list
...
void DefaultSkipListWriter::writeSkipData(int32_t level,
                                          IndexOutput* skipBuffer) {
    // To efficiently store payloads in the posting lists we do not store the
    // length of every payload. Instead we omit the length for a payload if the
    // previous payload had the same length. However, in order to support
    // skipping the payload length at every skip point must be known. So we use
    // the same length encoding that we use for the posting lists for the skip
    // data as well: Case 1: current field does not store payloads
    //           SkipDatum                 --> DocSkip, FreqSkip, ProxSkip
    //           DocSkip,FreqSkip,ProxSkip --> VInt
    //           DocSkip records the document number before every SkipInterval
    //           th  document in TermFreqs. Document numbers are represented as
    //           differences from the previous value in the sequence.
    // Case 2: current field stores payloads
    //           SkipDatum                 --> DocSkip, PayloadLength?,
    //           FreqSkip,ProxSkip DocSkip,FreqSkip,ProxSkip --> VInt
    //           PayloadLength             --> VInt
    ...
    if (curStorePayloads) {
        ...
        if (curPayloadLength == lastSkipPayloadLength[level]) {
            // the current payload length equals the length at the previous skip
            // point, so we don't store the length again
            skipBuffer->writeVInt(delta * 2);
        } else {
            // the payload length is different from the previous one. We shift
            // the DocSkip, set the lowest bit and store the current payload
            // length as VInt.
    ...
    skipBuffer->writeVInt(
        (int32_t)(curFreqPointer - lastSkipFreqPointer[level]));
    skipBuffer->writeVInt(
        (int32_t)(curProxPointer - lastSkipProxPointer[level]));
...
DefaultSkipListWriter::DefaultSkipListWriter(int32_t skipInterval,
