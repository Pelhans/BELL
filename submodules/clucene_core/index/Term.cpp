...
Term::Term(const TCHAR* fld, const TCHAR* txt, bool internField) {
    // Func - Constructor.
    //       Constructs a Term with the given field and text. Field and text are
    //       not copied Field and text are deleted in destructor only if intern
    //       is false.
    // Pre  - fld != NULL and contains the name of the field
    //       txt != NULL and contains the value of the field
    //       internF is true or false and indicates if term Field is interned or
    //       not internT is true or false and indicates if term Text is interned
    //       or not canDelete defaults to true but can be false and indicates to
    //       the IGarbageCollector that the Term can be deleted when finalized
    // Post - An instance of Term has been created.Field and txt have not been
    // copied but assigned
...
Term::~Term() {
    ...
    // if pre(intrn) is false

    // Unintern field
    if (internF) CLStringIntern::unintern(_field);
    _field = NULL;
...
const TCHAR* Term::field() const {
    // Func - Returns the field of this term, an interned string. The field
    // indicates
...
const TCHAR* Term::text() const {
    // Func - Returns the text of this term.  In the case of words, this is
    // simply the
...
void Term::set(const TCHAR* fld, const TCHAR* txt, const bool internField) {
...
#ifdef LUCENE_TERM_TEXT_LENGTH
    if (textLen > LUCENE_TERM_TEXT_LENGTH) textLen = LUCENE_TERM_TEXT_LENGTH;
    ...
    if (internF) CLStringIntern::unintern(oldField);
    internF = internField;
...
size_t Term::hashCode() {
    if (cachedHashCode == 0)
        cachedHashCode =
            Misc::thashCode(_field) + Misc::thashCode(_text, textLen);
...
int32_t Term::compareTo(const Term* other) const {
    // Func - Compares two terms, to see if this term belongs before,is equal to
    // or after
    //       after the argument term.
    // Pre  - other is a reference to another term
    // Post - A negative integer is returned if this term belongs before the
    // argument,
    //       zero is returned if this term is equal to the argument, and a
...
TCHAR* Term::toString() const {
    // Func - Forms the contents of Field and term in some kind of tuple
    // notation
    //       <field:text>
    // Pre  - true
    // Post - a string formatted as <field:text> is returned if pre(field) is
    // NULL and
    //       text is NULL the returned string will be formatted as <:>

    // Note: Should this representation ever change, make sure to update Query
    // and Filter classes
    //       that may be using this format without calling toString (to save on
    //       memory allocations) For example: PrefixFilter::toString()
