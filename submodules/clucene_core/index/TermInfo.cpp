...
TermInfo::TermInfo(const int32_t df, const int64_t fp, const int64_t pp) {
    // Func - Constructor.
    // Pre  - df >= 0, fp >= 0 pp >= 0
    // Post - An instance has been created with FreqPointer = fp, proxPointer=pp
    // and docFreq= df
...
TermInfo::TermInfo(const TermInfo* ti) {
    // Func - Constructor.
    //       Initialises this instance by copying the values of another TermInfo
    //       ti
    ...
    CND_PRECONDITION(ti->freqPointer >= 0,
                     "ti->freqPointer contains negative number");
    CND_PRECONDITION(ti->proxPointer >= 0,
                     "ti->proxPointer contains negative number");
...
void TermInfo::set(const int32_t df, const int64_t fp, const int64_t pp,
                   int32_t so) {
    // Func - Sets a new document frequency, a new freqPointer and a new
    // proxPointer Pre  - df >= 0, fp >= 0 pp >= 0 Post - The new document
    // frequency, a new freqPointer and a new proxPointer
    //       have been set
