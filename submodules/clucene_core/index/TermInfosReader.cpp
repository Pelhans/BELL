...
TermInfosReader::TermInfosReader(Directory* dir, const char* seg,
    ...
    try {
        ...
        // segment
        origEnum = _CLNEW SegmentTermEnum(
            directory->openInput(tisFile.c_str(), readBufferSize), fieldInfos,
            false);
        _size = origEnum->size;
        totalIndexInterval = origEnum->indexInterval;
        indexEnum = _CLNEW SegmentTermEnum(
            directory->openInput(tiiFile.c_str(), readBufferSize), fieldInfos,
            true);

        // Check if enumerator points to a valid instance
        CND_CONDITION(origEnum != NULL,
                      "No memory could be allocated for orig enumerator");
        CND_CONDITION(indexEnum != NULL,
                      "No memory could be allocated for index enumerator");
...
TermInfosReader::~TermInfosReader() {
    ...
    // closed and the arrays indexTerms, indexPointers and indexInfos and  their
...
void TermInfosReader::ensureIndexIsRead() {
    ...
    try {
        ...
        // Instantiate an big block of TermInfo's, so that each one doesn't have
        // to be new'd
        ...
        // Instantiate an array indexPointers that contains pointers to the term
        // info index file
        indexPointers = _CL_NEWARRAY(int64_t, indexTermsLength);
        CND_CONDITION(
            indexPointers != NULL,
            "No memory could be allocated for indexPointers"); // Check if is
                                                               // indexPointers
                                                               // is a valid
                                                               // array
    ...
    _CLFINALLY(indexEnum->close();
               ...
               // destructor.
               _CLDELETE(indexEnum->input); _CLDELETE(indexEnum););
}
...
int32_t TermInfosReader::getIndexOffset(const Term* term) {
    ...
    // or equal to term. Pre  - term holds a reference to a valid term
    //       indexTerms != NULL
...
void TermInfosReader::seekEnum(const int32_t indexOffset) {
    ...
    CND_PRECONDITION(indexOffset >= 0,
                     "indexOffset contains a negative number");
    CND_PRECONDITION(indexTerms != NULL, "indexTerms is NULL");
