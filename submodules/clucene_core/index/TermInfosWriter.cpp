...
TermInfosWriter::TermInfosWriter(Directory* directory, const char* segment,
    ...
    other =
        _CLNEW TermInfosWriter(directory, segment, fieldInfos, interval, true);
...
void TermInfosWriter::initialise(Directory* directory, const char* segment,
    ...
    output = directory->createOutput(
        Misc::segmentname(segment, (isIndex ? ".tii" : ".tis")).c_str());
...
void TermInfosWriter::add(Term* term, TermInfo* ti) {
    ...
    add(fieldInfos->fieldNumber(term->field()), termTextBuffer.values, length,
        ti);
}

// Currently used only by assert statement
int32_t TermInfosWriter::compareToLastTerm(int32_t fieldNumber,
                                           const TCHAR* termText,
                                           int32_t length) {
    int32_t pos = 0;

    if (lastFieldNumber != fieldNumber) {
        const int32_t cmp = _tcscmp(fieldInfos->fieldName(lastFieldNumber),
...
void TermInfosWriter::add(int32_t fieldNumber, const TCHAR* termText,
                          int32_t termTextLength, const TermInfo* ti) {
    // Func - Writes a Term and TermInfo to the outputstream
    // Pre  - Term must be lexicographically greater than all previous Terms
    // added.
    //       Pointers of TermInfo ti (freqPointer and proxPointer) must be
    //       positive and greater than all previous.

// TODO: This is a hack. If _ASCII is defined, Misc::toString(const TCHAR*, int)
// will cause linking errors,
//       at least on VS. Needs a prettier fix no doubt... ISH 2009-11-08
...
#else
    CND_PRECONDITION(
        compareToLastTerm(fieldNumber, termText, termTextLength) < 0 ||
            (isIndex && termTextLength == 0 && lastTermTextLength == 0),
        (string("Terms are out of order: field=") +
         Misc::toString(fieldInfos->fieldName(fieldNumber)) + " (number " +
         Misc::toString(fieldNumber) + ")" + " lastField=" +
         Misc::toString(fieldInfos->fieldName(lastFieldNumber)) + " (number " +
         Misc::toString(lastFieldNumber) + ")" +
    ...
    CND_PRECONDITION(
        ...
        ("freqPointer out of order (" + Misc::toString(ti->freqPointer) +
         " < " + Misc::toString(lastTi->freqPointer) + ")")
            .c_str());
    CND_PRECONDITION(
        ti->proxPointer >= lastTi->proxPointer,
        ("proxPointer out of order (" + Misc::toString(ti->proxPointer) +
         " < " + Misc::toString(lastTi->proxPointer) + ")")
            .c_str());
...
void TermInfosWriter::writeTerm(int32_t fieldNumber, const TCHAR* termText,
    ...
    const int32_t limit = termTextLength < lastTermTextLength
                              ...
                              : lastTermTextLength;
    while (start < limit) {
        if (termText[start] != lastTermText.values[start]) break;
        start++;
    }
