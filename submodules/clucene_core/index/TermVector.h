 ...
 *  a document field.  The vector consists of the name of the field, an array of
 * the terms tha occur in the field of the
 * {@link org.apache.lucene.document.Document} and a parallel array of
 * frequencies.  Thus, getTermFrequencies()[5] corresponds with the frequency of
 * getTerms()[5], assuming there are at least 5 terms in the Document.
 */
...
public:
    ...
    /** Just like <code>indexOf(int32_t)</code> but searches for a number of
     * terms at the same time. Returns an array that has the same size as the
     * number of terms searched for, each slot containing the result of
     * searching for that term number.
     *
     *  @param terms array containing terms to look for
     *  @param start index in the array where the list of terms starts
     *  @param len the number of terms in the list
     */
    virtual CL_NS(util)::ArrayBase<int32_t>* indexesOf(
        const CL_NS(util)::ArrayBase<TCHAR*>& terms, const int32_t start,
        const int32_t len) = 0;

    /** Solve the diamond inheritence problem by providing a reinterpret
     *function. No dynamic casting is required and no RTTI data is needed to do
     *this
     */
 ...
 * {@link TermPositionVector}'s offset information.  This offset information is
 * the character offset as set during the Analysis phase (and thus may not be
 * the actual offset in the original content).
 */
...
public: // TODO: Remove after TermVectorWriter has been ported;
     ...
     * @return true if both {@link #getStartOffset()} and {@link
     * #getEndOffset()} are the same for both objects.
     */
...
/**
 * Convenience declaration when creating a {@link
 * org.apache.lucene.index.TermPositionVector} that stores only position
 * information.
 */
extern CL_NS(util)::ObjectArray<
    TermVectorOffsetInfo>* TermVectorOffsetInfo_EMPTY_OFFSET_INFO;
...
public:
     ...
     */
    virtual const CL_NS(util)::ArrayBase<int32_t>* getTermPositions(
     ...
     */
    virtual const CL_NS(util)::ArrayBase<TermVectorOffsetInfo*>* getOffsets(
        const size_t index) = 0;
