...
#include "clucene_core/shared/util/StringBuffer.h"
...
TermVectorsReader::TermVectorsReader(CL_NS(store)::Directory* d,
                                     ...
                                     int32_t docStoreOffset, int32_t size)
    : fieldInfos(NULL),
      tvx(NULL),
      tvd(NULL),
      tvf(NULL),
    ...
    try {
        if (d->fileExists(fbuf)) {
            ...
            if (-1 == docStoreOffset) {
                ...
                // docs
                CND_CONDITION(
                    ((int64_t)(tvx->length() / 8)) >= size + docStoreOffset,
                    "file is not long enough to hold all of our docs");
            }
        }
...
TermVectorsReader* TermVectorsReader::clone() const {
    if (tvx == NULL || tvd == NULL || tvf == NULL) return NULL;
    return _CLNEW TermVectorsReader(*this);
}

TermVectorsReader::~TermVectorsReader() { close(); }
...
void TermVectorsReader::close() {
    ...
    if (thrown) throw keep;
}

int64_t TermVectorsReader::size() const { return _size; }

void TermVectorsReader::get(const int32_t docNum, const TCHAR* field,
    ...
    if (tvx != NULL) {
        ...
        if (found != -1) {
            ...
            position = 0;
            for (int32_t i = 0; i <= found;
                 i++) // TODO: Was ++i, make sure its still good
...
TermFreqVector* TermVectorsReader::get(const int32_t docNum,
                                       const TCHAR* field) {
    // Check if no term vectors are available for this segment at all
    ParallelArrayTermVectorMapper* mapper =
        _CLNEW ParallelArrayTermVectorMapper();
...
ArrayBase<TermFreqVector*>* TermVectorsReader::get(const int32_t docNum) {
    ...
    if (tvx != NULL) {
        ...
        if (fieldCount != 0) {
            ...
            result = (ObjectArray<TermFreqVector>*)readTermVectors(
                docNum, fields, tvfPointers, fieldCount);
...
ObjectArray<SegmentTermVector>* TermVectorsReader::readTermVectors(
    const int32_t docNum, const TCHAR** fields, const int64_t* tvfPointers,
    const int32_t len) {
    ObjectArray<SegmentTermVector>* res =
        _CLNEW CL_NS(util)::ObjectArray<SegmentTermVector>(len);
    ParallelArrayTermVectorMapper* mapper =
        _CLNEW ParallelArrayTermVectorMapper();
    for (int32_t i = 0; i < len; i++) {
        ...
        res->values[i] =
            static_cast<SegmentTermVector*>(mapper->materializeVector());
        mapper->reset();
    }
    _CLLDELETE(mapper);
    return res;
...
void TermVectorsReader::readTermVector(const TCHAR* field,
    ...
    ValueArray<TCHAR> buffer(
        10); // init the buffer with a length of 10 character
    ...
    for (int32_t i = 0; i < numTerms; ++i) {
        ...
        if (storePositions) { // read in the positions
            ...
            if (mapper->isIgnoringPositions() == false) {
                ...
                // we need to skip over the positions.  Since these are VInts, I
                // don't believe there is anyway to know for sure how far to
                // skip
                //
                for (int32_t j = 0; j < freq; j++) {
                    tvf->readVInt();
        ...
        if (storeOffsets) {
            ...
            if (mapper->isIgnoringOffsets() == false) {
                ...
                for (int32_t j = 0; j < freq; j++) {
                    ...
                    offsets->values[j] =
                        _CLNEW TermVectorOffsetInfo(startOffset, endOffset);
...
ObjectArray<TermVectorOffsetInfo>* TermVectorOffsetInfo_EMPTY_OFFSET_INFO =
    _CLNEW ObjectArray<TermVectorOffsetInfo>;
...
TermVectorOffsetInfo::TermVectorOffsetInfo(int32_t startOffset,
                                           int32_t endOffset) {
    this->endOffset = endOffset;
    this->startOffset = startOffset;
}

int32_t TermVectorOffsetInfo::getEndOffset() const { return endOffset; }

void TermVectorOffsetInfo::setEndOffset(const int32_t _endOffset) {
    this->endOffset = _endOffset;
}

int32_t TermVectorOffsetInfo::getStartOffset() const { return startOffset; }
...
bool TermVectorOffsetInfo::equals(TermVectorOffsetInfo* termVectorOffsetInfo) {
    if (this == termVectorOffsetInfo) return true;
...
TermVectorMapper::TermVectorMapper(const bool _ignoringPositions,
                                   const bool _ignoringOffsets) {
    this->ignoringPositions = _ignoringPositions;
    this->ignoringOffsets = _ignoringOffsets;
}

bool TermVectorMapper::isIgnoringPositions() const { return ignoringPositions; }

bool TermVectorMapper::isIgnoringOffsets() const { return ignoringOffsets; }
...
void ParallelArrayTermVectorMapper::setExpectations(const TCHAR* _field,
                                                    const int32_t numTerms,
    ...
    if (storePositions) {
        positions = (ArrayBase<ArrayBase<int32_t>*>*)
            _CLNEW ObjectArray<ValueArray<int32_t>>(numTerms);
    }
    if (storeOffsets) {
        offsets =
            _CLNEW ObjectArray<ArrayBase<TermVectorOffsetInfo*>>(numTerms);
    }
}

void ParallelArrayTermVectorMapper::map(
    const TCHAR* term, int32_t termLen, const int32_t frequency,
...
TermFreqVector* ParallelArrayTermVectorMapper::materializeVector() {
    ...
    if (field != NULL && terms != NULL) {
        if (storingPositions || storingOffsets) {
            tv = _CLNEW SegmentTermPositionVector(field, terms, termFreqs,
                                                  positions, offsets);
        } else {
