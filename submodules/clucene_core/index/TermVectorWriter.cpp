    ...
    if (_vectors != NULL) {
        ...
        for (int32_t i = 0; i < numFields; i++) {
            ...
            const int32_t fieldNumber =
                fieldInfos->fieldNumber(vectors[i]->getField());
            ...
            if (vectors[i]->__asTermPositionVector() != NULL) {
                ...
                storeOffsets =
                    tpVector->size() > 0 && tpVector->getOffsets(0) != NULL;
                bits =
                    ((storePositions
                          ? TermVectorsReader::STORE_POSITIONS_WITH_TERMVECTOR
                          : 0) +
                     (storeOffsets
                          ? TermVectorsReader::STORE_OFFSET_WITH_TERMVECTOR
                          : 0));
            } else {
                tpVector = NULL;
                bits = 0;
            ...
            for (int32_t j = 0; j < numTerms; j++) {
                ...
                int32_t start = Misc::stringDifference(
                    lastTermText, lastTermTextLen, termText, termTextLen);
                ...
                if (storePositions) {
                    const ArrayBase<int32_t>* _positions =
                ...
                if (storeOffsets) {
                    const ArrayBase<TermVectorOffsetInfo*>* _offsets =
