...
private:
    ...
    typedef CL_NS(util)::CLHashMap<
        char*, ReaderFileEntry*, CL_NS(util)::Compare::Char,
...
public:
    CompoundFileReader(CL_NS(store)::Directory* dir, const char* name,
    ...
    void close();
    bool openInput(const char* name, CL_NS(store)::IndexInput*& ret,
                   CLuceneError& error, int32_t bufferSize = 0);
 ...
 * entry contains an encoding identifier, an int64_t pointer to the start of
...
public:
    ...
    CompoundFileWriter(CL_NS(store)::Directory* dir, const char* name,
                       SegmentMerger::CheckAbort* checkAbort = NULL);
    ~CompoundFileWriter();
