...
public:
    ...
    typedef CL_NS(
        util)::CLHashMap<Term*, Num*, Term_Compare, Term_Equals,
                         CL_NS(util)::Deletor::Object<Term>,
                         CL_NS(util)::Deletor::Object<Num>> TermNumMapType;
...
private:
    ...
    int32_t
        docStoreOffset; // Current starting doc-store offset of current segment
    ...
    struct Posting {
        int32_t
            textStart;   // Address into char[] blocks where our text is stored
    ...
    typedef CL_NS(util)::CLArrayList<
        TCHAR*, CL_NS(util)::Deletor::tcArray> FreeCharBlocksType;
    ...
    std::vector<std::string>*
        _abortedFiles; // List of files that were written before last abort()
    ...
    typedef CL_NS(util)::CLArrayList<
        uint8_t*, CL_NS(util)::Deletor::vArray<uint8_t>> FreeByteBlocksType;
    ...
    public:
        ...
        public:
            ...
            FieldData(DocumentsWriter* _parent, ThreadState* __threadState,
                      FieldInfo* fieldInfo);
            ~FieldData();
            ...
            void invertField(CL_NS(document)::Field* field,
    ...
    private:
        CL_NS(util)::ValueArray<Posting*> postingsFreeListTS; // Free Posting
                                                              // instances
        ...
        CL_NS(util)::ValueArray<FieldData*> fieldDataArray; // Fields touched by
                                                            // current doc
        ...
        void doVectorSort(CL_NS(util)::ArrayBase<PostingVector*>& postings,
                          int32_t numPosting);

        void quickSort(CL_NS(util)::ArrayBase<PostingVector*>& postings,
    ...
    public:
        ...
        CL_NS(util)::ValueArray<FieldData*> allFieldDataArray; // All FieldData
                                                               // instances
        bool doFlushAfter;
    ...
    public:
        ...
        virtual ~BlockPool() { buffers.deleteValues(); }
        ...
        void nextBuffer() {
            ...
            buffer = buffers.values[1 + bufferUpto] =
                getNewBlock(trackAllocations);
            bufferUpto++;
    ...
    CL_NS(util)::CLHashMap<
        _LUCENE_THREADID_TYPE, ThreadState*,
...
public:
     ...
     * flush. */
    bool addDocument(CL_NS(document)::Document* doc,
                     CL_NS(analysis)::Analyzer* analyzer);

    bool updateDocument(Term* t, CL_NS(document)::Document* doc,
                        CL_NS(analysis)::Analyzer* analyzer);

    bool updateDocument(CL_NS(document)::Document* doc,
                        CL_NS(analysis)::Analyzer* analyzer, Term* delTerm);
    ...
    static void fillBytes(CL_NS(store)::IndexOutput* out, uint8_t b,
                          int32_t numBytes);

    uint8_t* copyByteBuffer;

    /** Copy numBytes from srcIn to destIn */
    void copyBytes(CL_NS(store)::IndexInput* srcIn,
                   CL_NS(store)::IndexOutput* destIn, int64_t numBytes);
    ...
    static const int32_t
        POSTING_NUM_BYTE; /// = OBJECT_HEADER_BYTES + 9*INT_NUM_BYTE +
                          /// 5*POINTER_NUM_BYTE;

    /* Allocate more Postings from shared pool */
    void getPostings(CL_NS(util)::ValueArray<Posting*>& postings);
    void recyclePostings(CL_NS(util)::ValueArray<Posting*>& postings,
                         int32_t numPostings);
    ...
    void recycleBlocks(CL_NS(util)::ArrayBase<uint8_t*>& blocks, int32_t start,
                       int32_t end);
    ...
    void recycleBlocks(CL_NS(util)::ArrayBase<TCHAR*>& blocks, int32_t start,
                       int32_t numBlocks);
