...
private:
    ...
    static void compress(const CL_NS(util)::ValueArray<uint8_t>& input,
...
public:
    ...
    FieldsWriter(CL_NS(store)::Directory* d, const char* segment,
                 FieldInfos* fn);
    FieldsWriter(CL_NS(store)::IndexOutput* fdx, CL_NS(store)::IndexOutput* fdt,
                 FieldInfos* fn);
    ~FieldsWriter();
    ...
    void flushDocument(int32_t numStoredFields,
     ...
     *  bytes. */
    void addRawDocuments(CL_NS(store)::IndexInput* stream,
                         const int32_t* lengths, const int32_t numDocs);
