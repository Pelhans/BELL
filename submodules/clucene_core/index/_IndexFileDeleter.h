...
private:
     ...
     * equals.
     */
    class CommitPoint : public IndexCommitPoint,
                        public CL_NS(util)::Comparable {
        int64_t gen;
...
private:
    ...
    typedef CL_NS(util)::CLHashMap<
        char*, RefCount*, CL_NS(util)::Compare::Char, CL_NS(util)::Equals::Char,
    ...
    typedef CL_NS(util)::CLVector<
        IndexCommitPoint*,
        CL_NS(util)::Deletor::Object<IndexCommitPoint>> CommitsType;
...
public:
     ...
     */
    IndexFileDeleter(CL_NS(store)::Directory* directory,
                     IndexDeletionPolicy* policy, SegmentInfos* segmentInfos,
                     std::ostream* infoStream, DocumentsWriter* docWriter);
    ~IndexFileDeleter();
    ...
    void CLUCENE_LOCAL_DECL
    deleteNewFiles(const std::vector<std::string>& files);
