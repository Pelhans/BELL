...
protected:
    ...
    virtual void skipProx(const int64_t /*proxPointer*/,
...
class SegmentReader : public DirectoryIndexReader {
     ...
     * These normalizations are read from an IndexInput in into an array of
     * bytes called bytes
     */
    class Norm : LUCENE_BASE {
         ...
         * It is still valid to access all other norm properties after close is
         * called.
         * @throws IOException
         */
        void close();
    ...
    public:
        ...
        bool dirty;
        // Constructor
        Norm(CL_NS(store)::IndexInput* instrm, bool useSingleNormStream,
             int32_t number, int64_t normSeek, SegmentReader* reader,
             const char* segment);
        // Destructor
        ~Norm();
    ...
    typedef CL_NS(
        util)::CLHashtable<const TCHAR*, Norm*, CL_NS(util)::Compare::TChar,
                           CL_NS(util)::Equals::TChar,
                           CL_NS(util)::Deletor::Dummy, Norm> NormsType;
    NormsType _norms;
...
public:
    ...
    void SegmentName(char* buffer, int32_t bufferLen, const char* ext,
                     const int32_t x = -1);
     ...
     * @throws IOException
     */
    TermFreqVector* getTermFreqVector(int32_t docNumber,
                                      const TCHAR* field = NULL);

    void getTermFreqVector(int32_t docNumber, const TCHAR* field,
     ...
     * @throws IOException
     */
    CL_NS(util)::ArrayBase<TermFreqVector*>* getTermFreqVectors(
        int32_t docNumber);
...
private:
    ...
    // have a more tight idea of the package
