...
public:
    ...
    LUCENE_STATIC_CONSTANT(int32_t,
                           CHECK_DIR = 0); // e.g. must check dir to see if
                                           // there are norms/deletions
    LUCENE_STATIC_CONSTANT(
        int32_t, WITHOUT_GEN = 0); // a file name that has no GEN in it.
...
private:
    ...
    CL_NS(util)::ValueArray<
        int64_t> normGen; // current generation of each field's norm file.
                          ...
                          // norms; CHECK_DIR says it is a preLockLess segment
                          // and filesystem must be checked; >= YES says this
                          // field has separate norms with the specified
                          // generation
    ...
    bool hasSingleNormFile; // true if this segment maintains norms in a single
                            // file; false otherwise this is currently false for
                            // segments populated by DocumentWriter and true for
                            // newly created merged segments (both compound and
                            // non compound).

private:
    std::vector<std::string> _files; // cached list of files that this segment
                                     // uses in the Directory

    int64_t _sizeInBytes; // total byte size of all of our files (computed on
                          // demand)

    int32_t docStoreOffset; // if this segment shares stored fields & vectors,
                            // this offset is where in that file this segment's
                            // docs begin
    std::string
        docStoreSegment; // name used to derive fields/vectors file we share
                         // with other segments This string is being interned.
                         // There might be a way around this, and if found, this
                         // would greatly improve perfomance.

    bool docStoreIsCompoundFile; // whether doc store files are stored in
                                 // compound file (*.cfx)
    ...
    void addIfExists(std::vector<std::string>& files,
                     const std::string& fileName);

public:
    SegmentInfo(const char* _name, const int32_t _docCount,
                CL_NS(store)::Directory* _dir,
                bool _isCompoundFile = SegmentInfo::CHECK_DIR,
                bool _hasSingleNormFile = false, int32_t _docStoreOffset = -1,
     ...
     */
    SegmentInfo(CL_NS(store)::Directory* dir, int32_t format,
    ...
    CL_NS(store)::Directory* getDir() const {
        return dir;
    } // todo: since dir is public, consider removing this function
...
typedef CL_NS(util)::CLVector<
    SegmentInfo*, CL_NS(util)::Deletor::Object<SegmentInfo>> segmentInfosType;
...
public:
     ...
     * See <a
     * href="http://issues.apache.org/jira/browse/LUCENE-756">LUCENE-756</a> for
     * details.
     */
...
private:
    ...
    int64_t lastGeneration; // generation of the "segments_N" file we last
                            // successfully read or wrote; this is normally the
                            // same as generation except if there was an
                            // IOException that had interrupted a commit
...
public:
     ...
     */
    static int64_t getCurrentSegmentGeneration(
     ...
     * @param files -- array of file names to check
     */
    static std::string getCurrentSegmentFileName(
    ...
    // Writes a new segments file based upon the SegmentInfo instances it
    // manages note: still does not support lock-less writes (still pre-2.1
    // format)
    ...
    protected:
        ...
        void doRun();
        virtual bool tryDoBody(const char* segmentFileName,
                               CLuceneError& ret_err) = 0;
    };
