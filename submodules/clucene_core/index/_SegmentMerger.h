 ...
 *appropriate readers, call the merge method to combine the segments. <P> If the
 *compoundFile flag is set, then the segments will be merged into a compound
 *file.
 *
 *
 * @see #merge
...
public:
    ...
    SegmentMerger(IndexWriter* writer, std::string name,
                  MergePolicy::OneMerge* merge);
...
private:
     ...
     * <code>smis</code> contains segments that are positioned at the same term.
     * <code>N</code> is the number of cells in the array actually occupied.
     *
    ...
    void createCompoundFile(const char* filename,
