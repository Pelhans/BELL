...
private:
    ...
    int64_t lastChildPointer; // childPointer of last read skip entry with docId
                              // <= target
...
public:
     ...
     */
    MultiLevelSkipListReader(CL_NS(store)::IndexInput* _skipStream,
...
protected:
     ...
     */
    virtual int32_t readSkipData(const int32_t level,
                                 CL_NS(store)::IndexInput* skipStream) = 0;
...
protected:
    ...
    private:
        ...
        /* Make sure b is passed after the offset has been calculated into it,
         * if necessary! */
...
public:
    DefaultSkipListReader(CL_NS(store)::IndexInput* _skipStream,
                          const int32_t maxSkipLevels,
                          const int32_t _skipInterval);
     ...
     * the doc to which the last call of {@link
...
protected:
    ...
    int32_t readSkipData(const int32_t level,
                         CL_NS(store)::IndexInput* _skipStream);
};
