...
public:
    ...
    /** Close all streams.
     * to suppress exceptions from being thrown, pass an error object to be
     * filled in
     */
...
private:
    ...
    int32_t binarySearch(const CL_NS(util)::ArrayBase<TCHAR*>& array,
...
public:
    ...
    CL_NS(util)::ArrayBase<int32_t>* indexesOf(
        const CL_NS(util)::ArrayBase<TCHAR*>& termNumbers, const int32_t start,
        const int32_t len);
...
public:
    TermVectorsReader(CL_NS(store)::Directory* d, const char* segment,
                      FieldInfos* fieldInfos,
                      int32_t readBufferSize = LUCENE_STREAM_BUFFER_SIZE,
                      int32_t docStoreOffset = -1, int32_t size = 0);
    ~TermVectorsReader();
...
public:
    void get(const int32_t docNum, const TCHAR* field,
    ...
    // bool get(int32_t docNum, CL_NS(util)::ObjectArray<TermFreqVector*>&
    // result);

    void get(const int32_t docNumber, TermVectorMapper* mapper);

private:
    CL_NS(util)::ObjectArray<SegmentTermVector>* readTermVectors(
     ...
     * @param tvfPointer The pointer within the tvf file where we should start
     * reading
     * @param mapper The mapper used to map the TermVector
     * @return The TermVector located at that position
     * @throws IOException
     */
    void readTermVector(const TCHAR* field, const int64_t tvfPointer,
                        TermVectorMapper* mapper);
...
public:
    SegmentTermPositionVector(
        const TCHAR* field, CL_NS(util)::ArrayBase<TCHAR*>* terms,
        CL_NS(util)::ArrayBase<int32_t>* termFreqs,
        CL_NS(util)::ArrayBase<CL_NS(util)::ArrayBase<int32_t>*>* _positions,
        CL_NS(util)::ArrayBase<CL_NS(util)::ArrayBase<TermVectorOffsetInfo*>*>*
            _offsets);
    ~SegmentTermPositionVector();
     ...
     */
    const CL_NS(util)::ArrayBase<TermVectorOffsetInfo*>* getOffsets(
        const size_t index);
...
protected:
    ...
    /**
     *
     * @param ignoringPositions true if this mapper should tell Lucene to ignore
     * positions even if they are stored
     * @param ignoringOffsets similar to ignoringPositions
     */
    TermVectorMapper(const bool _ignoringPositions,
...
public:
     ...
     *
     * This method will be called before {@link
     ...
     */
    virtual void setExpectations(const TCHAR* _field, const int32_t numTerms,
     ...
     * @param offsets null if the offset is not specified, otherwise the offset
     * into the field of the term
     * @param positions null if the position is not specified, otherwise the
     * position in the field of the term
     * @memory offset and position objects must be cleaned up by implementing
     * class
     */
    virtual void map(const TCHAR* term, const int32_t termLen,
    ...
    /**
     *
     * @see #isIgnoringPositions() Same principal as {@link
     * #isIgnoringPositions()}, but applied to offsets.  false by default.
     * @return false
     */
     ...
     *internal to Lucene and subject to change depending on indexing operations.
     *
