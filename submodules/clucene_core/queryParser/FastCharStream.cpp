...
void FastCharStream::refill() {
    ...
    if (charsRead == -1) {
        ...
        memcpy(buffer, charBuf,
               charsRead * sizeof(TCHAR)); // TODO: Can we use the reader buffer
                                           // instead of copying to our own?
        bufferLength += charsRead;
    }
}

void FastCharStream::backup(const int32_t amount) { bufferPosition -= amount; }
...
int32_t FastCharStream::getLine() const { return 1; }
int32_t FastCharStream::getEndColumn() const {
    return bufferStart + bufferPosition;
}
int32_t FastCharStream::getEndLine() const { return 1; }
int32_t FastCharStream::getBeginColumn() const {
    return bufferStart + tokenStart;
}
int32_t FastCharStream::getBeginLine() const { return 1; }
