...
MultiFieldQueryParser::MultiFieldQueryParser(const TCHAR** _fields,
...
Query* MultiFieldQueryParser::getFieldQuery(const TCHAR* field,
                                            TCHAR* queryText,
                                            const int32_t slop) {
    if (field == NULL) {
        ...
        for (int i = 0; fields[i] != NULL; ++i) {
            ...
            if (q != NULL) {
                ...
                if (boosts != NULL) {
                    ...
                    BoostMap::const_iterator itr =
                        boosts->find((TCHAR*)fields[i]);
                    if (itr != boosts->end()) {
                ...
                clauses.push_back(
                    _CLNEW BooleanClause(q, true, BooleanClause::SHOULD));
            }
        }
...
Query* MultiFieldQueryParser::getFuzzyQuery(const TCHAR* field, TCHAR* termStr,
                                            const float_t minSimilarity) {
    if (field == NULL) {
        vector<BooleanClause*> clauses;
        for (int i = 0; fields[i] != NULL; ++i) {
            Query* q =
                QueryParser::getFuzzyQuery(fields[i], termStr, minSimilarity);
            if (q)
                clauses.push_back(
                    _CLNEW BooleanClause(q, true, BooleanClause::SHOULD));
        }
...
Query* MultiFieldQueryParser::getPrefixQuery(const TCHAR* field,
                                             TCHAR* termStr) {
    if (field == NULL) {
        vector<BooleanClause*> clauses;
        for (int i = 0; fields[i] != NULL; ++i) {
...
Query* MultiFieldQueryParser::getWildcardQuery(const TCHAR* field,
                                               TCHAR* termStr) {
    if (field == NULL) {
        vector<BooleanClause*> clauses;
        for (int i = 0; fields[i] != NULL; ++i) {
...
Query* MultiFieldQueryParser::getRangeQuery(const TCHAR* field, TCHAR* part1,
                                            TCHAR* part2,
    ...
    if (field == NULL) {
        ...
        for (int i = 0; fields[i] != NULL; ++i) {
            Query* q =
                QueryParser::getRangeQuery(fields[i], part1, part2, inclusive);
            if (q)
                clauses.push_back(
                    _CLNEW BooleanClause(q, true, BooleanClause::SHOULD));
        }
...
// static
Query* MultiFieldQueryParser::parse(const TCHAR** _queries,
                                    const TCHAR** _fields, Analyzer* analyzer) {
    BooleanQuery* bQuery = _CLNEW BooleanQuery();
    for (size_t i = 0; _fields[i] != NULL; i++) {
        if (_queries[i] == NULL) {
            _CLLDELETE(bQuery);
            _CLTHROWA(CL_ERR_IllegalArgument,
                      "_queries.length != _fields.length");
        }
        ...
        if (q != NULL && // q never null, just being defensive
            (!(q->instanceOf(BooleanQuery::getClassName()) ||
...
// static
Query* MultiFieldQueryParser::parse(const TCHAR* query, const TCHAR** _fields,
                                    const uint8_t* flags, Analyzer* analyzer) {
    BooleanQuery* bQuery = _CLNEW BooleanQuery();
    for (size_t i = 0; _fields[i] != NULL; i++) {
        // TODO: this is really confusing... why not refactor _fields and flags
        // to use a array object. flags can be NULL since NULL == 0...
        /*if (flags[i] == NULL) {
            _CLLDELETE(bQuery);
...
// static
Query* MultiFieldQueryParser::parse(const TCHAR** _queries,
                                    const TCHAR** _fields, const uint8_t* flags,
                                    Analyzer* analyzer) {
    BooleanQuery* bQuery = _CLNEW BooleanQuery();
    for (size_t i = 0; _fields[i] != NULL; i++) {
        // TODO: this is really confusing... why not refactor _fields and flags
        ...
        if (_queries[i] == NULL) { //|| flags[i] == NULL
            _CLLDELETE(bQuery);
            _CLTHROWA(CL_ERR_IllegalArgument,
                      "_queries, _fields, and flags array have have different "
                      "length");
        }
        QueryParser* qp = _CLNEW QueryParser(_fields[i], analyzer);
        Query* q = qp->parse(_queries[i]);
