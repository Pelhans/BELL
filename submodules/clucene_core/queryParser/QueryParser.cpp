...
#include "../search/MatchAllDocsQuery.h"
#include "../search/MultiPhraseQuery.h"
#include "../search/ConstantScoreQuery.h"
...
const TCHAR* QueryParserConstants::tokenImage[] = {_T("<EOF>"),
                                                   _T("<_NUM_CHAR>"),
                                                   _T("<_ESCAPED_CHAR>"),
                                                   _T("<_TERM_START_CHAR>"),
                                                   _T("<_TERM_CHAR>"),
...
const int64_t QueryParser::jj_la1_0[] = {
    0x180,      0x180,     0xe00,      0xe00,   0x1f69f80,  0x48000,
    0x10000,    0x1f69000, 0x1348000,  0x80000, 0x80000,    0x10000,
    0x18000000, 0x2000000, 0x18000000, 0x10000, 0x80000000, 0x20000000,
    0x80000000, 0x10000,   0x80000,    0x10000, 0x1f68000};
const int64_t QueryParser::jj_la1_1[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                                         0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                                         0x1, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0};
...
QueryParser::QueryParser(const TCHAR* f, Analyzer* a)
    : _operator(OR_OPERATOR),
      lowercaseExpandedTerms(true),
      useOldRangeQuery(false),
      ...
      dateResolution(CL_NS(document)::DateTools::NO_RESOLUTION),
...
CL_NS(search)::Query* QueryParser::parse(const TCHAR* q, const TCHAR* f,
                                         CL_NS(analysis)::Analyzer* a) {
...
Query* QueryParser::parse(const TCHAR* _query) {
    ...
    try {
        ...
        if (e.number() == CL_ERR_Parse || e.number() == CL_ERR_TokenMgr) {
            ...
            const size_t errLen =
                _tcslen(_twhat) + _tcslen(_query) +
                20; // make sure we have enough room for our error message
            TCHAR* err = _CL_NEWARRAY(TCHAR, errLen);
            cl_stprintf(err, errLen, _T("Cannot parse '%s': %s"), _query,
                        _twhat);
            _CLTHROWT_DEL(CL_ERR_Parse, err);
        } else if (e.number() == CL_ERR_TooManyClauses) {
            const size_t errLen =
                _tcslen(_query) +
                25; // make sure we have enough room for our error message
            TCHAR* err = _CL_NEWARRAY(TCHAR, errLen);
            cl_stprintf(err, errLen,
                        _T("Cannot parse '%s': too many boolean clauses"),
                        _query);
            _CLTHROWT_DEL(CL_ERR_Parse, err);
        } else
            throw e;
...
Analyzer* QueryParser::getAnalyzer() const { return analyzer; }

const TCHAR* QueryParser::getField() const { return field; }

float_t QueryParser::getFuzzyMinSim() const { return fuzzyMinSim; }

void QueryParser::setFuzzyMinSim(const float_t _fuzzyMinSim) {
    fuzzyMinSim = _fuzzyMinSim;
}

int32_t QueryParser::getFuzzyPrefixLength() const { return fuzzyPrefixLength; }
...
int32_t QueryParser::getPhraseSlop() const { return phraseSlop; }
...
void QueryParser::setDefaultOperator(Operator _op) { _operator = _op; }
QueryParser::Operator QueryParser::getDefaultOperator() const {
    return _operator;
}
void QueryParser::setLowercaseExpandedTerms(
    const bool _lowercaseExpandedTerms) {
...
bool QueryParser::getUseOldRangeQuery() const { return useOldRangeQuery; }
void QueryParser::setDateResolution(
    const CL_NS(document)::DateTools::Resolution _dateResolution) {
...
CL_NS(document)::DateTools::Resolution QueryParser::getDateResolution(
    const TCHAR* fieldName) const {
    if (fieldName == NULL)
        _CLTHROWA(CL_ERR_IllegalArgument, "Field cannot be null.");
    ...
    CL_NS(document)::DateTools::Resolution resolution =
        fieldToDateResolution->get(fieldName);
...
void QueryParser::addClause(std::vector<BooleanClause*>& clauses, int32_t conj,
                            int32_t mods, Query* q) {
    ...
    if (nPreviousClauses > 0 && conj == CONJ_AND) {
        ...
        if (!c->isProhibited()) c->setOccur(BooleanClause::MUST);
    }
    ...
    if (nPreviousClauses > 0 && _operator == AND_OPERATOR && conj == CONJ_OR) {
        ...
        // OR b) notice if the input is a OR b, first term is parsed as
        // required; without this modification a OR b would parsed as +a OR b
        BooleanClause* c = clauses[nPreviousClauses - 1];
        if (!c->isProhibited()) c->setOccur(BooleanClause::SHOULD);
    }
    ...
    if (required && !prohibited)
        ...
        clauses.push_back(
            _CLNEW BooleanClause(q, true, BooleanClause::MUST_NOT));
    else {
...
Query* QueryParser::getFieldQuery(const TCHAR* _field, TCHAR* queryText) {
    ...
    CLVector<CL_NS(analysis)::Token*, Deletor::Object<CL_NS(analysis)::Token>>
        v;
    ...
    while (true) {
        ...
        _CLCATCH_ERR(CL_ERR_IO, _CLLDELETE(source); _CLLDELETE(t);
                     _CLDELETE_LCARRAY(queryText);, { t = NULL; });
        if (t == NULL) break;
        v.push_back(t);
...
Query* QueryParser::getRangeQuery(const TCHAR* _field, TCHAR* part1,
                                  TCHAR* part2, const bool inclusive) {
    ...
    try {
        ...
        // SHORT means completely numeric df.setLenient(true); Date d1 =
        df.parse(part1); Date d2 = df.parse(part2);
        */
        ...
        if (inclusive) {
            ...
            // really include all documents:
            d2 = CL_NS(document)::DateTools::timeMakeInclusive(d2);
        }

        CL_NS(document)::DateTools::Resolution resolution =
            getDateResolution(_field);
        if (resolution == CL_NS(document)::DateTools::NO_RESOLUTION) {
    ...
    if (useOldRangeQuery) {
        ...
        // Make sure to delete the date strings we allocated only if we indeed
        // allocated them
        ...
        Query* q = _CLNEW ConstantScoreRangeQuery(_field, part1, part2,
                                                  inclusive, inclusive);
...
Query* QueryParser::getBooleanQuery(
    std::vector<CL_NS(search)::BooleanClause*>& clauses, bool disableCoord) {
    if (clauses.size() == 0) {
...
Query* QueryParser::getWildcardQuery(const TCHAR* _field, TCHAR* termStr) {
    if (_tcscmp(_T("*"), _field) == 0) {
        if (_tcscmp(_T("*"), termStr) == 0) return _CLNEW MatchAllDocsQuery();
    }
    if (!allowLeadingWildcard &&
        (termStr[0] == _T('*') || termStr[0] == _T('?'))) {
        _CLDELETE_LCARRAY(termStr);
        _CLTHROWT(
            CL_ERR_Parse,
            _T("'*' or '?' not allowed as first character in WildcardQuery"));
    }
    if (lowercaseExpandedTerms) {
        _tcslwr(termStr);
    }
...
Query* QueryParser::getPrefixQuery(const TCHAR* _field, TCHAR* _termStr) {
    if (!allowLeadingWildcard && _termStr[0] == _T('*')) {
        ...
        _CLTHROWT(CL_ERR_Parse,
                  _T("'*' not allowed as first character in PrefixQuery"));
    }
    if (lowercaseExpandedTerms) {
        _tcslwr(_termStr);
    }
...
Query* QueryParser::getFuzzyQuery(const TCHAR* _field, TCHAR* termStr,
                                  const float_t minSimilarity) {
    if (lowercaseExpandedTerms) {
        _tcslwr(termStr);
    }
...
TCHAR* QueryParser::discardEscapeChar(TCHAR* input, TCHAR* output) {
    ...
    if (output == NULL) {
        // TODO: Perhaps we can re-use an inner buffer instead of creating new
        // char arrays here and in several other places
...
int32_t QueryParser::hexToInt(TCHAR c) {
    if (_T('0') <= c && c <= _T('9')) {
        ...
        TCHAR err[50];
        cl_stprintf(err, 50,
                    _T("Non-hex character in unicode escape sequence: %c"), c);
...
TCHAR* QueryParser::escape(const TCHAR* s) {
    ...
    // Create a StringBuffer object a bit longer from the length of the query
    // (to prevent some reallocations), and declare we are the owners of the
    // buffer (to save on a copy)
    // TODO: 1. Test to see what is the optimal initial length
    //     2. Allow re-using the provided string buffer (argument s) instead of
...
Query* QueryParser::fQuery(TCHAR* _field) {
    CLVector<CL_NS(search)::BooleanClause*,
             Deletor::Object<CL_NS(search)::BooleanClause>>
        clauses;
    ...
    while (true) {
        switch ((jj_ntk == -1) ? f_jj_ntk() : jj_ntk) {
            ...
            case RANGEEX_START:
            case NUMBER: break;
            default: jj_la1[4] = jj_gen; goto label_1_brk;
        }
    ...
    if (jj_2_1(2)) {
        switch ((jj_ntk == -1) ? f_jj_ntk() : jj_ntk) {
            case TERM: {
                ...
                // make sure to delete _field only if it's not contained already
                // by the QP
...
Query* QueryParser::fTerm(const TCHAR* _field) {
    ...
    switch ((jj_ntk == -1) ? f_jj_ntk() : jj_ntk) {
        ...
        case NUMBER: {
            switch ((jj_ntk == -1) ? f_jj_ntk() : jj_ntk) {
                case TERM: term = jj_consume_token(TERM); break;
                case STAR:
                ...
                case WILDTERM:
                    ...
                    wildcard = true;
                    break;
                case NUMBER: term = jj_consume_token(NUMBER); break;
                default:
                    jj_la1[8] = jj_gen;
                    jj_consume_token(-1);
                    _CLTHROWT(CL_ERR_Parse, _T(""));
            ...
            if (wildcard) {
                ...
                termImage[tiLen - 1] = 0;
                q = getPrefixQuery(_field,
                                   discardEscapeChar(termImage, termImage));
            } else if (fuzzy) {
                ...
                if (fms < 0.0f || fms > 1.0f) {
                    _CLTHROWT(CL_ERR_Parse,
                              _T("Minimum similarity for a FuzzyQuery has to ")
                              _T("be between 0.0f and 1.0f !"));
                }
                termImage = discardEscapeChar(term->image);
        ...
        case RANGEIN_START: {
            ...
            // TODO: Allow analysis::Term to accept ownership on a TCHAR* and
            // save on extra dup's
        ...
        case RANGEEX_START: {
            ...
            if (goop1->kind == RANGEEX_QUOTED) goop1->image = goop1->image + 1;

            if (goop2->kind == RANGEEX_QUOTED) goop2->image = goop2->image + 1;
        ...
        case QUOTED: {
            ...
            // TODO: Make sure this hack, save an extra dup, is legal and not
            // harmful
...
bool QueryParser::jj_2_1(const int32_t xla) {
    ...
    try {
        ...
        if (e.number() == CL_ERR_Parse &&
            _tcscmp(e.twhat(), _T("LookaheadSuccess")) == 0)
            return true;
        else
            throw e;
    }
...
QueryParser::QueryParser(CharStream* stream)
    : _operator(OR_OPERATOR),
      lowercaseExpandedTerms(true),
      useOldRangeQuery(false),
      allowLeadingWildcard(false),
...
bool QueryParser::jj_scan_token(const int32_t kind) {
    ...
    if (jj_la == 0 && jj_scanpos == jj_lastpos)
        _CLTHROWT(CL_ERR_Parse, _T("LookaheadSuccess"));
    return false;
}
...
QueryParser::QueryParser(QueryParserTokenManager* tm)
    : _operator(OR_OPERATOR),
      lowercaseExpandedTerms(true),
      useOldRangeQuery(false),
      allowLeadingWildcard(false),
...
void QueryParser::jj_add_error_token(const int32_t kind, int32_t pos) {
    ...
    if (pos == jj_endpos + 1) {
        ...
        if (!jj_expentries)
            jj_expentries =
                _CLNEW CL_NS(util)::CLVector<CL_NS(util)::ValueArray<int32_t>*,
                                             CL_NS(util)::Deletor::Object<CL_NS(
                                                 util)::ValueArray<int32_t>>>();
...
void QueryParser::generateParseException() {
    ...
    if (!jj_expentries)
        jj_expentries = _CLNEW CL_NS(util)::CLVector<
            CL_NS(util)::ValueArray<int32_t>*,
...
TCHAR* QueryParserConstants::addEscapes(TCHAR* str) {
    ...
    for (size_t i = 0; i < len; i++) {
        switch (str[i]) {
            case 0: continue;
            case _T('\b'): retval.append(_T("\\b")); continue;
            case _T('\t'): retval.append(_T("\\t")); continue;
            case _T('\n'): retval.append(_T("\\n")); continue;
            case _T('\f'): retval.append(_T("\\f")); continue;
            case _T('\r'): retval.append(_T("\\r")); continue;
            case _T('"'): retval.append(_T("\\\"")); continue;
            case _T('\''): retval.append(_T("\\'")); continue;
            case _T('\\'): retval.append(_T("\\\\")); continue;
            default:
...
TCHAR* QueryParser::getParseExceptionMessage(
    ...
    for (size_t i = 0; i < expectedTokenSequences->size(); i++) {
        ...
        for (size_t j = 0; j < expectedTokenSequences->at(i)->length; j++) {
            expected.append(
                tokenImage[expectedTokenSequences->at(i)->values[j]]);
            expected.appendChar(_T(' '));
        }
        if (expectedTokenSequences->at(i)
                ->values[expectedTokenSequences->at(i)->length - 1] != 0) {
            expected.append(_T("..."));
        }
        expected.append(eol);
        expected.append(_T("    "));
