...
#include "../../util/_FastCharStream.h"
...
Lexer::Lexer(QueryParserBase* queryparser, const TCHAR* query) {
    ...
    CND_CONDITION(reader != NULL,
                  "Could not allocate memory for FastCharStream reader");
...
void Lexer::Lex(TokenList* tokenList) {
    ...
    // Pre  - tokens != NULL and contains a TokenList in which the tokens can be
    // stored Post - The tokens have been added to the TokenList tokens
...
bool Lexer::GetNextToken(QueryToken* token) {
    while (!reader->Eos()) {
        ...
        TCHAR buf[2] = {ch, '\0'};
        switch (ch) {
            case '+': token->set(buf, QueryToken::PLUS); return true;
            case '-': token->set(buf, QueryToken::MINUS); return true;
            case '(': token->set(buf, QueryToken::LPAREN); return true;
            case ')': token->set(buf, QueryToken::RPAREN); return true;
            case ':': token->set(buf, QueryToken::COLON); return true;
            case '!': token->set(buf, QueryToken::NOT); return true;
            case '^': token->set(buf, QueryToken::CARAT); return true;
            case '~':
                ...
                break;
            case '"': return ReadQuoted(ch, token);
            case '[': return ReadInclusiveRange(ch, token);
            case '{': return ReadExclusiveRange(ch, token);
            case ']':
            case '}':
            case '*':
                queryparser->throwParserException(
                    _T("Unrecognized char %d at %d::%d."), ch, reader->Column(),
                    reader->Line());
                return false;
            default:
...
bool Lexer::ReadInclusiveRange(const TCHAR prev, QueryToken* token) {
    ...
    queryparser->throwParserException(
        _T("Unterminated inclusive range! %d %d::%d"), ' ', reader->Column(),
        reader->Column());
    return false;
}
...
bool Lexer::ReadExclusiveRange(const TCHAR prev, QueryToken* token) {
    ...
    queryparser->throwParserException(
        _T("Unterminated exclusive range! %d %d::%d"), ' ', reader->Column(),
        reader->Column());
    return false;
}
...
bool Lexer::ReadTerm(const TCHAR prev, QueryToken* token) {
    ...
    if (hasQuestion)
        ...
        token->set(val.getBuffer(), QueryToken::WILDTERM);
    else if (_tcsicmp(val.getBuffer(), _T("AND")) == 0 ||
             _tcscmp(val.getBuffer(), _T("&&")) == 0)
        token->set(val.getBuffer(), QueryToken::AND_);
    else if (_tcsicmp(val.getBuffer(), _T("OR")) == 0 ||
             _tcscmp(val.getBuffer(), _T("||")) == 0)
...
bool Lexer::ReadEscape(TCHAR prev, TCHAR* buf) {
    ...
    queryparser->throwParserException(
        _T("Unrecognized escape sequence at %d %d::%d"), ' ', reader->Column(),
        reader->Line());
    return false;
}
