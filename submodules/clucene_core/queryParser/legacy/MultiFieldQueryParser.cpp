...
Query* MultiFieldQueryParser::parse(const TCHAR* query, const TCHAR** fields,
    ...
    while (fields[i] != NULL) {
        ...
        if (q && (!q->instanceOf(BooleanQuery::getClassName()) ||
        ...
        if (q && (!q->instanceOf(BooleanQuery::getClassName()) ||
            ...
            switch (flag) {
                ...
                case MultiFieldQueryParser::PROHIBITED_FIELD:
                    ...
                    break;
                default: bQuery->add(q, true, false, false); break;
            }
        } else {
            _CLDELETE(q);
        }
    ...
    if (field == NULL) {
        ...
        for (int i = 0; fields[i] != NULL; ++i) {
            ...
            if (q != NULL) {
                ...
                if (q)
                    clauses.push_back(
                        _CLNEW BooleanClause(q, true, false, false));
            }
        }
        ...
        if (q) q = QueryAddedCallback(field, q);
        return q;
    }
}
...
CL_NS(search)::Query* MultiFieldQueryParser::GetFuzzyQuery(const TCHAR* field,
                                                           TCHAR* termStr) {
    if (field == NULL) {
        vector<BooleanClause*> clauses;
        for (int i = 0; fields[i] != NULL; ++i) {
            Query* q = QueryParser::GetFuzzyQuery(
                fields[i], termStr); // todo: , minSimilarity
            if (q) {
                q = QueryAddedCallback(fields[i], q);
                if (q) {
                    clauses.push_back(
        ...
        Query* q =
            QueryParser::GetFuzzyQuery(field, termStr); // todo: , minSimilarity
        if (q) q = QueryAddedCallback(field, q);
        return q;
    }
}
