...
typedef CL_NS(
    util)::CLHashMap<const TCHAR*, float_t, CL_NS(util)::Compare::TChar,
                     CL_NS(util)::Equals::TChar, CL_NS(util)::Deletor::Dummy,
                     CL_NS(util)::Deletor::DummyFloat> BoostMap;
...
public:
     ...
     * </code>
     *
     * <p>In other words, all the query's terms must appear, but it doesn't
     * matter in what fields they appear.</p>
     */
     ...
     */
    static CL_NS(search)::Query* parse(const TCHAR* query, const TCHAR** fields,
...
protected:
    ...
    CL_NS(search)::Query* GetRangeQuery(const TCHAR* field, TCHAR* part1,
    ...
    /**
     * A special virtual function for the MultiFieldQueryParser which can be
     * used to clean up queries. Once the field name is known and the query has
     * been created, its passed to this function. An example of this usage is to
     * set boosts.
     */
    virtual CL_NS(search)::Query* QueryAddedCallback(
        const TCHAR* field, CL_NS(search)::Query* query) {
        return query;
    }
};
CL_NS_END2
