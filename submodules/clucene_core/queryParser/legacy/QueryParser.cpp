...
QueryParser::QueryParser(const TCHAR* _field, Analyzer* _analyzer)
    : QueryParserBase(_analyzer) {
    // Func - Constructor.
...
// static
Query* QueryParser::parse(const TCHAR* query, const TCHAR* field,
                          Analyzer* analyzer) {
    // Func - Returns a new instance of the Query class with a specified query,
    // field and
    //       analyzer values.
...
Query* QueryParser::parse(const TCHAR* query) {
    ...
    try {
        // Parse the query managed by the StringReader R and return a parsed
        // Query instance into ret
        ret = parse(r);
    }
    _CLFINALLY(_CLDELETE(r););
...
Query* QueryParser::parse(BufferedReader* reader) {
    // Func - Returns a parsed Query instance
    // Pre  - reader contains a valid reference to a Reader and manages the
    // query string Post - A parsed Query instance has been returned or
...
int32_t QueryParser::MatchConjunction() {
    ...
    //       the token is extracted and deleted and CONJ_AND or CONJ_OR is
    //       returned otherwise CONJ_NONE is returned
...
int32_t QueryParser::MatchModifier() {
    ...
    // Pre  - tokens != NULL
    // Post - if the first token is a PLUS the token is extracted and deleted
    // and MOD_REQ is returned
    //       if the first token is a MINUS or NOT the token is extracted and
    //       deleted and MOD_NOT is returned otherwise MOD_NONE is returned
    CND_PRECONDITION(tokens != NULL, "tokens is NULL");
...
Query* QueryParser::MatchQuery(const TCHAR* field) {
    ...
    //       QUERY ::= [MODIFIER] QueryParser::CLAUSE (<CONJUNCTION> [MODIFIER]
    //       CLAUSE)*
    // Pre  - field != NULL
    // Post -
    ...
    while (true) {
        ...
        q = MatchClause(field);
        if (q != NULL) AddClause(clauses, conj, mods, q);
    }
...
Query* QueryParser::MatchClause(const TCHAR* field) {
    ...
    //       CLAUSE ::= [TERM <COLONQueryParser::>] ( TERM | (<LPAREN> QUERY
    //       <RPAREN>))
    // Pre  - field != NULL
    // Post -
    ...
    if (term->Type == QueryToken::TERM &&
        tokens->peek()->Type == QueryToken::COLON) {
...
Query* QueryParser::MatchTerm(const TCHAR* field) {
    ...
    switch (term->Type) {
        ...
        case QueryToken::WILDTERM: { // start case
...
QueryToken* QueryParser::MatchQueryToken(QueryToken::Types expectedType) {
    ...
    if (expectedType != t->Type) {
        TCHAR buf[200];
        _sntprintf(buf, 200,
                   _T("Error: Unexpected QueryToken: %d, expected: %d"),
                   t->Type, expectedType);
        _CLDELETE(t);
