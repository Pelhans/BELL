...
void QueryParserBase::AddClause(std::vector<BooleanClause*>& clauses,
                                int32_t conj, int32_t mods, Query* q) {
    ...
    if (nPreviousClauses > 0 && defaultOperator == AND_OPERATOR &&
        conj == CONJ_OR) {
        ...
        // required; without this modification a OR b would parse as +a OR b
        BooleanClause* c = clauses[nPreviousClauses - 1];
        if (!c->prohibited) {
            c->required = false;
            c->prohibited = false;
    ...
    if (required && prohibited)
        throwParserException(
            _T("Clause cannot be both required and prohibited"), ' ', 0, 0);
    clauses.push_back(_CLNEW BooleanClause(q, true, required, prohibited));
}

void QueryParserBase::throwParserException(const TCHAR* message, TCHAR ch,
                                           int32_t col, int32_t line) {
    TCHAR msg[1024];
            ...
            if (severalTokensAtSamePosition) {
                if (positionCount == 1) {
                    ...
                    BooleanQuery* q = _CLNEW BooleanQuery(
                        true); // todo: disableCoord=true here, but not
                               // implemented in BooleanQuery
                    StringArray::iterator itr = v.begin();
                    while (itr != v.end()) {
                        Term* t = _CLNEW Term(field, *itr);
                        q->add(_CLNEW TermQuery(t), true, false,
                               false); // should occur...
                        _CLDECDELETE(t);
                        ++itr;
                    }
                    ...
                    _CLTHROWA(CL_ERR_UnsupportedOperation,
                              "MultiPhraseQuery NOT Implemented");
                }
            } else {
...
int QueryParserBase::getDefaultOperator() const { return defaultOperator; }

Query* QueryParserBase::ParseRangeQuery(const TCHAR* field, TCHAR* queryText,
                                        bool inclusive) {
    // todo: this must be fixed, [-1--5] (-1 to -5) should yield a result, but
    // won't parse properly because it uses an analyser, should split it up
    // differently...
    ...
    do {
        ...
        if (tret) {
            if (!from && _tcscmp(t.termBuffer(), _T("TO")) == 0) continue;
...
Query* QueryParserBase::GetPrefixQuery(const TCHAR* field, TCHAR* termStr) {
    // Pre  - field != NULL and field contains the name of the field that the
    // query will use
    //       termStr != NULL and is the  token to use for building term for the
    //       query (WITH or WITHOUT a trailing '*' character!)
    // Post - A PrefixQuery instance has been returned

    CND_PRECONDITION(field != NULL, "field is NULL");
    CND_PRECONDITION(termStr != NULL, "termStr is NULL");

    if (lowercaseExpandedTerms) _tcslwr(termStr);
...
Query* QueryParserBase::GetFuzzyQuery(const TCHAR* field, TCHAR* termStr) {
    // Func - Factory method for generating a query (similar to
    // getPrefixQuery}). Called when parser parses
...
Query* QueryParserBase::GetBooleanQuery(
    std::vector<CL_NS(search)::BooleanClause*>& clauses) {
...
CL_NS(search)::Query* QueryParserBase::GetRangeQuery(const TCHAR* field,
                                                     TCHAR* part1, TCHAR* part2,
                                                     bool inclusive) {
    ...
    // todo: should see if we can parse the strings as dates... currently we
    // leave that up to the end-developer...
