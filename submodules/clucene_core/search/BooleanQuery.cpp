...
class BooleanClause_Compare
    : public CL_NS_STD(
          binary_function)<const BooleanClause*, const BooleanClause*, bool> {
public:
    bool operator()(const BooleanClause* val1,
                    const BooleanClause* val2) const {
...
protected:
    ...
    CL_NS(
        util)::CLVector<Weight*, CL_NS(util)::Deletor::Object<Weight>> weights;
...
public:
    BooleanWeight(
        Searcher* searcher,
        CL_NS(util)::CLVector<BooleanClause*,
                              CL_NS(util)::Deletor::Object<BooleanClause>>*
            clauses,
        BooleanQuery* parentQuery);
...
const char* BooleanQuery::getObjectName() const { return getClassName(); }
const char* BooleanQuery::getClassName() { return "BooleanQuery"; }
...
size_t BooleanQuery::getMaxClauseCount() { return maxClauseCount; }
...
void BooleanQuery::add(Query* query, const bool deleteQuery,
    ...
    BooleanClause* bc =
        _CLNEW BooleanClause(query, deleteQuery, required, prohibited);
    try {
        add(bc);
    } catch (...) {
        _CLDELETE(bc);
...
int32_t BooleanQuery::getMinNrShouldMatch() { return minNrShouldMatch; }
bool BooleanQuery::getUseScorer14() { return getAllowDocsOutOfOrder(); }

bool BooleanQuery::allowDocsOutOfOrder = false;

void BooleanQuery::setUseScorer14(bool use14) { setAllowDocsOutOfOrder(use14); }

void BooleanQuery::setAllowDocsOutOfOrder(bool allow) {
    allowDocsOutOfOrder = allow;
}

bool BooleanQuery::getAllowDocsOutOfOrder() { return allowDocsOutOfOrder; }

size_t BooleanQuery::getClauseCount() const { return (int32_t)clauses->size(); }

TCHAR* BooleanQuery::toString(const TCHAR* field) const {
    StringBuffer buffer;
    bool needParens =
        (getBoost() != 1.0) /* TODO: || (getMinimumNumberShouldMatch()>0)*/;
    if (needParens) {
        buffer.append(_T("("));
    }
    ...
    for (uint32_t i = 0; i < clauses->size(); i++) {
        ...
        if (c->getQuery()->instanceOf(
                BooleanQuery::getClassName())) { // wrap sub-bools in parens
            buffer.append(_T("("));
        ...
        if (i != clauses->size() - 1) buffer.append(_T(" "));
    }
...
void BooleanQuery::getClauses(BooleanClause** ret) const {
    ...
    for (uint32_t i = 0; i < size; i++) ret[i] = (*clauses)[i];
}
Query* BooleanQuery::rewrite(IndexReader* reader) {
    if (clauses->size() == 1) { // optimize 1-clause queries
        ...
        if (!c->prohibited) {                              // just return clause
            ...
            if (query == c->getQuery()) query = query->clone();
    ...
    for (uint32_t i = 0; i < clauses->size(); i++) {
        ...
        if (query != c->getQuery()) { // clause rewrote: must clone
            ...
            clone->clauses->set(
                i, _CLNEW BooleanClause(query, true, c->getOccur()));
        }
    }
    if (clone != NULL) {
...
bool BooleanQuery::equals(Query* o) const {
    if (!(o->instanceOf(BooleanQuery::getClassName()))) return false;
    const BooleanQuery* other = (BooleanQuery*)o;

    bool ret = (this->getBoost() == other->getBoost());
    if (ret) {
        CLListEquals<BooleanClause, BooleanClause_Compare,
                     const BooleanQuery::ClausesType,
                     const BooleanQuery::ClausesType>
            comp;
...
BooleanWeight::~BooleanWeight() { this->weights.clear(); }
...
float_t BooleanWeight::sumOfSquaredWeights() {
    ...
    sum *= parentQuery->getBoost() *
           parentQuery->getBoost(); // boost each sub-weight
    return sum;
}
...
Scorer* BooleanWeight::scorer(IndexReader* reader) {
    BooleanScorer2* result =
        _CLNEW BooleanScorer2(similarity, parentQuery->minNrShouldMatch,
...
Explanation* BooleanWeight::explain(IndexReader* reader, int32_t doc) {
    ...
    for (size_t i = 0; i < weights.size(); i++) {
        ...
        if (e->isMatch()) {
            ...
            if (c->getOccur() == BooleanClause::SHOULD) shouldMatchCount++;
        } else if (c->isRequired()) {
            StringBuffer buf(100);
    ...
    if (fail) {
        ...
        sumExpl->setDescription(_T("Failure to meet condition(s) of ")
                                _T("required/prohibited clause(s)"));
        return sumExpl;
    ...
    if (coordFactor == 1.0f) // coord is no-op
        ...
        ComplexExplanation* result = _CLNEW ComplexExplanation(
            sumExpl->isMatch(), sum * coordFactor, _T("product of:"));
...
BooleanClause::BooleanClause(Query* q, const bool DeleteQuery, const bool req,
                             const bool p)
    : query(q),
      occur(SHOULD),
...
BooleanClause::~BooleanClause() {
    if (deleteQuery) _CLDELETE(query);
}
...
size_t BooleanClause::hashCode() const {
    return query->hashCode() ^ ((occur == MUST) ? 1 : 0) ^
           ((occur == MUST_NOT) ? 2 : 0);
}
