...
bool BooleanScorer::skipTo(int32_t /*target*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: BooleanScorer::skipTo");
}

Explanation* BooleanScorer::explain(int32_t /*doc*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: BooleanScorer::explain");
}
...
void BooleanScorer::add(Scorer* scorer, const bool required,
        ...
        while (current != NULL) {
            if ((current->bits & prohibitedMask) == 0 &&
                ...
                if (current->coord >= minNrShouldMatch) {
                    results->collect(
                        current->doc,
                        current->score * coordFactors[current->coord]);
                }
            }
...
BooleanScorer::SubScorer::SubScorer(Scorer* scr, const bool r, const bool p,
                                    HitCollector* c, SubScorer* nxt)
    : scorer(scr),
      required(r),
      prohibited(p),
      collector(c),
