...
public:
    ...
    void initDoc() { nrMatchers = 0; }

    float_t coordFactor() { return coordFactors[nrMatchers]; }
};
...
public:
    ...
    virtual ~SingleMatchScorer() { _CLDELETE(scorer); }
    ...
    int32_t doc() const { return scorer->doc(); }

    bool next() { return scorer->next(); }

    bool skipTo(int32_t docNr) { return scorer->skipTo(docNr); }

    virtual TCHAR* toString() { return scorer->toString(); }

    Explanation* explain(int32_t doc) { return scorer->explain(doc); }
};

/** A scorer that matches no document at all. */
class BooleanScorer2::NonMatchingScorer : public Scorer {
public:
    NonMatchingScorer() : Scorer(NULL) {}
    virtual ~NonMatchingScorer(){};

    int32_t doc() const {
        _CLTHROWA(CL_ERR_UnsupportedOperation,
                  "UnsupportedOperationException: "
                  "BooleanScorer2::NonMatchingScorer::doc");
        return 0;
    }
    ...
    float_t score() {
        _CLTHROWA(CL_ERR_UnsupportedOperation,
                  "UnsupportedOperationException: "
                  "BooleanScorer2::NonMatchingScorer::score");
        return 0.0;
    }
...
public:
    ...
    ReqOptSumScorer(Scorer* _reqScorer, Scorer* _optScorer)
        : Scorer(NULL),
          reqScorer(_reqScorer),
          optScorer(_optScorer),
     ...
     * score of the optional scorer when it also matches the current document.
     */
    float_t score() {
        ...
        if (firstTimeOptScorer) {
            ...
            return reqScore;
        } else if ((optScorer->doc() < curDoc) &&
                   (!optScorer->skipTo(curDoc))) {
            _CLDELETE(optScorer);
            return reqScore;
        }
    ...
    int32_t doc() const { return reqScorer->doc(); }

    bool next() { return reqScorer->next(); }

    bool skipTo(int32_t target) { return reqScorer->skipTo(target); }

    virtual TCHAR* toString() { return stringDuplicate(_T("ReqOptSumScorer")); }
 ...
 * subscorer. <br> This <code>Scorer</code> implements {@link
 * Scorer#skipTo(int)}, and it uses the skipTo() on the given scorers.
 */
...
public:
     ...
     */
    float_t score() { return reqScorer->score(); }

    virtual TCHAR* toString() { return stringDuplicate(_T("ReqExclScorer")); }
     ...
     * <br>When this method is used the {@link #explain(int)} method should not
     * be used.
...
class BooleanScorer2::BSConjunctionScorer
    : public CL_NS(search)::ConjunctionScorer {
private:
    CL_NS(search)::BooleanScorer2::Coordinator* coordinator;
    int32_t lastScoredDoc;
    int32_t requiredNrMatchers;
    typedef CL_NS(util)::CLVector<
        Scorer*, CL_NS(util)::Deletor::Object<Scorer>> ScorersType;

public:
    BSConjunctionScorer(
        CL_NS(search)::BooleanScorer2::Coordinator* _coordinator,
        ScorersType* _requiredScorers, int32_t _requiredNrMatchers)
...
class BooleanScorer2::BSDisjunctionSumScorer
    : public CL_NS(search)::DisjunctionSumScorer {
private:
    CL_NS(search)::BooleanScorer2::Coordinator* coordinator;
    int32_t lastScoredDoc;
    typedef CL_NS(util)::CLVector<
...
public:
    BSDisjunctionSumScorer(
        CL_NS(search)::BooleanScorer2::Coordinator* _coordinator,
        ScorersType* subScorers, int32_t minimumNrMatchers)
...
public:
    ...
    Scorer* countingDisjunctionSumScorer(ScorersType* scorers,
                                         int32_t minNrShouldMatch) {
        return _CLNEW BSDisjunctionSumScorer(coordinator, scorers,
                                             minNrShouldMatch);
    }

    Scorer* countingConjunctionSumScorer(ScorersType* requiredScorers) {
        return _CLNEW BSConjunctionScorer(coordinator, requiredScorers,
    ...
    Scorer* dualConjunctionSumScorer(Scorer* req1, Scorer* req2) {
        ...
        scorers[0] = req1;
        scorers[1] = req2;
        return _CLNEW CL_NS(search)::ConjunctionScorer(Similarity::getDefault(),
                                                       &scorers);
    }
    ...
    Scorer* makeCountingSumScorerNoReq() {
        if (optionalScorers.size() == 0) {
            ...
            size_t nrOptRequired =
                (minNrShouldMatch < 1) ? 1 : minNrShouldMatch;
                ...
                Scorer* requiredCountingSumScorer =
                    (optionalScorers.size() > nrOptRequired)
                        ...
                        : (optionalScorers.size() == 1)
                              ? _CLNEW SingleMatchScorer(
                                    (Scorer*)optionalScorers[0], coordinator)
    ...
    Scorer* makeCountingSumScorerSomeReq() {
        if (optionalScorers.size() < minNrShouldMatch) {
            ...
            for (Internal::ScorersType::iterator it = requiredScorers.begin();
                 it != requiredScorers.end(); it++) {
                allReq.push_back(*it);
            }
            for (Internal::ScorersType::iterator it2 = optionalScorers.begin();
                 it2 != optionalScorers.end(); it2++) {
            ...
            Scorer* requiredCountingSumScorer =
                (requiredScorers.size() == 1)
                    ? _CLNEW SingleMatchScorer((Scorer*)requiredScorers[0],
                                               coordinator)
            ...
            if (minNrShouldMatch > 0) {
                ...
                return _CLNEW ReqOptSumScorer(
                    ...
                    ((optionalScorers.size() == 1)
                         ? _CLNEW SingleMatchScorer((Scorer*)optionalScorers[0],
                                                    coordinator)
    ...
    Scorer* addProhibitedScorers(Scorer* requiredCountingSumScorer) {
        return (prohibitedScorers.size() == 0)
                   ? requiredCountingSumScorer
                   : _CLNEW ReqExclScorer(
                         requiredCountingSumScorer,
                         ((prohibitedScorers.size() == 1)
                              ? (Scorer*)prohibitedScorers[0]
                              : _CLNEW CL_NS(search)::DisjunctionSumScorer(
                                    &prohibitedScorers)));
    }

    Internal(BooleanScorer2* parent, int32_t _minNrShouldMatch,
             bool _allowDocsOutOfOrder)
    ...
    ~Internal() {
        ...
        /* TODO: these leak memory... haven't figure out how it should be fixed
        though... requiredScorers.clear(); optionalScorers.clear();
        prohibitedScorers.clear();
        */
    }
};

BooleanScorer2::BooleanScorer2(Similarity* similarity, int32_t minNrShouldMatch,
                               bool allowDocsOutOfOrder)
    : Scorer(similarity) {
    _internal = new Internal(this, minNrShouldMatch, allowDocsOutOfOrder);
}

BooleanScorer2::~BooleanScorer2() { delete _internal; }
...
void BooleanScorer2::score(HitCollector* hc) {
    if (_internal->allowDocsOutOfOrder &&
        ...
        BooleanScorer* bs =
            _CLNEW BooleanScorer(getSimilarity(), _internal->minNrShouldMatch);
