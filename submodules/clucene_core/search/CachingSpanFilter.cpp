...
public:
    ...
    ~ResultHolder() {
        if (deleteResult) _CLDELETE(result);
    }
};

struct CachingSpanFilter::Internal {
    typedef CL_NS(util)::CLHashMap<
        CL_NS(index)::IndexReader*, ResultHolder*,
        ...
        CL_NS(util)::Deletor::Object<ResultHolder>> ResultCacheType;
...
CL_NS(util)::BitSet* CachingSpanFilter::bits(
    CL_NS(index)::IndexReader* reader) {
    SpanFilterResult* result = getCachedResult(reader);
    return result != NULL ? result->getBits() : NULL;
}

SpanFilterResult* CachingSpanFilter::getCachedResult(
    CL_NS(index)::IndexReader* reader) {
...
SpanFilterResult* CachingSpanFilter::bitSpans(
    CL_NS(index)::IndexReader* reader) {
