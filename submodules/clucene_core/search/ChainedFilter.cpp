...
Filter* ChainedFilter::clone() const { return _CLNEW ChainedFilter(*this); }
...
TCHAR* ChainedFilter::toString() {
    ...
    int* la = logicArray;
    while (*filter) {
        if (filter != filters) buf.appendChar(' ');
...
BitSet* ChainedFilter::bits(IndexReader* reader, int logic) {
    ...
    if (*filter) {
        ...
        if ((*filter)->shouldDeleteBitSet(
                tmp))  // if we are supposed to delete this BitSet, then
            bts = tmp; // we can safely call it our own
        else if (tmp == NULL) {
            int32_t len = reader->maxDoc();
            bts = _CLNEW BitSet(
                len); // bitset returned null, which means match _all_
            for (int32_t i = 0; i < len; i++) bts->set(i);
        } else {
            bts = tmp->clone(); // else it is probably cached, so we need to
    ...
    if (*filter) {
        ...
        if ((*filter)->shouldDeleteBitSet(
            ...
            for (int32_t i = 0; i < len; i++)
                bts->set(i); // todo: this could mean that we can skip certain
                             // types of filters
        } else {
            bts = tmp->clone(); // else it is probably cached, so we need to
                                // copy it before using it.
...
void ChainedFilter::doUserChain(CL_NS(util)::BitSet* /*chain*/,
                                CL_NS(util)::BitSet* /*filter*/,
                                int /*logic*/) {
    _CLTHROWA(CL_ERR_Runtime, "User chain logic not implemented by superclass");
}

BitSet* ChainedFilter::doChain(BitSet* resultset, IndexReader* reader,
            ...
            case XOR:
                for (i = 0; i < maxDoc; i++)
                    resultset->set(
                        i,
                        resultset->get(i) ^
                            ((filterbits == NULL || filterbits->get(i)) ? 1
                                                                        : 0));
                break;
            default: doChain(resultset, reader, DEFAULT, filter);
        }
    }

    if (filter->shouldDeleteBitSet(filterbits)) _CLDELETE(filterbits);
