...
public:
    ConstantScorer(Similarity* similarity, IndexReader* reader, Weight* w,
        ...
        : Scorer(similarity),
          ...
          theScore(w->getValue()),
          _doc(-1) {}
    virtual ~ConstantScorer() { _CLLDELETE(bits); }
    ...
    Explanation* explain(int32_t /*doc*/) {
        _CLTHROWA(CL_ERR_UnsupportedOperation,
                  "Unsupported operation at ConstantScoreQuery::explain");
    }
...
public:
    ...
    float_t getValue() { return queryWeight; }
    ...
    Scorer* scorer(IndexReader* reader) {
        return _CLNEW ConstantScorer(similarity, reader, this,
                                     parentQuery->filter);
    }
...
ConstantScoreQuery::~ConstantScoreQuery() { _CLLDELETE(filter); }

Filter* ConstantScoreQuery::getFilter() const { return filter; }

Query* ConstantScoreQuery::rewrite(IndexReader* reader) { return this; }
...
// TODO: Filter is missing an equals() function, hence this equals() is
// incomplete
...
ConstantScoreRangeQuery::ConstantScoreRangeQuery(
    const ConstantScoreRangeQuery& copy)
    : fieldName(const_cast<TCHAR*>(CLStringIntern::intern(copy.fieldName))),
      lowerVal(STRDUP_TtoT(copy.lowerVal)),
      upperVal(STRDUP_TtoT(copy.upperVal)),
      includeLower(copy.includeLower),
      includeUpper(copy.includeUpper) {}

ConstantScoreRangeQuery::ConstantScoreRangeQuery(const TCHAR* _fieldName,
                                                 const TCHAR* _lowerVal,
                                                 const TCHAR* _upperVal,
                                                 bool _includeLower,
                                                 bool _includeUpper)
    : fieldName(NULL),
      lowerVal(NULL),
    ...
    this->fieldName = const_cast<TCHAR*>(
        CLStringIntern::intern(_fieldName)); // intern it, just like terms...
    if (_lowerVal != NULL) this->lowerVal = STRDUP_TtoT(_lowerVal);
    if (_upperVal != NULL) this->upperVal = STRDUP_TtoT(_upperVal);
...
Query* ConstantScoreRangeQuery::rewrite(CL_NS(index)::IndexReader* reader) {
    ...
    RangeFilter* rangeFilt = _CLNEW RangeFilter(
        fieldName, lowerSafe, upperVal,
...
bool ConstantScoreRangeQuery::equals(Query* o) const {
    ...
    if (this->lowerVal != NULL ? _tcscmp(this->lowerVal, other->lowerVal) != 0
                               : other->lowerVal != NULL)
        return false;
    if (this->upperVal != NULL ? _tcscmp(this->upperVal, other->upperVal) != 0
                               : other->upperVal != NULL)
        return false;
    return this->getBoost() == other->getBoost();
}
...
size_t ConstantScoreRangeQuery::hashCode() const {
    int32_t h =
        Similarity::floatToByte(getBoost()) ^ Misc::thashCode(fieldName);
