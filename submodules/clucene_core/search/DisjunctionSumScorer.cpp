...
DisjunctionSumScorer::DisjunctionSumScorer(
    DisjunctionSumScorer::ScorersType* _subScorers,
    const int32_t _minimumNrMatchers)
    : Scorer(NULL),
    ...
    for (DisjunctionSumScorer::ScorersType::iterator itr = _subScorers->begin();
         itr != _subScorers->end(); itr++) {
        subScorers.push_back(*itr);
    }
}

DisjunctionSumScorer::~DisjunctionSumScorer() { _CLLDELETE(scorerDocQueue); }
...
float_t DisjunctionSumScorer::score() { return currentScore; }
int32_t DisjunctionSumScorer::doc() const { return currentDoc; }

int32_t DisjunctionSumScorer::nrMatchers() const { return _nrMatchers; }
...
void DisjunctionSumScorer::initScorerDocQueue() {
    // No need to _CLLDELETE here since this function since this function is
    // only called if scorerDocQueue==NULL
    scorerDocQueue = _CLNEW ScorerDocQueue(nrScorers);
    queueSize = 0;

    for (ScorersType::iterator it = subScorers.begin(); it != subScorers.end();
         ++it) {
