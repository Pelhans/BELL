...
Explanation::Explanation(float_t _value, const TCHAR* _description)
    : value(_value),
      details(NULL) {
    _tcsncpy(this->description, _description,
...
Explanation::Explanation(const Explanation& copy) : details(NULL) { set(copy); }
void Explanation::set(const Explanation& copy) {
    ...
    if (copy.details != NULL) {
        this->details = _CLNEW CL_NS(
            util)::CLArrayList<Explanation*,
                               CL_NS(util)::Deletor::Object<Explanation>>(true);
        CL_NS(util)::CLArrayList<
            Explanation*,
            CL_NS(util)::Deletor::Object<Explanation>>::const_iterator itr;
...
Explanation::~Explanation() { _CLLDELETE(this->details); }

bool Explanation::isMatch() const { return (0.0f < getValue()); }

float_t Explanation::getValue() const { return value; }
void Explanation::setValue(const float_t value) { this->value = value; }

const TCHAR* Explanation::getDescription() const { return description; }
void Explanation::setDescription(const TCHAR* description) {
    _tcsncpy(this->description, description,
             LUCENE_SEARCH_EXPLANATION_DESC_LEN);
}
...
void Explanation::addDetail(Explanation* detail) {
    if (details == NULL)
        details = _CLNEW CL_NS(
            util)::CLArrayList<Explanation*,
                               CL_NS(util)::Deletor::Object<Explanation>>(true);
    details->push_back(detail);
}

TCHAR* Explanation::toString() { return toString(0); }
...
ComplexExplanation::ComplexExplanation(const bool _match, const float_t _value,
                                       const TCHAR* _description)
    : Explanation(_value, _description),
      match(_match) {}
