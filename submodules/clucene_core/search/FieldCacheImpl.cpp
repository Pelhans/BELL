...
class fieldcacheCacheReaderType
    : public CL_NS(util)::CLHashMap<
          FieldCacheImpl::FileEntry*, FieldCacheAuto*,
          FieldCacheImpl::FileEntry::Compare, FieldCacheImpl::FileEntry::Equals,
...
public:
    ...
    ~fieldcacheCacheReaderType() {
        ...
        while (itr != end()) {
            ...
            if (f->getType() != SortField::AUTO) _CLDELETE(itr->second);
            _CLDELETE(f);
            ++itr;
        }
        clear();
...
class fieldcacheCacheType
    : public CL_NS(util)::CLHashMap<
          CL_NS(index)::IndexReader*, fieldcacheCacheReaderType*,
...
FieldCache::StringIndex::StringIndex(int32_t* values, TCHAR** lookup,
                                     int count) {
    this->count = count;
    this->order = values;
    this->lookup = lookup;
...
FieldCache::StringIndex::~StringIndex() {
    ...
    for (int i = 0; i < count; i++) _CLDELETE_CARRAY(lookup[i]);
...
FieldCacheImpl::FileEntry::FileEntry(const TCHAR* field,
...
FieldCacheImpl::FileEntry::~FileEntry() { CLStringIntern::unintern(field); }

size_t FieldCacheImpl::FileEntry::hashCode() {
    if (_hashCode == 0) {
        // todo: cache hashcode?
        size_t ret = Misc::thashCode(field);
        if (custom != NULL) ret = ret ^ custom->hashCode();
        ret = ret ^ (type * 7); // type with a seed
        _hashCode = ret;
    }
    return _hashCode;
}
int32_t FieldCacheImpl::FileEntry::compareTo(
    const FieldCacheImpl::FileEntry* other) const {
...
FieldCacheAuto* FieldCacheImpl::lookup(IndexReader* reader, const TCHAR* field,
                                       int32_t type) {
    ...
    FileEntry* entry = _CLNEW FileEntry(field, type);
    {
        ...
        if (readerCache != NULL) ret = readerCache->get(entry);
        _CLDELETE(entry);
    }
    return ret;
}
...
void FieldCacheImpl::closeCallback(CL_NS(index)::IndexReader* reader,
                                   void* fieldCacheImpl) {
...
void FieldCacheImpl::store(IndexReader* reader, const TCHAR* field,
                           int32_t type, FieldCacheAuto* value) {
    FileEntry* entry = _CLNEW FileEntry(field, type);
    {
        SCOPED_LOCK_MUTEX(THIS_LOCK)
        fieldcacheCacheReaderType* readerCache = cache->get(reader);
        ...
        // this is supposed to return the previous value, but it needs to be
        // deleted!!!
    }
}
...
// inherit javadocs
FieldCacheAuto* FieldCacheImpl::getInts(IndexReader* reader,
                                        const TCHAR* field) {
    ...
    if (ret == NULL) {
        ...
        if (retLen > 0) {
            ...
            try {
                if (termEnum->term(false) == NULL) {
                    _CLTHROWA(CL_ERR_Runtime,
                              "no terms in field"); // todo: add detailed error:
                                                    // + field);
                }
                do {
            ...
            _CLFINALLY(termDocs->close(); _CLDELETE(termDocs);
                       termEnum->close(); _CLDELETE(termEnum);)
        }

        FieldCacheAuto* fa =
            _CLNEW FieldCacheAuto(retLen, FieldCacheAuto::INT_ARRAY);
...
// inherit javadocs
FieldCacheAuto* FieldCacheImpl::getFloats(IndexReader* reader,
                                          const TCHAR* field) {
    field = CLStringIntern::intern(field);
    ...
    if (ret == NULL) {
        ...
        if (retLen > 0) {
            ...
            try {
                if (termEnum->term(false) == NULL) {
                    _CLTHROWA(CL_ERR_Runtime,
                              "no terms in field "); // todo: make richer error
                                                     // + field);
                }
                do {
                    Term* term = termEnum->term(false);
        ...
        FieldCacheAuto* fa =
            _CLNEW FieldCacheAuto(retLen, FieldCacheAuto::FLOAT_ARRAY);
...
// inherit javadocs
FieldCacheAuto* FieldCacheImpl::getStrings(IndexReader* reader,
                                           const TCHAR* field) {
        ...
        if (retLen > 0) {
            ...
            try {
                if (termEnum->term(false) == NULL) {
                    _CLTHROWA(
                        CL_ERR_Runtime,
                        "no terms in field "); // todo: extend to + field);
                }
                do {
                    Term* term = termEnum->term(false);
                    if (term->field() != field) break;
                    ...
                    termDocs->seek(termEnum);
                    while (termDocs->next()) {
                        retArray[termDocs->doc()] = STRDUP_TtoT(
                            termval); // todo: any better way of doing this???
                    }
                } while (termEnum->next());
            }
            _CLFINALLY(retArray[retLen] = NULL; termDocs->close();
                       _CLDELETE(termDocs); termEnum->close();
                       _CLDELETE(termEnum);)
        }
        FieldCacheAuto* fa =
            _CLNEW FieldCacheAuto(retLen, FieldCacheAuto::STRING_ARRAY);
...
// inherit javadocs
FieldCacheAuto* FieldCacheImpl::getStringIndex(IndexReader* reader,
                                               const TCHAR* field) {
    field = CLStringIntern::intern(field);
    ...
    if (ret == NULL) {
        ...
        if (retLen > 0) {
            ...
            // FieldDocSortedHitQueue needs to change as well.
            mterms[t++] = NULL;

            try {
                if (termEnum->term(false) == NULL) {
                    _CLTHROWA(CL_ERR_Runtime,
                              "no terms in field"); // todo: make rich message "
                                                    // + field);
                }
                do {
                    Term* term = termEnum->term(false);
                    ...
                    if (t >= retLen + 1)
                        _CLTHROWA(CL_ERR_Runtime,
                                  "there are more terms than documents in "
                                  "field"); // todo: rich error \"" + field +
                                            // "\"");
        ...
        FieldCache::StringIndex* value =
            _CLNEW FieldCache::StringIndex(retArray, mterms, t);

        FieldCacheAuto* fa =
            _CLNEW FieldCacheAuto(retLen, FieldCacheAuto::STRING_INDEX);
...
// inherit javadocs
FieldCacheAuto* FieldCacheImpl::getAuto(IndexReader* reader,
                                        const TCHAR* field) {
    field = CLStringIntern::intern(field);
    ...
    if (ret == NULL) {
        ...
        try {
            ...
            if (term == NULL) {
                _CLTHROWA(CL_ERR_Runtime,
                          "no terms in field - cannot determine sort "
                          "type"); // todo: make rich error: " + field
                                   // + "
            }
            if (term->field() == field) {
                ...
                if (isint)
                    ...
                    if (termtext[termTextLen - 1] == 'f') searchLen--;
                    for (int32_t i = 0; i < searchLen; i++) {
                        if (_tcschr(_T("0123456789 Ee.+-"), termtext[i]) ==
                            NULL) {
                            isfloat = false;
                            break;
                        }
                ...
                _CLTHROWA(CL_ERR_Runtime,
                          "field does not appear to be indexed"); // todo: make
                                                                  // rich error:
                                                                  // \"" + field
                                                                  // + "\"
            }
...
// inherit javadocs
FieldCacheAuto* FieldCacheImpl::getCustom(IndexReader* reader,
                                          const TCHAR* field,
    ...
    if (ret == NULL) {
        ...
        if (retLen > 0) {
            ...
            try {
                if (termEnum->term(false) == NULL) {
                    _CLTHROWA(CL_ERR_Runtime,
                              "no terms in field "); // todo: make rich error +
                                                     // field);
                }
                do {
                    Term* term = termEnum->term(false);
                    if (term->field() != field) break;
                    Comparable* termval =
                        comparator->getComparable(term->text());
                    termDocs->seek(termEnum);
                    while (termDocs->next()) {
                        retArray[termDocs->doc()] = termval;
                    }
        ...
        FieldCacheAuto* fa =
            _CLNEW FieldCacheAuto(retLen, FieldCacheAuto::COMPARABLE_ARRAY);
