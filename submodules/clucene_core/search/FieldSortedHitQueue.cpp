...
class hitqueueCacheType
    : public CL_NS(util)::CLHashMap<
          CL_NS(index)::IndexReader*, hitqueueCacheReaderType*,
...
class hitqueueCacheReaderType
    : public CL_NS(util)::CLHashMap<
          FieldCacheImpl::FileEntry*, ScoreDocComparator*,
...
public:
    ...
    ~hitqueueCacheReaderType() { clear(); }
};

hitqueueCacheType* FieldSortedHitQueue::Comparators =
    _CLNEW hitqueueCacheType(false, true);
...
FieldSortedHitQueue::FieldSortedHitQueue(IndexReader* reader,
                                         SortField** _fields, int32_t size)
    : fieldsLen(0),
      maxscore(1.0f) {
    while (_fields[fieldsLen] != 0) fieldsLen++;
    ...
    for (int32_t i = 0; i < fieldsLen; ++i) {
        ...
        comparators[i] = getCachedComparator(
            reader, fieldname, _fields[i]->getType(), _fields[i]->getFactory());
        tmp[i] = _CLNEW SortField(fieldname, comparators[i]->sortType(),
                                  _fields[i]->getReverse());
    }
...
bool FieldSortedHitQueue::lessThan(FieldDoc* docA, FieldDoc* docB) {
    ...
    for (int32_t i = 0; c == 0 && i < comparatorsLen; ++i) {
        c = (fields[i]->getReverse())
                ? comparators[i]->compare(&docB->scoreDoc, &docA->scoreDoc)
                : comparators[i]->compare(&docA->scoreDoc, &docB->scoreDoc);
    }
    // avoid random sort order that could lead to duplicates (bug #31241):
    if (c == 0) return docA->scoreDoc.doc > docB->scoreDoc.doc;
    return c > 0;
}
...
ScoreDocComparator* FieldSortedHitQueue::comparatorString(IndexReader* reader,
    ...
    CND_PRECONDITION(fa->contentType == FieldCacheAuto::STRING_INDEX,
...
// static
ScoreDocComparator* FieldSortedHitQueue::comparatorInt(IndexReader* reader,
                                                       const TCHAR* field) {
    // const TCHAR* field = CLStringIntern::intern(fieldname);
    FieldCacheAuto* fa = FieldCache::DEFAULT()->getInts(reader, field);
    // CLStringIntern::unintern(field);

    CND_PRECONDITION(fa->contentType == FieldCacheAuto::INT_ARRAY,
                     "Content type is incorrect");
    return _CLNEW ScoreDocComparators::Int32(fa->intArray, fa->contentLen);
}

// static
ScoreDocComparator* FieldSortedHitQueue::comparatorFloat(IndexReader* reader,
                                                         const TCHAR* field) {
    // const TCHAR* field = CLStringIntern::intern(fieldname);
    FieldCacheAuto* fa = FieldCache::DEFAULT()->getFloats(reader, field);
    // CLStringIntern::unintern(field);

    CND_PRECONDITION(fa->contentType == FieldCacheAuto::FLOAT_ARRAY,
                     "Content type is incorrect");
    return _CLNEW ScoreDocComparators::Float(fa->floatArray, fa->contentLen);
}
// static
ScoreDocComparator* FieldSortedHitQueue::comparatorAuto(IndexReader* reader,
                                                        const TCHAR* field) {
    // const TCHAR* field = CLStringIntern::intern(fieldname);
    ...
    if (fa->contentType == FieldCacheAuto::STRING_INDEX) {
        ...
        _CLTHROWA(CL_ERR_Runtime,
                  "unknown data type in field"); // todo: rich error
                                                 // information: '"+field+"'");
    }
}

// todo: Locale locale, not implemented yet
ScoreDocComparator* FieldSortedHitQueue::getCachedComparator(
    IndexReader* reader, const TCHAR* fieldname, int32_t type,
    SortComparatorSource* factory) {
    if (type == SortField::DOC) return ScoreDocComparator::INDEXORDER();
    if (type == SortField::DOCSCORE) return ScoreDocComparator::RELEVANCE();
...
FieldDoc* FieldSortedHitQueue::fillFields(FieldDoc* doc) const {
    ...
    fields[n] = NULL;
    doc->fields = fields;
    if (maxscore > 1.0f) doc->scoreDoc.score /= maxscore; // normalize scores
    return doc;
}

ScoreDocComparator* FieldSortedHitQueue::lookup(IndexReader* reader,
                                                const TCHAR* field,
                                                int32_t type,
                                                SortComparatorSource* factory) {
    ScoreDocComparator* sdc = NULL;
    FieldCacheImpl::FileEntry* entry =
        (factory != NULL) ? _CLNEW FieldCacheImpl::FileEntry(field, factory)
...
void FieldSortedHitQueue::closeCallback(CL_NS(index)::IndexReader* reader,
                                        void*) {
    SCOPED_LOCK_MUTEX(Comparators_LOCK)
...
// static
void FieldSortedHitQueue::store(IndexReader* reader, const TCHAR* field,
                                int32_t type, SortComparatorSource* factory,
                                ScoreDocComparator* value) {
    FieldCacheImpl::FileEntry* entry =
        (factory != NULL) ? _CLNEW FieldCacheImpl::FileEntry(field, factory)
                          : _CLNEW FieldCacheImpl::FileEntry(field, type);
...
FieldSortedHitQueue::~FieldSortedHitQueue() {
    ...
    if (fields != NULL) {
        for (int i = 0; fields[i] != NULL; i++) _CLDELETE(fields[i]);
