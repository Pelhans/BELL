 ...
 * Uses <code>FieldCache.DEFAULT</code> for maintaining internal term lookup
 * tables.
 *
 * @see Searchable#search(Query,Filter,int32_t,Sort)
 * @see FieldCache
 */
class CLUCENE_EXPORT FieldSortedHitQueue
    : public CL_NS(util)::PriorityQueue<
          FieldDoc*, CL_NS(util)::Deletor::Object<FieldDoc>> {
...
private:
    ...
    static ScoreDocComparator* lookup(CL_NS(index)::IndexReader* reader,
                                      const TCHAR* field, int32_t type,
                                      SortComparatorSource* factory);

    /** Stores a comparator into the cache.
        returns the valid ScoreDocComparator.
    */
    static void store(CL_NS(index)::IndexReader* reader, const TCHAR* field,
    ...
    static ScoreDocComparator* getCachedComparator(
        CL_NS(index)::IndexReader* reader, const TCHAR* fieldname, int32_t type,
        SortComparatorSource* factory);
     ...
     */
    static ScoreDocComparator* comparatorInt(CL_NS(index)::IndexReader* reader,
     ...
     * @throws IOException If an error occurs reading the index.
     */
    static ScoreDocComparator* comparatorFloat(
        CL_NS(index)::IndexReader* reader, const TCHAR* fieldname);
     ...
     * @throws IOException If an error occurs reading the index.
     */
    static ScoreDocComparator* comparatorString(
        CL_NS(index)::IndexReader* reader, const TCHAR* fieldname);
    ...
    (locale); TCHAR* field = fieldname.intern(); TCHAR** index =
    FieldCache.DEFAULT.getStrings (reader, field); return _CLNEW
    ScoreDocComparator() {
     ...
     * @throws IOException If an error occurs reading the index.
     */
    static ScoreDocComparator* comparatorAuto(CL_NS(index)::IndexReader* reader,
                                              const TCHAR* fieldname);
...
public:
     ...
     * @throws IOException
     */
    FieldSortedHitQueue(CL_NS(index)::IndexReader* reader, SortField** fields,
                        int32_t size);
    ...
    void setFields(SortField** fields) { this->fields = fields; }

    /** Returns the SortFields being used by this hit queue. */
    SortField** getFields() { return fields; }
};
