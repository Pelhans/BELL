...
FuzzyTermEnum::FuzzyTermEnum(IndexReader* reader, Term* term,
                             float_t minSimilarity, size_t _prefixLength)
    : FilteredTermEnum(),
      d(NULL),
      dLen(0),
      _similarity(0),
      ...
      text(NULL),
      textLen(0),
      prefix(NULL) /* ISH: was STRDUP_TtoT(LUCENE_BLANK_STRING)*/,
      prefixLength(0),
    ...
    scale_factor =
        1.0f /
        (1.0f -
         minimumSimilarity); // only now we are safe from a division by zero
    // TODO: this.field = searchTerm.field();

    // The prefix could be longer than the word.
    // It's kind of silly though.  It means we must match the entire word.
    const size_t fullSearchTermLength = searchTerm->textLength();
    const size_t realPrefixLength = _prefixLength > fullSearchTermLength
    ...
    Term* trm =
        _CLNEW Term(searchTerm->field(),
                    prefix); // _CLNEW Term(term, prefix); -- not intern'd?
...
FuzzyTermEnum::~FuzzyTermEnum() { close(); }
...
bool FuzzyTermEnum::endEnum() { return _endEnum; }
...
bool FuzzyTermEnum::termCompare(Term* term) {
    ...
    //       if the distance of the current term in the enumeration is bigger
    //       than the FUZZY_THRESHOLD then true is returned
...
float_t FuzzyTermEnum::similarity(const TCHAR* target, const size_t m) {
    ...
    if (maxDistance < abs((int32_t)(m - n))) {
        ...
        // that given this optimal circumstance, the edit distance cannot be less
        // than 5. which is 8-3 or more precisesly Math.abs(3-8). if our maximum
        // edit distance is 4, then we can discard this word without looking at
        // it.
        return 0.0f;
    }

    // let's make sure we have enough room in our array to do the distance
    // calculations. Check if the array must be reallocated because it is too
    ...
    for (i = 1; i <= n; i++) {
        ...
        for (j = 1; j <= m; j++) {
            if (s_i != target[j - 1]) {
                min3(d[i - 1 + (j * dWidth)], d[i + ((j - 1) * dWidth)],
                     d[i - 1 + ((j - 1) * dWidth)]);
                d[i + (j * dWidth)] = __t + 1;
            } else {
                min3(d[i - 1 + (j * dWidth)] + 1, d[i + ((j - 1) * dWidth)] + 1,
                     d[i - 1 + ((j - 1) * dWidth)]);
                d[i + (j * dWidth)] = __t;
            }
            bestPossibleEditDistance =
                cl_min(bestPossibleEditDistance, d[i + (j * dWidth)]);
        }
        ...
        if (i > maxDistance &&
            bestPossibleEditDistance >
                maxDistance) { // equal is okay, but not greater
    ...
    // greater than 0.0)
    return 1.0f - ((float_t)d[n + m * dWidth] /
                   (float_t)(prefixLength + cl_min(n, m)));
}

int32_t FuzzyTermEnum::getMaxDistance(const size_t m) {
    return (m < LUCENE_TYPICAL_LONGEST_WORD_IN_INDEX) ? maxDistances[m]
...
int32_t FuzzyTermEnum::calculateMaxDistance(const size_t m) const {
    return (int32_t)((1 - minimumSimilarity) *
                     (cl_min(textLen, m) + prefixLength));
}
...
public:
    ...
    virtual ~ScoreTerm() { _CLLDECDELETE(term); }
};

class ScoreTermQueue
    : public PriorityQueue<ScoreTerm*,
                           CL_NS(util)::Deletor::Object<ScoreTerm>> {
public:
    ScoreTermQueue(int32_t size) { initialize(size, true); }
...
FuzzyQuery::FuzzyQuery(Term* term, float_t _minimumSimilarity,
...
float_t FuzzyQuery::getMinSimilarity() const { return minimumSimilarity; }

size_t FuzzyQuery::getPrefixLength() const { return prefixLength; }

TCHAR* FuzzyQuery::toString(const TCHAR* field) const {
    StringBuffer buffer(
        100); // TODO: Have a better estimation for the initial buffer length
...
FuzzyQuery::FuzzyQuery(const FuzzyQuery& clone) : MultiTermQuery(clone) {
...
Query* FuzzyQuery::clone() const { return _CLNEW FuzzyQuery(*this); }
...
bool FuzzyQuery::equals(Query* other) const {
    ...
    if (!(other->instanceOf(FuzzyQuery::getClassName()))) return false;
...
FilteredTermEnum* FuzzyQuery::getEnum(IndexReader* reader) {
    ...
    FuzzyTermEnum* ret =
        _CLNEW FuzzyTermEnum(reader, term, minimumSimilarity, prefixLength);
    return ret;
}
