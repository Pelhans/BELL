...
Hits::Hits(Searcher* s, Query* q, Filter* f, const Sort* _sort)
    ...
    hitDocs = _CLNEW CL_NS(
        util)::CLVector<HitDoc*, CL_NS(util)::Deletor::Object<HitDoc>>;
...
int32_t Hits::countDeletions(CL_NS(search)::Searcher* s) {
    ...
    if (s->getObjectName() == IndexSearcher::getClassName()) {
        cnt = s->maxDoc() -
              static_cast<IndexSearcher*>(s)->getReader()->numDocs();
    }
    return cnt;
}

size_t Hits::length() const { return _length; }
...
int32_t Hits::id(const int32_t n) { return getHitDoc(n)->id; }

float_t Hits::score(const int32_t n) { return getHitDoc(n)->score; }

void Hits::getMoreDocs(const size_t m) {
    size_t _min = m;
    if (hitDocs->size() > _min) _min = hitDocs->size();
    ...
    if (scoreDocs != NULL) {
        ...
        if (nDeletions < 0 || nDels2 > nDeletions) {
            ...
            size_t i2 = 0;
            for (size_t i1 = 0; i1 < hitDocs->size() && i2 < scoreDocsLength;
                 i1++) {
        ...
        for (size_t i = start; i < end; i++) {
            hitDocs->push_back(_CLNEW HitDoc(scoreDocs[i].score * scoreNorm,
                                             scoreDocs[i].doc));
        }
...
HitDoc* Hits::getHitDoc(const size_t n) {
    ...
    if (n >= hitDocs->size()) getMoreDocs(n);
