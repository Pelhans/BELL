 ...
 * &ge; current_{@link #length()} (but <code>n</code> &lt; {@link
 * #length()}_at_start).
 */
...
private:
    ...
    CL_NS(util)::CLVector<HitDoc*,
                          CL_NS(util)::Deletor::Object<
                              HitDoc>>* hitDocs; // cache of hits retrieved
    ...
    size_t _lengthAtStart; // this is the number apps usually count on (although
                           // deletions can bring it down).
    int32_t nDeletedHits;  // # of already collected hits that were meanwhile
                           // deleted.
...
public:
     ...
     * <p>Documents are cached, so that repeated requests for the same element
     * may return the same Document object.
