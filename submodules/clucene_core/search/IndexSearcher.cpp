...
public:
    SimpleTopDocsCollector(const CL_NS(util)::BitSet* bs, HitQueue* hitQueue,
                           int32_t* totalhits, size_t ndocs,
                           const float_t ms = -1.0f)
        : minScore(ms),
          bits(bs),
          hq(hitQueue),
          nDocs(ndocs),
...
public:
    SortedTopDocsCollector(const CL_NS(util)::BitSet* bs,
                           FieldSortedHitQueue* hitQueue, int32_t* totalhits,
                           size_t _nDocs)
        : bits(bs),
          hq(hitQueue),
          nDocs(_nDocs),
    ...
    void collect(const int32_t doc, const float_t score) {
        if (score > 0.0f &&                     // ignore zeroed buckets
            (bits == NULL || bits->get(doc))) { // skip docs not in bits
            ++totalHits[0];
            FieldDoc* fd = _CLNEW FieldDoc(
                doc, score);     // todo: see jlucene way... with fields def???
...
_CL_DEPRECATED(doc(i, document))
CL_NS(document)::Document* IndexSearcher::doc(int32_t i) {
    CL_NS(document)::Document* ret = _CLNEW CL_NS(document)::Document;
    if (!doc(i, ret)) _CLDELETE(ret);
    return ret;
}
...
// todo: find out why we are passing Query* and not Weight*, as Weight is being
// extracted anyway from Query*
TopDocs* IndexSearcher::_search(Query* query, Filter* filter,
                                const int32_t nDocs) {
    // Func -
    // Pre  - reader != NULL
    // Post -
    ...
    _CLDELETE(hq);
    if (bits != NULL && filter->shouldDeleteBitSet(bits)) _CLDELETE(bits);
...
TopFieldDocs* IndexSearcher::_search(Query* query, Filter* filter,
    ...
    if (bits != NULL && filter->shouldDeleteBitSet(bits)) _CLLDELETE(bits);
    _CLDELETE_LARRAY(totalHits);
    return _CLNEW TopFieldDocs(totalHits0, fieldDocs, hqLen, hqFields);
}

void IndexSearcher::_search(Query* query, Filter* filter,
                            HitCollector* results) {
    // Func - _search an index and fetch the results
    //       Applications should only use this if they need all of the
    //       matching documents.  The high-level search API (search(Query)) is
    //       usually more efficient, as it skips non-high-scoring hits.
    // Pre  - query is a valid reference to a query
    //       filter may or may not be NULL
    //       results is a valid reference to a HitCollector and used to store
    //       the results
...
void IndexSearcher::explain(Query* query, int32_t doc, Explanation* ret) {
    Weight* weight = query->weight(this);
    ret->addDetail(
        weight->explain(reader, doc)); // TODO: A hack until this function will
                                       // return Explanation* as well
...
CL_NS(index)::IndexReader* IndexSearcher::getReader() { return reader; }

const char* IndexSearcher::getClassName() { return "IndexSearcher"; }
