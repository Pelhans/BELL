...
public:
    MatchAllDocsWeight(MatchAllDocsQuery* enclosingInstance,
                       Searcher* searcher);
...
public:
    MatchAllScorer(CL_NS(index)::IndexReader* _reader, Similarity* similarity,
                   Weight* w);
...
MatchAllDocsQuery::MatchAllScorer::MatchAllScorer(
    CL_NS(index)::IndexReader* _reader, Similarity* similarity, Weight* w)
    : Scorer(similarity),
      reader(_reader),
      id(-1) {
...
int32_t MatchAllDocsQuery::MatchAllScorer::doc() const { return id; }
...
float_t MatchAllDocsQuery::MatchAllScorer::score() { return _score; }
...
MatchAllDocsQuery::MatchAllDocsWeight::MatchAllDocsWeight(
    MatchAllDocsQuery* enclosingInstance, Searcher* searcher)
...
Query* MatchAllDocsQuery::MatchAllDocsWeight::getQuery() { return parentQuery; }
...
Scorer* MatchAllDocsQuery::MatchAllDocsWeight::scorer(
    CL_NS(index)::IndexReader* reader) {
    return _CLNEW MatchAllScorer(reader, similarity, this);
}

Explanation* MatchAllDocsQuery::MatchAllDocsWeight::explain(
    CL_NS(index)::IndexReader* reader, int32_t doc) {
...
const char* MatchAllDocsQuery::getClassName() { return "MatchAllDocsQuery"; }
const char* MatchAllDocsQuery::getObjectName() const { return getClassName(); }
...
bool MatchAllDocsQuery::equals(Query* o) const {
    if (!(o->instanceOf(MatchAllDocsQuery::getClassName()))) return false;
