...
#include "../index/_Term.h"
#include "../index/Term.h"
#include "../index/Terms.h"
#include "../index/IndexReader.h"
#include "../index/MultipleTermPositions.h"
...
public:
    MultiPhraseWeight(Searcher* searcher, MultiPhraseQuery* _parentQuery)
        : similarity(_parentQuery->getSimilarity(searcher)),
          value(0),
          idf(0),
          queryNorm(0),
          queryWeight(0),
        ...
        for (size_t i = 0; i < parentQuery->termArrays->size(); i++) {
            ...
            for (size_t j = 0; j < terms->length; j++) {
                idf += parentQuery->getSimilarity(searcher)->idf(
                    terms->values[j], searcher);
            }
        }
    }
    ...
    Scorer* scorer(IndexReader* reader) {
        ...
        // tps can be deleted safely. SloppyPhraseScorer or ExactPhraseScorer
        // will take care of its values
    ...
    Explanation* explain(IndexReader* reader, int32_t doc) {
        ...
        Explanation* boostExpl =
            _CLNEW Explanation(parentQuery->getBoost(), _T("boost"));
        if (parentQuery->getBoost() != 1.0f) queryExpl->addDetail(boostExpl);

        queryExpl->addDetail(idfExpl);

        Explanation* queryNormExpl =
            _CLNEW Explanation(queryNorm, _T("queryNorm"));
...
MultiPhraseQuery::MultiPhraseQuery()
    : field(NULL),
      termArrays(_CLNEW CL_NS(
          util)::CLArrayList<CL_NS(util)::ArrayBase<CL_NS(index)::Term*>*>),
      positions(_CLNEW CL_NS(
          util)::CLVector<int32_t, CL_NS(util)::Deletor::DummyInt32>),
      slop(0) {}
...
MultiPhraseQuery::MultiPhraseQuery(const MultiPhraseQuery& clone)
    ...
    this->termArrays = _CLNEW CL_NS(
        util)::CLArrayList<CL_NS(util)::ArrayBase<CL_NS(index)::Term*>*>();
    this->positions = _CLNEW CL_NS(
        util)::CLVector<int32_t, CL_NS(util)::Deletor::DummyInt32>();
    ...
    for (size_t j = 0; j < size; j++) {
        CL_NS(util)::ArrayBase<CL_NS(index)::Term*>* termsToClone =
            (*clone.termArrays)[j];
        CL_NS(util)::ArrayBase<CL_NS(index)::Term*>* terms = _CLNEW CL_NS(
            util)::ValueArray<CL_NS(index)::Term*>(termsToClone->length);
...
void MultiPhraseQuery::add(
    const CL_NS(util)::ArrayBase<CL_NS(index)::Term*>* terms) {
    int32_t position = 0;
    if (positions->size() > 0)
...
void MultiPhraseQuery::add(
    const CL_NS(util)::ArrayBase<CL_NS(index)::Term*>* _terms,
    const int32_t position) {
    if (termArrays->size() == 0) field = STRDUP_TtoT((*_terms)[0]->field());

    CL_NS(util)::ArrayBase<CL_NS(index)::Term*>* terms =
        _CLNEW CL_NS(util)::ValueArray<CL_NS(index)::Term*>(_terms->length);
    for (size_t i = 0; i < _terms->length; i++) {
        if (_tcscmp(_terms->values[i]->field(), field) != 0) {
            TCHAR buf[250];
            _sntprintf(
                buf, 250,
                _T("All phrase terms must be in the same field (%s): %s"),
                field, (*terms)[i]->field());
...
const CL_NS(util)::CLArrayList<CL_NS(
    util)::ArrayBase<CL_NS(index)::Term*>*>* MultiPhraseQuery::getTermArrays() {
    return termArrays;
}
...
TCHAR* MultiPhraseQuery::toString(const TCHAR* f) const {
    ...
    CL_NS(util)::CLArrayList<CL_NS(
        util)::ArrayBase<CL_NS(index)::Term*>*>::iterator i;
    i = termArrays->begin();
    while (i != termArrays->end()) {
        ...
        if (terms.length > 1) {
            ...
            for (size_t j = 0; j < terms.length; j++) {
                ...
                if (j < terms.length - 1) buffer.appendChar(_T(' '));
            }
            buffer.appendChar(_T(')'));
        } else {
            buffer.append(terms[0]->text());
        }
        if (i + 1 != termArrays->end()) buffer.appendChar(_T(' '));
...
class TermArray_Equals
    : public CL_NS_STD(binary_function)<const Term**, const Term**, bool> {
public:
...
bool MultiPhraseQuery::equals(Query* o) const {
    ...
    bool ret =
        (this->getBoost() == other->getBoost()) && (this->slop == other->slop);

    if (ret) {
        CLListEquals<
            int32_t, Equals::Int32,
            const CL_NS(
                util)::CLVector<int32_t, CL_NS(util)::Deletor::DummyInt32>,
            const CL_NS(
                util)::CLVector<int32_t, CL_NS(util)::Deletor::DummyInt32>>
    ...
    if (ret) {
        if (this->termArrays->size() != other->termArrays->size()) return false;

        for (size_t i = 0; i < this->termArrays->size(); i++) {
            CLListEquals<Term*, TermArray_Equals,
                         const CL_NS(util)::CLVector<CL_NS(
                             util)::ArrayBase<CL_NS(index)::Term*>*>,
                         const CL_NS(util)::CLVector<CL_NS(
                             util)::ArrayBase<CL_NS(index)::Term*>*>>
                comp;
