...
MultiSearcher::MultiSearcher(Searchable** _searchables) : _maxDoc(0) {
    searchablesLen = 0;
    while (_searchables[searchablesLen] != NULL) ++searchablesLen;
...
int32_t* MultiSearcher::getStarts() { return starts; }
int32_t MultiSearcher::getLength() { return searchablesLen; }
...
int32_t MultiSearcher::searcherIndex(int32_t n) const { return subSearcher(n); }
...
int32_t MultiSearcher::maxDoc() const { return _maxDoc; }

TopDocs* MultiSearcher::_search(Query* query, Filter* filter,
                                const int32_t nDocs) {
    ...
    for (int32_t i = 0; i < searchablesLen; i++) { // search each searcher
        ...
        for (j = 0; j < docs->scoreDocsLength;
             ++j) {                               // merge scoreDocs int_to hq
            scoreDocs[j].doc += starts[i];        // convert doc
            if (!hq->insert(scoreDocs[j])) break; // no more scores > minScore
        }
 ...
 */
void MultiSearcher::_search(Query* query, Filter* filter,
                            HitCollector* results) {
    for (int32_t i = 0; i < searchablesLen; ++i) {
        ...
        ** cumulatively (and was never deleted). */
        HitCollector* docNoAdjuster =
            _CLNEW MultiHitCollector(results, starts[i]);
        searchables[i]->_search(query, filter, docNoAdjuster);
        _CLDELETE(docNoAdjuster);
    }
}

TopFieldDocs* MultiSearcher::_search(Query* query, Filter* filter,
                                     const int32_t n, const Sort* sort) {
    ...
    for (int32_t i = 0; i < searchablesLen; ++i) { // search each searcher
        ...
        for (j = 0; j < docs->scoreDocsLength; ++j) { // merge scoreDocs into hq
            ...
            if (!hq->insert(fieldDocs[j])) break; // no more scores > minScore
        }
...
void MultiSearcher::explain(Query* query, int32_t doc, Explanation* ret) {
    ...
    searchables[i]->explain(query, doc - starts[i],
                            ret); // dispatch to searcher
}
