...
Query* PhraseQuery::clone() const { return _CLNEW PhraseQuery(*this); }
...
bool PhraseQuery::equals(CL_NS(search)::Query* other) const {
    if (!(other->instanceOf(PhraseQuery::getClassName()))) return false;
...
size_t PhraseQuery::hashCode() const {
    ...
    size_t ret =
        Similarity::floatToByte(getBoost()) ^ Similarity::floatToByte(slop);
...
const char* PhraseQuery::getClassName() { return "PhraseQuery"; }
...
void PhraseQuery::add(Term* term, int32_t position) {
    ...
    if (terms->size() == 0)
        ...
        // Check if the field of the _CLNEW term matches the field of the
        // PhraseQuery can use != because fields are interned
        if (term->field() != field) {
            TCHAR buf[200];
            _sntprintf(buf, 200,
                       _T("All phrase terms must be in the same field: %s"),
                       term->field());
...
float_t PhraseWeight::sumOfSquaredWeights() {
    idf =
        parentQuery->getSimilarity(searcher)->idf(parentQuery->terms, searcher);
...
Scorer* PhraseWeight::scorer(IndexReader* reader) {
    ...
    if (slop != 0)
        ...
        ret = _CLNEW SloppyPhraseScorer(
            this, tps, positions.values, parentQuery->getSimilarity(searcher),
    ...
    // tps can be deleted safely. SloppyPhraseScorer or ExactPhraseScorer will
    // take care of its values
...
Explanation* PhraseWeight::explain(IndexReader* reader, int32_t doc) {
    ...
    _sntprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN, _T("idf(%s: %s)"),
               parentQuery->field, docFreqs.getBuffer());
    ...
    if (deleteBoostExpl) _CLLDELETE(boostExpl);
    ...
    _sntprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN,
               _T("fieldWeight(%s:%s in %d), product of:"), parentQuery->field,
               query.getBuffer(), doc);
    ...
    fieldExpl->addDetail(
        _CLNEW Explanation(idfExpl->getValue(), idfExpl->getDescription()));
