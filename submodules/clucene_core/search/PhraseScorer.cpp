...
PhraseScorer::~PhraseScorer() {
    ...
    // The PhraseQueue pq (which is a PriorityQueue) pq is actually empty at
    // present, the elements having been transferred by pqToList() to the linked
    // list starting with first.  The nodes of that linked list are deleted by
    // the destructor of first, rather than the destructor of pq.
    _CLLDELETE(first);
    _CLLDELETE(pq);
}
...
void PhraseScorer::sort() {
    pq->clear();
    for (PhrasePositions* pp = first; pp != NULL; pp = pp->_next) pq->put(pp);
    pqToList();
}
...
void PhraseScorer::pqToList() {
    ...
    //       of PhrasePositions of which the first element is pointed to by
    //       first and the last element is pointed to by last
