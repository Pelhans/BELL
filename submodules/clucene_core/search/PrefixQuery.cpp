...
Query* PrefixQuery::clone() const { return _CLNEW PrefixQuery(*this); }
...
bool PrefixQuery::equals(Query* other) const {
    if (!(other->instanceOf(PrefixQuery::getClassName()))) return false;
        ...
        do {
                ...
                lastTerm->field() == prefixField) // interned comparison
            {
                ...
                if (prefixLen > termLen)
                    break; // the prefix is longer than the term, can't be
                           // matched

                tmp = lastTerm->text();

                // check for prefix match in reverse, since most change will be
                // at the end
...
TCHAR* PrefixQuery::toString(const TCHAR* field) const {
    // Func - Creates a user-readable version of this query and returns it as as
    // string Pre  - field != NULL Post - a user-readable version of this query
    // has been returned as as string

    // Instantiate a stringbuffer buffer to store the readable version
    // temporarily
...
public:
    PrefixGenerator(const Term* prefix) { this->prefix = prefix; }
    ...
    void generate(IndexReader* reader) {
        ...
        try {
            do {
                ...
                if (term != NULL &&
                    ...
                    // check for prefix match in reverse, since most change will
                    // be at the end
                    for (i = prefixLen - 1; i != -1; --i) {
                        if (tmp[i] != prefixText[i]) {
                            tmp = NULL; // signals inequality
        ...
        _CLFINALLY(termDocs->close(); _CLDELETE(termDocs); enumerator->close();
...
public:
    ...
    void handleDoc(int doc) { bts->set(doc); }
};

PrefixFilter::PrefixFilter(Term* prefix) { this->prefix = _CL_POINTER(prefix); }

PrefixFilter::~PrefixFilter() { _CLDECDELETE(prefix); }

PrefixFilter::PrefixFilter(const PrefixFilter& copy)
    : Filter(),
      prefix(_CL_POINTER(copy.prefix)) {}

Filter* PrefixFilter::clone() const { return _CLNEW PrefixFilter(*this); }
