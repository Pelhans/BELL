...
RangeFilter::RangeFilter(const TCHAR* _fieldName, const TCHAR* _lowerTerm,
                         const TCHAR* _upperTerm, bool _includeLower,
                         bool _includeUpper)
    : fieldName(NULL),
      lowerTerm(NULL),
      upperTerm(NULL),
    ...
    if (NULL == _lowerTerm && NULL == _upperTerm) {
        _CLTHROWT(CL_ERR_IllegalArgument,
                  _T("At least one value must be non-null"));
    }
    if (_includeLower && NULL == _lowerTerm) {
        _CLTHROWT(CL_ERR_IllegalArgument,
                  _T("The lower bound must be non-null to be inclusive"));
    }
    if (_includeUpper && NULL == _upperTerm) {
        _CLTHROWT(CL_ERR_IllegalArgument,
                  _T("The upper bound must be non-null to be inclusive"));
    }

    this->fieldName = STRDUP_TtoT(_fieldName);
    if (_lowerTerm != NULL) this->lowerTerm = STRDUP_TtoT(_lowerTerm);
    if (_upperTerm != NULL) this->upperTerm = STRDUP_TtoT(_upperTerm);
}
...
RangeFilter* RangeFilter::Less(const TCHAR* _fieldName,
                               const TCHAR* _upperTerm) {
    return _CLNEW RangeFilter(_fieldName, NULL, _upperTerm, false, true);
}

RangeFilter* RangeFilter::More(const TCHAR* _fieldName,
                               const TCHAR* _lowerTerm) {
...
BitSet* RangeFilter::bits(IndexReader* reader) {
    ...
    TermEnum* enumerator =
        reader->terms(t); // get enumeration of all terms after lowerValue
    _CLDECDELETE(t);
...
#define CLEANUP           \
    ...
    try {
        do {
            ...
            if (term == NULL || _tcscmp(term->field(), fieldName)) break;

            if (!checkLower || lowerTerm == NULL ||
                _tcscmp(term->text(), lowerTerm) > 0) {
                checkLower = false;
                if (upperTerm != NULL) {
                    ...
                    if ((compare < 0) || (!includeUpper && compare == 0)) break;
                }
...
TCHAR* RangeFilter::toString() {
    size_t len = (fieldName ? _tcslen(fieldName) : 0) +
                 (lowerTerm ? _tcslen(lowerTerm) : 0) +
                 (upperTerm ? _tcslen(upperTerm) : 0) + 8;
    TCHAR* ret = _CL_NEWARRAY(TCHAR, len);
    ret[0] = 0;
    _sntprintf(ret, len, _T("%s: [%s-%s]"), fieldName,
               (lowerTerm ? lowerTerm : _T("")),
               (upperTerm ? upperTerm : _T("")));

    return ret;
}

Filter* RangeFilter::clone() const { return _CLNEW RangeFilter(*this); }
