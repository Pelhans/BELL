...
ScorerDocQueue::HeapedScorerDoc::HeapedScorerDoc(Scorer* scorer)
    : _scorer(scorer),
      _doc(scorer->doc()) {}

ScorerDocQueue::HeapedScorerDoc::HeapedScorerDoc(Scorer* scorer, int32_t doc)
    : _scorer(scorer),
      _doc(doc) {}

ScorerDocQueue::HeapedScorerDoc::~HeapedScorerDoc() {}

void ScorerDocQueue::HeapedScorerDoc::adjust() { this->_doc = _scorer->doc(); }
...
int32_t ScorerDocQueue::size() { return _size; }
...
Scorer* ScorerDocQueue::top() { return topHsd->_scorer; }

int32_t ScorerDocQueue::topDoc() { return topHsd->_doc; }

float_t ScorerDocQueue::topScore() { return topHsd->_scorer->score(); }
