...
Query* Query::mergeBooleanQueries(CL_NS(util)::ArrayBase<Query*>* queries) {
    ...
    for (size_t i = 0; i < queries->length; i++) {
        assert(BooleanQuery::getClassName() ==
               queries->values[i]->getObjectName());
    ...
    bool coordDisabled =
        (queries->length == 0)
            ? false
            : ((BooleanQuery*)queries->values[0])->isCoordDisabled();
...
Weight* Query::_createWeight(Searcher* /*searcher*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: Query::_createWeight");
}

Query::Query() : boost(1.0f) {}
Query::~Query() {}

const char* Query::getQueryName() const { return getObjectName(); }
/** Expert: called to re-write queries into primitive queries. */
Query* Query::rewrite(CL_NS(index)::IndexReader* /*reader*/) { return this; }
...
TCHAR* Query::toString() const { return toString(LUCENE_BLANK_STRING); }
...
void Query::extractTerms(TermSet* termset) const {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: Query::extractTerms");
}

TopFieldDocs::TopFieldDocs(int32_t totalHits, FieldDoc** fieldDocs,
                           int32_t scoreDocsLen, SortField** fields)
...
TopFieldDocs::~TopFieldDocs() {
    if (fieldDocs) {
        for (int32_t i = 0; i < scoreDocsLength; i++) _CLLDELETE(fieldDocs[i]);
        _CLDELETE_LARRAY(fieldDocs);
    }
    if (fields != NULL) {
        for (int32_t i = 0; fields[i] != NULL; i++) _CLLDELETE(fields[i]);
...
Hits* Searcher::search(Query* query) { return search(query, (Filter*)NULL); }
...
Similarity* Searcher::getSimilarity() { return this->similarity; }

const char* Searcher::getClassName() { return "Searcher"; }

const char* Searcher::getObjectName() const { return Searcher::getClassName(); }

Weight::~Weight() {}

TCHAR* Weight::toString() { return STRDUP_TtoT(_T("Weight")); }
