...
#pragma message( \
    "==================Using fallback float<->byte encodings!!!==================")
#else
#warning \
    "==================Using fallback float<->byte encodings!!!=================="
#endif
...
float_t Similarity::byteToFloat(uint8_t b) { return NORM_TABLE[b]; }

uint8_t Similarity::floatToByte(float_t f) { return Similarity::encodeNorm(f); }
...
int32_t floatToIntBits(float_t value) {
    ...
    if (e == 0x7f800000 && f != 0) u.i = 0x7fc00000;
...
void Similarity::_shutdown() { _CLDELETE(Similarity_defaultImpl); }

float_t Similarity::decodeNorm(uint8_t b) {
#ifndef _CL_HAVE_NO_FLOAT_BYTE
    if (!NORM_TABLE_initd) {
        for (int i = 0; i < 256; i++) NORM_TABLE[i] = byteToFloat(i);
...
float_t Similarity::idf(CL_NS(util)::CLVector<Term*>* terms,
                        Searcher* searcher) {
    float_t _idf = 0.0f;
    for (CL_NS(util)::CLVector<Term*>::iterator i = terms->begin();
         i != terms->end(); i++) {
...
float_t DefaultSimilarity::lengthNorm(const TCHAR* /*fieldName*/,
                                      int32_t numTerms) {
...
float_t DefaultSimilarity::tf(float_t freq) { return sqrt(freq); }
