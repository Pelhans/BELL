...
public:
     ...
     */
    float_t idf(CL_NS(util)::CLVector<CL_NS(index)::Term*>* terms,
                Searcher* searcher);
     ...
     * terms contained in a field.  These values, together with field boosts,
     * are stored in an index and multipled into scores for hits on each field
     * by the search code.
     *
     ...
     * squared weights of each of the query terms.  This value is then multipled
     * into the weight of each query term.
     *
     ...
     * {@link #tf(int32_t)} factor for each term in the query and these products
