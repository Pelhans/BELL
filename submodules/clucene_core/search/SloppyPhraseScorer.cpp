...
SloppyPhraseScorer::SloppyPhraseScorer(Weight* _weight, TermPositions** tps,
                                       int32_t* offsets, Similarity* similarity,
                                       int32_t _slop, uint8_t* norms)
    : PhraseScorer(_weight, tps, offsets, similarity, norms),
      slop(_slop),
      repeats(NULL),
      repeatsLen(0) {
    // Func - Constructor
...
SloppyPhraseScorer::~SloppyPhraseScorer() { _CLDELETE_LARRAY(repeats); }
...
float_t SloppyPhraseScorer::phraseFreq() {
    ...
    while (!done) {
        ...
        bool tpsDiffer = true;
        for (int32_t pos = start; pos <= next || !tpsDiffer;
             pos = pp->position) {
...
int32_t SloppyPhraseScorer::initPhrasePositions() {
    ...
    // position the pp's
    for (pp = first; pp != NULL; pp = pp->_next) pp->firstPosition();
    ...
    if (!checkedRepeats) {
        ...
        for (pp = first; pp != NULL; pp = pp->_next) {
            ...
            for (PhrasePositions* pp2 = pp->_next; pp2 != NULL;
                 pp2 = pp2->_next) {
                int32_t tpPos2 = pp2->position + pp2->offset;
                if (tpPos2 == tpPos) {
                    if (m == NULL) m = new PhrasePositionsMap(false, false);
                    pp->repeats = true;
                    pp2->repeats = true;
                    m->put(pp, NULL);
                    m->put(pp2, NULL);
    ...
    if (repeats != NULL) {
        ...
        qsort(repeats, repeatsLen, sizeof(PhrasePositions*),
              comparePhrasePositions);
        // now advance them
        for (size_t i = 0; i < repeatsLen; i++) {
            PhrasePositions* pp = repeats[i];
            while (!termPositionsDiffer(pp)) {
                if (!pp->nextPosition()) return -1; // ran out of a term -- done
            }
        }
    }
...
bool SloppyPhraseScorer::termPositionsDiffer(PhrasePositions* pp) {
    // efficiency note: a more efficient implemention could keep a map between
    // repeating pp's, so that if pp1a, pp1b, pp1c are repeats term1, and pp2a,
    // pp2b are repeats of term2, pp2a would only be checked against pp2b but
    // not against pp1a, pp1b, pp1c. However this would complicate code, for a
    // rather rare case, so choice is to compromise here.
    const int32_t tpPos = pp->position + pp->offset;
    for (size_t i = 0; i < repeatsLen; i++) {
        PhrasePositions* pp2 = repeats[i];
        if (pp2 == pp) continue;
        const int32_t tpPos2 = pp2->position + pp2->offset;
        if (tpPos2 == tpPos) return false;
    }
    return true;
}
