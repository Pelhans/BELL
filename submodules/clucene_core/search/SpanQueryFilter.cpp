...
#include "../search/spans/Spans.h"

CL_NS_DEF(search)

SpanQueryFilter::SpanQueryFilter() : query(NULL) {}

SpanQueryFilter::SpanQueryFilter(const CL_NS2(search,
                                              spans)::SpanQuery* query) {
...
SpanQueryFilter::~SpanQueryFilter() {
    if (bDeleteQuery) _CLDELETE(query);
}

Filter* SpanQueryFilter::clone() const { return _CLNEW SpanQueryFilter(*this); }
...
SpanFilterResult* SpanQueryFilter::bitSpans(CL_NS(index)::IndexReader* reader) {
    ...
    CL_NS(util)::CLList<SpanFilterResult::PositionInfo*>* tmp =
        new CL_NS(util)::CLList<SpanFilterResult::PositionInfo*>();
