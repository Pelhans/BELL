...
public:
    TermWeight(Searcher* searcher, TermQuery* parentQuery,
               CL_NS(index)::Term* _term);
    virtual ~TermWeight();
...
TermQuery::TermQuery(Term* t) : term(_CL_POINTER(t)) {}
TermQuery::TermQuery(const TermQuery& clone) : Query(clone) {
    this->term = _CL_POINTER(clone.term);
}
TermQuery::~TermQuery() { _CLLDECDELETE(term); }

Query* TermQuery::clone() const { return _CLNEW TermQuery(*this); }

const char* TermQuery::getClassName() { return "TermQuery"; }
const char* TermQuery::getObjectName() const { return getClassName(); }
...
bool TermQuery::equals(Query* other) const {
    if (!(other->instanceOf(TermQuery::getClassName()))) return false;

    TermQuery* tq = (TermQuery*)other;
    return (this->getBoost() == tq->getBoost()) && this->term->equals(tq->term);
}

TermWeight::TermWeight(Searcher* _searcher, TermQuery* _parentQuery, Term* term)
    : similarity(_searcher->getSimilarity()),
      value(0),
      queryNorm(0),
      queryWeight(0),
...
float_t TermWeight::sumOfSquaredWeights() {
    // legacy // idf = parentQuery->getSimilarity(searcher)->idf(_term,
    // searcher); // compute idf
...
Explanation* TermWeight::explain(IndexReader* reader, int32_t doc) {
    ...
    _sntprintf(buf, LUCENE_SEARCH_EXPLANATION_DESC_LEN,
               _T("idf(docFreq=%d, numDocs=%d)"), reader->docFreq(_term),
               reader->numDocs());
    ...
    queryExpl->setValue(
        parentQuery->getBoost() * // always 1.0 | TODO: original Java code is
                                  // boostExpl.getValue()
