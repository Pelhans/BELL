...
WildcardQuery::WildcardQuery(Term* term) : MultiTermQuery(term) {
    ...
    termContainsWildcard = (_tcschr(term->text(), _T('*')) != NULL ||
                            _tcschr(term->text(), _T('?')) != NULL);
}
...
const char* WildcardQuery::getClassName() { return "WildcardQuery"; }
...
Query* WildcardQuery::clone() const { return _CLNEW WildcardQuery(*this); }
...
bool WildcardQuery::equals(Query* other) const {
    if (!(other->instanceOf(WildcardQuery::getClassName()))) return false;
...
Query* WildcardQuery::rewrite(CL_NS(index)::IndexReader* reader) {
    if (termContainsWildcard) return MultiTermQuery::rewrite(reader);

    return _CLNEW TermQuery(getTerm(false));
}

WildcardFilter::WildcardFilter(Term* term) { this->term = _CL_POINTER(term); }

WildcardFilter::~WildcardFilter() { _CLDECDELETE(term); }

WildcardFilter::WildcardFilter(const WildcardFilter& copy)
    : term(_CL_POINTER(copy.term)) {}

Filter* WildcardFilter::clone() const { return _CLNEW WildcardFilter(*this); }
...
BitSet* WildcardFilter::bits(IndexReader* reader) {
    ...
    if (termEnum.term(false) == NULL) return bts;
    ...
    _CLFINALLY(termDocs->close(); _CLDELETE(termDocs); termEnum.close();)
