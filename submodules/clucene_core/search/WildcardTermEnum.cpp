...
bool WildcardTermEnum::termCompare(Term* term) {
    if (term != NULL && __term->field() == term->field()) {
        ...
        if (_tcsncmp(searchText, pre, preLen) == 0) {
            return wildcardEquals(patternText + preLen,
                                  __term->textLength() - preLen, 0, searchText,
                                  term->textLength(), preLen);
        }
    }
    _endEnum = true;
    return false;
...
WildcardTermEnum::WildcardTermEnum(IndexReader* reader, Term* term)
    ...
    preLen = idx;
    CND_PRECONDITION(preLen < term->textLength(),
...
WildcardTermEnum::~WildcardTermEnum() { close(); }

float_t WildcardTermEnum::difference() { return 1.0f; }

bool WildcardTermEnum::endEnum() { return _endEnum; }
...
bool WildcardTermEnum::wildcardEquals(const TCHAR* pattern, int32_t patternLen,
                                      int32_t patternIdx, const TCHAR* str,
                                      int32_t strLen, int32_t stringIdx) {
    for (int32_t p = patternIdx;; ++p) {
        for (int32_t s = stringIdx;; ++p, ++s) {
            ...
            if (sEnd) {
                ...
                // return true.
                if (justWildcardsLeft) return true;
            }
            ...
            if (pattern[p] == LUCENE_WILDCARDTERMENUM_WILDCARD_CHAR) continue;
