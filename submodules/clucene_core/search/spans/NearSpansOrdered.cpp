...
bool spanDocCompare(Spans *s1, Spans *s2) { return s1->doc() < s2->doc(); }

NearSpansOrdered::NearSpansOrdered(SpanNearQuery *spanNearQuery,
...
NearSpansOrdered::~NearSpansOrdered() {
    for (size_t i = 0; i < subSpansCount; i++) _CLLDELETE(subSpans[i]);
...
bool NearSpansOrdered::advanceAfterOrdered() {
    while (more && (inSameDoc || toSameDoc())) {
        if (stretchToOrder() && shrinkToAfterShortestMatch()) return true;
    }
...
bool NearSpansOrdered::toSameDoc() {
    ...
    while (subSpansByDoc[firstIndex]->doc() != maxDoc) {
        ...
        if (++firstIndex == subSpansCount) firstIndex = 0;
    }
...
bool NearSpansOrdered::docSpansOrdered(Spans *spans1, Spans *spans2) {
    assert(
        spans1->doc() ==
        spans2->doc()); // "doc1 " + spans1.doc() + " != doc2 " + spans2.doc();
...
bool NearSpansOrdered::docSpansOrdered(int start1, int end1, int start2,
                                       int end2) {
    ...
    for (int32_t i = (int32_t)subSpansCount - 2; i >= 0; i--) {
        ...
        while (true) // Advance prevSpans until after (lastStart, lastEnd)
        {
            if (!prevSpans->next()) {
                ...
                int32_t ppEnd =
                    prevSpans->end(); // Cannot avoid invoking .end()
