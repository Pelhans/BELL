...
/////////////////////////////////////////////////////////////////////////////
NearSpansUnordered::SpansCell::SpansCell(NearSpansUnordered* parentSpans,
                                         Spans* spans, int32_t index) {
...
NearSpansUnordered::SpansCell::~SpansCell() { _CLLDELETE(spans); }

bool NearSpansUnordered::SpansCell::adjust(bool condition) {
    if (length != -1) parentSpans->totalLength -= length; // subtract old length

    if (condition) {
        length = end() - start();
        parentSpans->totalLength += length; // add new length

        if (!parentSpans->max || doc() > parentSpans->max->doc() ||
            (doc() == parentSpans->max->doc() &&
             end() > parentSpans->max->end())) {
...
/////////////////////////////////////////////////////////////////////////////
bool NearSpansUnordered::CellQueue::lessThan(SpansCell* spans1,
                                             SpansCell* spans2) {
...
/////////////////////////////////////////////////////////////////////////////
NearSpansUnordered::NearSpansUnordered(SpanNearQuery* query,
    ...
    this->max =
        NULL; // CLucene specific, SpansCell::adjust tests this member to NULL
    ...
    for (size_t i = 0; i < query->getClausesCount(); i++) {
        SpansCell* cell =
            _CLNEW SpansCell(this, clauses[i]->getSpans(reader), i);
...
NearSpansUnordered::~NearSpansUnordered() {
    for (list<SpansCell*>::iterator iCell = ordered.begin();
         iCell != ordered.end(); iCell++)
        _CLLDELETE(*iCell);
...
void NearSpansUnordered::initList(bool next) {
    for (list<SpansCell*>::iterator iCell = ordered.begin();
         more && iCell != ordered.end(); iCell++) {
        if (next) more = (*iCell)->next(); // move to first entry

        if (more) addToList(*iCell); // add to list
    }
}
...
void NearSpansUnordered::queueToList() {
    last = NULL;
    first = NULL;
    while (queue->top()) addToList(queue->pop());
}
