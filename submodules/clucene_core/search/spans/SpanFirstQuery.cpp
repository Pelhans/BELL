...
public:
    SpanFirstQuerySpans(SpanFirstQuery* parentQuery,
...
/////////////////////////////////////////////////////////////////////////////
SpanFirstQuery::SpanFirstQuery(SpanQuery* match, int32_t end,
                               bool bDeleteQuery) {
    this->match = match;
    this->end = end;
...
SpanFirstQuery::SpanFirstQuery(const SpanFirstQuery& clone) : SpanQuery(clone) {
...
const char* SpanFirstQuery::getClassName() { return "SpanFirstQuery"; }

const char* SpanFirstQuery::getObjectName() const { return getClassName(); }

SpanQuery* SpanFirstQuery::getMatch() const { return match; }

int32_t SpanFirstQuery::getEnd() const { return end; }

const TCHAR* SpanFirstQuery::getField() const { return match->getField(); }

void SpanFirstQuery::extractTerms(CL_NS(search)::TermSet* terms) const {
    match->extractTerms(terms);
}

CL_NS(search)::Query* SpanFirstQuery::rewrite(
    CL_NS(index)::IndexReader* reader) {
...
bool SpanFirstQuery::equals(Query* other) const {
    ...
    return end == that->end && getBoost() == that->getBoost() &&
