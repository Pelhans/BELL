...
SpanNearQuery::SpanNearQuery(const SpanNearQuery &clone) : SpanQuery(clone) {
...
SpanNearQuery::~SpanNearQuery() {
    if (bDeleteClauses) {
        for (size_t i = 0; i < clausesCount; i++) _CLLDELETE(clauses[i]);
    }
...
const char *SpanNearQuery::getClassName() { return "SpanNearQuery"; }

const char *SpanNearQuery::getObjectName() const { return getClassName(); }

SpanQuery **SpanNearQuery::getClauses() const { return clauses; }

size_t SpanNearQuery::getClausesCount() const { return clausesCount; }
...
const TCHAR *SpanNearQuery::getField() const { return field; }

int32_t SpanNearQuery::getSlop() const { return slop; }

bool SpanNearQuery::isInOrder() const { return inOrder; }

void SpanNearQuery::extractTerms(CL_NS(search)::TermSet *terms) const {
    for (size_t i = 0; i < clausesCount; i++) clauses[i]->extractTerms(terms);
}

CL_NS(search)::Query *SpanNearQuery::rewrite(
...
bool SpanNearQuery::equals(Query *other) const {
    ...
    if (inOrder != that->inOrder || slop != that->slop ||
        getBoost() != that->getBoost() ||
        0 != _tcscmp(field, that->field)) // CLucene: java version does not
                                          // compare field names
    {
        return false;
    }

    if (clausesCount != that->clausesCount) return false;
    for (size_t i = 0; i < clausesCount; i++) {
        if (!clauses[i]->equals(that->clauses[i])) return false;
    }
...
Spans *SpanNearQuery::getSpans(CL_NS(index)::IndexReader *reader) {
    if (clausesCount == 0)
        return _CLNEW
        EmptySpans(); // CLucene: 0-clause case - different to java version,
                      // because java creates SpanOrQuery to call its function
                      // to create empty spans

    if (clausesCount == 1) // optimize 1-clause case
        return clauses[0]->getSpans(reader);

    return inOrder ? (Spans *)_CLNEW NearSpansOrdered(this, reader)
