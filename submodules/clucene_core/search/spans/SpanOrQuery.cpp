...
class SpanOrQuery::SpanQueue
    : public CL_NS(
          util)::PriorityQueue<Spans*, CL_NS(util)::Deletor::Object<Spans>> {
public:
...
public:
    SpanOrQuerySpans(SpanOrQuery* parentQuery,
                     CL_NS(index)::IndexReader* reader);
...
SpanOrQuery::SpanOrQuerySpans::~SpanOrQuerySpans() { _CLLDELETE(queue); }

bool SpanOrQuery::SpanOrQuerySpans::next() {
    if (!queue) return initSpanQueue(-1);
...
bool SpanOrQuery::SpanOrQuerySpans::skipTo(int32_t target) {
    if (!queue) return initSpanQueue(target);
...
/////////////////////////////////////////////////////////////////////////////
SpanOrQuery::SpanOrQuery(const SpanOrQuery& clone) : SpanQuery(clone) {
...
const char* SpanOrQuery::getClassName() { return "SpanOrQuery"; }

const char* SpanOrQuery::getObjectName() const { return getClassName(); }

SpanQuery** SpanOrQuery::getClauses() const { return clauses; }

size_t SpanOrQuery::getClausesCount() const { return clausesCount; }
...
const TCHAR* SpanOrQuery::getField() const { return field; }
...
bool SpanOrQuery::equals(Query* other) const {
    ...
    if (0 != _tcscmp(field, that->field) || getBoost() != that->getBoost()) {
        return false;
    }
...
Spans* SpanOrQuery::getSpans(CL_NS(index)::IndexReader* reader) {
    if (clausesCount == 0) return _CLNEW EmptySpans(); // CLucene: 0-clause case
