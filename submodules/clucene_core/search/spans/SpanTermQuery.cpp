...
SpanTermQuery::SpanTermQuery(const SpanTermQuery& clone) : SpanQuery(clone) {
    this->term = _CL_POINTER(clone.term);
}

SpanTermQuery::~SpanTermQuery() { _CLLDECDELETE(term); }

CL_NS(search)::Query* SpanTermQuery::clone() const {
    return _CLNEW SpanTermQuery(*this);
}

const char* SpanTermQuery::getClassName() { return "SpanTermQuery"; }

const char* SpanTermQuery::getObjectName() const { return getClassName(); }
...
const TCHAR* SpanTermQuery::getField() const { return term->field(); }
...
bool SpanTermQuery::equals(Query* other) const {
    if (!(other->instanceOf(SpanTermQuery::getClassName()))) return false;
