...
SpanWeight::~SpanWeight() {
    for (TermSet::iterator iTerm = terms->begin(); iTerm != terms->end();
         iTerm++)
        _CLLDECDELETE(*iTerm);
...
CL_NS(search)::Query* SpanWeight::getQuery() { return query; }

float_t SpanWeight::getValue() { return value; }
...
CL_NS(search)::Scorer* SpanWeight::scorer(CL_NS(index)::IndexReader* reader) {
    return _CLNEW SpanScorer(query->getSpans(reader), this, similarity,
                             reader->norms(query->getField()));
}

CL_NS(search)::Explanation* SpanWeight::explain(
    ...
    for (TermSet::iterator itTerms = terms->begin(); itTerms != terms->end();
         itTerms++) {
        CL_NS(index)::Term* term = (*itTerms);
        docFreqs.append(term->text());
        docFreqs.append(_T( "=" ));
        docFreqs.appendInt(reader->docFreq(term));
        if (itTerms != terms->end()) docFreqs.append(_T( " " ));
    }
    ...
    if (getQuery()->getBoost() != 1.0f)
        queryExpl->addDetail(
            _CLNEW Explanation(getQuery()->getBoost(), _T( "boost" )));
    ...
    queryExpl->setValue(getQuery()->getBoost() * idfExpl->getValue() *
