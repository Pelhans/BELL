...
/**
 * This header contains definitions and macros for helping create cross-platform
 * code. It is primarily for use by the clucene-core library, but is split off
 * so that other applications such as the demo, test, benchmarks, etc can
 * benefit from the cross platform code. Cross platform code is not distributed
 * with the clucene-core and is not available through the shared library.
 */
...
#define _LUCENE_PRAGMA_WARNINGS // tell lucene to display warnings using pragmas
                                // instead of #warning
#endif
...
#define CL_STRUCT_DEF(sub, clazz) \
    namespace lucene {            \
    namespace sub {               \
    struct clazz;                 \
    }                             \
    }
#define CL_CLASS_DEF(sub, clazz) \
    namespace lucene {           \
    namespace sub {              \
    class clazz;                 \
    }                            \
    }
#define CL_CLASS_DEF2(sub, sub2, clazz) \
    namespace lucene {                  \
    namespace sub {                     \
    namespace sub2 {                    \
    class clazz;                        \
...
#define CL_TEMPATE_DEF(sub, clazz, typedefs) \
    namespace lucene {                       \
    namespace sub {                          \
    template <typedefs>                      \
    class clazz;                             \
...
#define CL_TYPE_DEF(sub, clazz, def) \
    namespace lucene {               \
    namespace sub {                  \
    typedef def clazz;               \
    }                                \
...
#define CLUCENE_SHARED_EXPORT // don't export if we are building a static
                              // library
#else
...
#define _CL_DEPRECATE_TEXT(_Text)
#endif
#define _CL_DEPRECATED(_NewItem)                                             \
    _CL_DEPRECATE_TEXT(                                                      \
        "This function or variable has been superceded by newer library or " \
        "operating system functionality. Consider using " #_NewItem          \
