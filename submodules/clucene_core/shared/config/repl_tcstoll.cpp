...
#define LUCENE_TCSTOI64_RADIX(x, r)        \
    ((x >= _T('0') && x <= _T('9'))        \
         ? x - _T('0')                     \
         : ((x >= _T('a') && x <= _T('z')) \
                ? x - _T('a') + 10         \
                : ((x >= _T('A') && x <= _T('Z')) ? x - _T('A') + 10 : 1000)))

    if (radix < 2 || radix > 36) return 0;

    /* Skip white space.  */
    while (_istspace(*str)) ++str;
    ...
    *end = (TCHAR*)str;
    ...
    for (; p >= str; p--) {
        ...
        if (pos == 0)
            ...
            ret += (int64_t)pow((float_t)radix, (float_t)pos) *
                   i; // todo: might be quicker with a different pow overload
