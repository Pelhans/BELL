...
#pragma message( \
    "==================Not implementing any thread mutex==================")
#else
...
mutex_thread::mutex_thread() : _internal(new Internal) {
    InitializeCriticalSection(&_internal->mtx);
}
...
void mutex_thread::lock() { EnterCriticalSection(&_internal->mtx); }

void mutex_thread::unlock() { LeaveCriticalSection(&_internal->mtx); }

_LUCENE_THREADID_TYPE mutex_thread::_GetCurrentThreadId() {
    return GetCurrentThreadId();
}
void mutex_thread::_exitThread(int val) { ExitThread(val); }
...
public:
    void* _event;
    Internal() { _event = CreateEventA(NULL, false, false, NULL); }
    ~Internal() { CloseHandle(_event); }
};
shared_condition::shared_condition() { _internal = new Internal; }
shared_condition::~shared_condition() { delete _internal; }
...
_LUCENE_THREADID_TYPE mutex_thread::CreateThread(luceneThreadStartRoutine* func,
                                                 void* arg) {
...
mutex_thread::mutex_thread(const mutex_thread& /*clone*/)
    : _internal(new Internal) {
#ifdef _CL_HAVE_PTHREAD_MUTEX_RECURSIVE
    _CLPTHREAD_CHECK(pthread_mutex_init(&_internal->mtx, &mutex_thread_attr),
                     "mutex_thread(clone) constructor failed")
#else
#if defined(__hpux) && defined(_DECTHREADS_)
    _CLPTHREAD_CHECK(
        pthread_mutex_init(&_internal->mtx, pthread_mutexattr_default),
        "mutex_thread(clone) constructor failed")
#else
    _CLPTHREAD_CHECK(pthread_mutex_init(&_internal->mtx, 0),
                     "mutex_thread(clone) constructor failed")
#endif
...
mutex_thread::~mutex_thread() {
    _CLPTHREAD_CHECK(pthread_mutex_destroy(&_internal->mtx),
                     "~mutex_thread destructor failed")
    delete _internal;
}
...
void mutex_thread::lock() {
    ...
    if (pthread_equal(_internal->lockOwner, currentThread)) {
        ...
        _CLPTHREAD_CHECK(pthread_mutex_lock(&_internal->mtx),
                         "mutex_thread::lock")
...
void mutex_thread::unlock() {
    ...
    if (_internal->lockCount == 0) {
        _internal->lockOwner = 0;
        _CLPTHREAD_CHECK(pthread_mutex_unlock(&_internal->mtx),
                         "mutex_thread::unlock")
    }
#else
    _CLPTHREAD_CHECK(pthread_mutex_unlock(&_internal->mtx),
...
struct shared_condition::Internal {
    ...
    Internal() { pthread_cond_init(&condition, NULL); }
    ~Internal() { pthread_cond_destroy(&condition); }
};
shared_condition::shared_condition() { _internal = new Internal; }
shared_condition::~shared_condition() { delete _internal; }
void shared_condition::Wait(mutex_thread* shared_lock) {
    int res = 0;
    res =
        pthread_cond_wait(&_internal->condition, &shared_lock->_internal->mtx);
    assert(res == 0);
}
void shared_condition::NotifyAll() {
    int res = 0;
...
mutexGuard::mutexGuard(_LUCENE_THREADMUTEX& rMutex) : mrMutex(&rMutex) {
    mrMutex->lock();
}
mutexGuard::~mutexGuard() { mrMutex->unlock(); }
