...
void _Cnd_OutDebug(const char* FormattedMsg, const char* StrTitle,
    ...
    MessageBox(
        NULL, FormattedMsg, StrTitle,
        (fatal == 1 ? MB_ICONSTOP : MB_ICONEXCLAMATION) | MB_OK | MB_TASKMODAL);
#else
    printf("%s\n", FormattedMsg);
#endif
    ...
    if (fatal) debugFatalExit(1);
}
#endif

void __cnd_FormatDebug(const char* File, int32_t Line, int32_t Title,
                       const char* Mes2, int32_t fatal) {
    char M[512];
