...
#include <cctype>
#include <limits.h>
#include "../util/dirent.h" //if we have dirent, then the native one will be used
...
size_t Misc::ahashCode(const char* str) {
    ...
    size_t hashCode = 0;
    while (*str != 0) hashCode = hashCode * 31 + *str++;
    return hashCode;
}
size_t Misc::ahashCode(const char* str, size_t len) {
    // Compute the hash code using a local variable to be reentrant.
    size_t hashCode = 0;
    for (size_t i = 0; i < len; i++) hashCode = hashCode * 31 + *str++;
    return hashCode;
}

int64_t Misc::filelength(int filehandle) {
    struct cl_stat_t info;
    if (fileHandleStat(filehandle, &info) == -1) return -1;
    return info.st_size;
}
...
void Misc::_cpycharToWide(const char* s, wchar_t* d, size_t len) {
    ...
    for (uint32_t i = 0; i < len && i < sLen + 1; i++) d[i] = s[i];
}
#endif
...
// static
const TCHAR* Misc::replace_all(const TCHAR* val, const TCHAR* srch,
                               const TCHAR* repl) {
    int32_t cnt = 0;
...
int Misc::file_Unlink(const char* path, int32_t maxAttempts) {
    ...
    while (maxAttempts != 0) {
        if (_unlink(path) != 0) return -1;

        i = 0;
        while (i < 100) {
            if (!Misc::dir_Exists(path)) return 1;

            if (++i > 50) // if it still doesn't show up, then we do some
                          // sleeping for the last 50ms
                _LUCENE_SLEEP(1);
        }
...
#define LEN(x) (x == NULL ? 0 : _tcslen(x))
    const size_t totalLen = LEN(a) + LEN(b) + LEN(c) + LEN(d) + LEN(e) +
                            LEN(f) + sizeof(TCHAR); /* Space for terminator. */
...
string Misc::segmentname(const char* segment, const char* ext,
                         const int32_t x) {
...
void Misc::segmentname(char* buffer, int32_t bufferLen, const char* Segment,
                       const char* ext, const int32_t x) {
    // Func - Static Method
    //       Creates a filename in buffer by concatenating Segment with ext and
    //       x
    // Pre  - buffer  != NULL
    //       Segment != NULL and holds the name of the segment
    //       ext     != NULL and holds the extension
    //       x contains a number
    // Post - When x = -1 buffer contains the concatenation of Segment and ext
    // otherwise
...
// static
int32_t Misc::stringDifference(const TCHAR* s1, const int32_t len1,
                               const TCHAR* s2, const int32_t len2) {
...
TCHAR* Misc::stringTrim(TCHAR* text) {
    ...
    for (i = 0; i < len;
         i++) { // find the first non-space character and store it as i
        if (!_istspace(text[i])) break;
    }
    for (j = len - 1; j > i;
         --j) { // find the last non-space character and store it as j
        if (!_istspace(text[j])) {
            break;
        }
    }
...
int64_t Misc::base36ToLong(const char* value) {
    ...
    while (*ptr != '\0') {
        lval = isdigit(*ptr) ? (36 * lval) + (*ptr - '0')
                             : (36 * lval) + (*ptr - 'a' + 10);
        ptr++;
    }
...
bool Misc::listFiles(const char* directory, std::vector<std::string>& files,
                     bool fullPath) {
    // clear old files
...
void Misc::zerr(int ret, string& err) {
    switch (ret) {
        case Z_ERRNO:
            err =
                "error occurred while reading or writing from the zlib streams";
            break;
        case Z_STREAM_ERROR: err = "invalid compression level"; break;
        case Z_DATA_ERROR: err = "invalid or incomplete deflate data"; break;
        case Z_MEM_ERROR: err = "out of memory"; break;
        case Z_VERSION_ERROR: err = "zlib version mismatch";
    }
}
...
bool Misc::deflate(const uint8_t* in, size_t inlen, std::ostream& dest,
                   string& err, int CHUNK, int level) {
    int ret, flush;
    unsigned have;
    z_stream strm;
...
bool Misc::inflate(const uint8_t* in, size_t inlen, std::ostream& dest,
                   string& err, int CHUNK) {
    int ret;
    unsigned have;
    z_stream strm;
    if (CHUNK == -1) CHUNK = CL_Z_DEFAULT_CHUNK;
