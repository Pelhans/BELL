...
StringBuffer::StringBuffer(TCHAR* buf, size_t maxlen,
...
StringBuffer::StringBuffer(const size_t initSize) {
    ...
    // Initialize the bufferLength to initSize + 1 The +1 is for the terminator
    // '\0'
...
StringBuffer::StringBuffer(const TCHAR* value) {
    // Func - Constructor.
    //       Creates an instance of Stringbuffer containing a copy of the string
    //       value
    // Pre  - value != NULL
    // Post - An instance of StringBuffer has been created containing the copy
    // of the string value
    ...
    bufferLength = (occupiedLength >= LUCENE_DEFAULT_TOKEN_BUFFER_SIZE
                        ? occupiedLength
                        : LUCENE_DEFAULT_TOKEN_BUFFER_SIZE);
    ...
    CND_PRECONDITION(buffer[len] == '\0',
                     "Buffer was not correctly terminated");
}
...
void StringBuffer::clear() {
    ...
    // TODO:	Should we really delete and recreate the buffer - perhaps just
    // reseting len should suffice?
...
void StringBuffer::appendChar(const TCHAR character) {
    ...
    // Check if the current buffer length is sufficient to have the string value
    // appended
    if (len + 1 > bufferLength) {
        // Have the size of the current string buffer increased because it is
        // too small
        growBuffer(len + 1);
    }
    // Put character at position len which is the end of the string in the
    // buffer Note that this action might overwrite the terminator of the string
    // '\0', which is kind of tricky
    buffer[len] = character;
    // Increase the len by to represent the correct length of the string in the
    // buffer
    len++;
}
...
void StringBuffer::append(const TCHAR* value, size_t appendedLength) {
    // Func - Appends a copy of the string value
    // Pre  - value != NULL
    //       appendedLength contains the length of the string value which is to
    //       be appended
    // Post - value has been copied and appended to the string in buffer
    ...
    // Add the length of the copied string to len to reflect the new length of
    // the string in the buffer (Note: len is not the bufferlength!)
    len += appendedLength;
}
...
void StringBuffer::appendInt(const int64_t value, const int32_t _Radix) {
    ...
    // Pre  - true
    // Post - The converted integer value has been appended to the string in
    // buffer
    ...
    // Have the converted integer now stored in buf appended to the string in
    // buffer
    append(buf);
}
...
void StringBuffer::appendFloat(const float_t value, const size_t digits) {
    ...
    // Post - The converted float_t value has been appended to the string in
    // buffer

    // using sprintf("%f" was not reliable on other plaforms... we use a custom
    // float convertor bvk: also, using sprintf and %f seems excessivelly slow
    assert(digits <= 8);

    // the maximum number of characters that int64 will hold is 23. so we need
    // 23*2+2
    ...
    if (dig > 0) {
        ...
        int64_t remi = (int64_t)(
            (value - v) *
            pow((float_t)10,
                (float_t)(dig +
                          1))); // take the remainder and make a whole number
        if (remi < 0) remi *= -1;
        int64_t remadj = remi / 10;
        if (remi - (remadj * 10) >= 5) remadj++; // adjust remainder
...
void StringBuffer::prepend(const TCHAR* value) {
    // Func - Puts a copy of the string value infront of the current string in
    // the StringBuffer Pre  - value != NULL Post - The string in pre(buffer)
    // has been shifted n positions where n equals the length of value.
...
void StringBuffer::prepend(const TCHAR* value, const size_t prependedLength) {
    // Func - Puts a copy of the string value in front of the string in the
    // StringBuffer Pre  - value != NULL
    //       prependedLength contains the length of the string value which is to
    //       be prepended
    // Post - A copy of the string value is has been in front of the string in
    // buffer todo: something is wrong with this code, i'm sure... it only grows
    // (and therefore moves if the buffer is to small) Check if the current
    // buffer length is sufficient to have the string value prepended
    if (prependedLength + len + 1 > bufferLength) {
        // Have the size of the current string buffer increased because it is
        // too small Because prependedLength is passed as the second argument to
        // growBuffer, growBuffer will have left the first prependedLength
        // characters empty when it recopied buffer during reallocation.
...
TCHAR* StringBuffer::toString() {
    // Func - Returns a copy of the current string in the StringBuffer sized
...
void StringBuffer::growBuffer(const size_t minLength,
                              const size_t skippingNInitialChars) {
    ...
    // Pre  - After growth, must have at least enough room for contents +
    // terminator so
    //       minLength >= skippingNInitialChars + len + 1
    //       skippingNInitialChars >= 0
    // Post - The buffer has been grown to a minimum length of minLength or
    // bigger and
    //       if skippingNInitialChars > 0, the contents of the buffer has beeen
    //       shifted forward by skippingNInitialChars positions as the buffer is
    //       reallocated, leaving the first skippingNInitialChars uninitialized
    //       (presumably to be filled immediately thereafter by the caller).

    CND_PRECONDITION(minLength >= skippingNInitialChars + len + 1,
                     "skippingNInitialChars is not large enough");
...
void StringBuffer::insert(const size_t pos, const TCHAR* chrs, size_t length) {
    ...
    if (length > 0) {
        ...
        memmove(&buffer[pos + length], &buffer[pos],
                sizeof(TCHAR) * (len - pos));
...
void StringBuffer::toLower() { _tcslwr(buffer); }

bool StringBuffer::substringEquals(size_t start, size_t end, const TCHAR* str,
                                   size_t length) const {
    if (length == -1) {
        length = _tcslen(str);
    }
