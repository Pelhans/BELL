...
#if !defined(_CL_HAVE_DIRENT_H) && !defined(_CL_HAVE_SYS_NDIR_H) && \
    !defined(_CL_HAVE_SYS_DIR_H) && !defined(_CL_HAVE_NDIR_H)
#include "dirent.h"
#include <sys/stat.h>
#include <errno.h>
...
DIR *opendir(const char *szPath) {
    ...
    // nd = (DIR *) malloc (sizeof (DIR) + _tcslen (szFullPath) + _tcslen
    // (DIRENT_SLASH) + 					_tcslen (DIRENT_SEARCH_SUFFIX)+1);
    nd = new DIR;
