...
class CLUCENE_EXPORT IndexWriter : LUCENE_BASE {
  ...
  SegmentInfos *rollbackSegmentInfos; // segmentInfos we will fallback to if the
                                      // commit fails
  ...
  SegmentsToOptimizeType *
      segmentsToOptimize; // used by optimize to note those needing optimization
...
public:
   ...
   *  IO error
   */
  explicit IndexWriter(CL_NS(store)::Directory *d, CL_NS(analysis)::Analyzer *a,
                       const bool create,
  ...
  /**
   * Sets the default (for any instance of IndexWriter) maximum time to wait for
   * a write lock (in milliseconds).
   */
   ...
   *
   * </p>
   * <p>See http://www.gossamer-threads.com/lists/lucene/java-dev/47895 for more
   * discussion. </p>
   *
   ...
   * @throws IOException if there is a low-level IO error
   */
  // NOT IMPLEMENTED YET: void addIndexes(CL_NS(util)::ArrayBase<IndexReader*>&
  // readers);
