...
public:
    ...
    // Returns an null terminated array of strings, one for each file in the
    // directory.
    char** list() const;
    virtual bool list(
        std::vector<std::string>* names) const = 0; // todo: deprecate this...
    ...
    // An advanced overload to avoid throwing an error. if result is false,
    // error is filled with the reason
    virtual bool openInput(const char* name, IndexInput*& ret,
                           CLuceneError& error, int32_t bufferSize = -1) = 0;
