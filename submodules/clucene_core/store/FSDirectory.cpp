 ...
 */
static CL_NS(
    util)::CLHashMap<const char*, FSDirectory*, CL_NS(util)::Compare::Char,
...
class FSDirectory::FSIndexInput : public BufferedIndexInput {
    ...
    public:
        ...
        char path[CL_MAX_DIR]; // todo: this is only used for cloning, better to
                               // get information from the fhandle
...
bool FSDirectory::FSIndexInput::open(const char* path, IndexInput*& ret,
                                     CLuceneError& error,
    ...
    // Open the file
    handle->fhandle =
        ::_cl_open(path, _O_BINARY | O_RDONLY | _O_RANDOM, _S_IREAD);
...
FSDirectory::FSIndexOutput::FSIndexOutput(const char* path, int filemode) {
    ...
    // O_CREAT - Creates and opens new file for writing. Has no effect if file
    // specified by filename exists O_RANDOM - Specifies that caching is
    // optimized for, but not restricted to, random access from disk. O_WRONLY -
    ...
    if (bExists)
        fhandle =
            _cl_open(path, _O_BINARY | O_RDWR | _O_RANDOM | O_TRUNC, filemode);
    else // added by JBP
        fhandle =
            _cl_open(path, _O_BINARY | O_RDWR | _O_RANDOM | O_CREAT, filemode);
...
/** output methods: */
void FSDirectory::FSIndexOutput::flushBuffer(const uint8_t* b,
                                             const int32_t size) {
...
void FSDirectory::init(const char* _path, LockFactory* lockFactory) {
    ...
    if (lockFactory == NULL) {
        if (disableLocks) {
            ...
            lockFactory =
                _CLNEW FSLockFactory(directory.c_str(), this->filemode);
            doClearLockID = true;
        }
    }
    ...
    if (!Misc::dir_Exists(directory.c_str())) {
        char* err =
            _CL_NEWARRAY(char, 19 + directory.length() +
                                   1); // 19: len of " is not a directory"
    ...
    while (itr != files.end()) {
        if (CL_NS(index)::IndexReader::isLuceneFile(itr->c_str())) {
            if (_unlink((directory + PATH_DELIMITERA + *itr).c_str()) == -1) {
                _CLTHROWA(CL_ERR_IO,
                          "Couldn't delete file "); // todo: make richer error
            }
        }
        itr++;
    }
...
void FSDirectory::setFileMode(int mode) { this->filemode = mode; }
int FSDirectory::getFileMode() { return this->filemode; }
void FSDirectory::setUseMMap(bool value) { useMMap = value; }
bool FSDirectory::getUseMMap() const { return useMMap; }
const char* FSDirectory::getClassName() { return "FSDirectory"; }
const char* FSDirectory::getObjectName() const { return getClassName(); }
...
const char* FSDirectory::getDirName() const { return directory.c_str(); }

FSDirectory* FSDirectory::getDirectory(const char* file, bool create,
...
// static
FSDirectory* FSDirectory::getDirectory(const char* _file,
                                       LockFactory* lockFactory) {
    FSDirectory* dir = NULL;
    {
        ...
        char* file =
            _realpath(_file, buf); // set a realpath so that if we change
                                   // directory, we can still function
        if (!file || !*file) {
        ...
        if (fileStat(file, &fstat) != 0) {
            // todo: should construct directory using _mkdirs... have to write
            // replacement
            if (_mkdir(file) == -1) {
        ...
        if (dir == NULL) {
            ...
            if (lockFactory != NULL && lockFactory != dir->getLockFactory()) {
                _CLTHROWA(
                    CL_ERR_IO,
                    "Directory was previously created with a different "
                    "LockFactory instance, please pass NULL as the lockFactory "
...
void FSDirectory::touchFile(const char* name) {
    CND_PRECONDITION(directory[0] != 0, "directory is not open");
    char buffer[CL_MAX_DIR];
    _snprintf(buffer, CL_MAX_DIR, "%s%s%s", directory.c_str(), PATH_DELIMITERA,
              name);
...
IndexInput* FSDirectory::openMMapFile(const char* name, int32_t bufferSize) {
    ...
    char fl[CL_MAX_DIR];
    priv_getFN(fl, name);
    if (Misc::file_Size(fl) <
        LUCENE_INT32_MAX_SHOULDBE) // todo: would this be bigger on 64bit
                                   // systems?. i suppose it would be...test
                                   // first
...
bool FSDirectory::openInput(const char* name, IndexInput*& ret,
    ...
    if (useMMap && Misc::file_Size(fl) <
                       LUCENE_INT32_MAX_SHOULDBE) // todo: would this be bigger
                                                  // on 64bit systems?. i suppose
                                                  // it would be...test first
...
void FSDirectory::close() {
    SCOPED_LOCK_MUTEX(DIRECTORIES_LOCK) {
        ...
        if (--refCount <= 0) { // refcount starts at 1
            ...
            if (dir) {
                DIRECTORIES.remove(
                    getDirName()); // this will be removed in ~FSDirectory
                _CLDECDELETE(dir);
                // NOTE: Don't unlock the mutex, since it has been destroyed
                // now...
                return;
            }
        }
...
char* FSDirectory::getLockPrefix() const {
    ...
    if (dirName[1] == ':') dirName[0] = (char)_totupper((char)dirName[0]);
...
void FSDirectory::renameFile(const char* from, const char* to) {
    ...
    if (Misc::dir_Exists(nu)) {
        ...
        if (Misc::file_Unlink(nu) == -1) {
            char* err = _CL_NEWARRAY(
                char, 16 + strlen(to) + 1); // 16: len of "couldn't delete "
    ...
    if (_rename(old, nu) != 0) {
        // todo: jlucene has some extra rename code - if the rename fails, it
        // copies the whole file to the new file... might want to implement that
        // if renaming fails on some platforms
