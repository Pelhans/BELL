...
IndexInput::IndexInput() : NamedObject() {}
...
int32_t IndexInput::readString(TCHAR* buffer, const int32_t maxLength) {
    ...
    if (len >= ml) {
        ...
        if (len - ml > 0) {
            // seek(getFilePointer()+(len-ml)); <- that was the wrong way to
            // "finish reading"
            skipChars(len - ml);
        }
        return ml;
...
void IndexInput::readChars(TCHAR* buffer, const int32_t start,
                           const int32_t len) {
...
void BufferedIndexInput::readBytes(uint8_t* _b, const int32_t _len,
                                   bool useBuffer) {
    int32_t len = _len;
    uint8_t* b = _b;
    ...
    if (len <= (bufferLength - bufferPosition)) {
        ...
        if (useBuffer && len < bufferSize) {
            ...
            if (after > length()) _CLTHROWA(CL_ERR_IO, "read past EOF");
            readInternal(b, len);
            bufferStart = after;
            bufferPosition = 0;
...
void BufferedIndexInput::seek(const int64_t pos) {
    if (pos < 0)
        _CLTHROWA(CL_ERR_IO,
                  "IO Argument Error. Value must be a positive value.");
...
BufferedIndexInput::~BufferedIndexInput() { BufferedIndexInput::close(); }
...
void BufferedIndexInput::refill() {
    ...
    if (bufferLength <= 0) _CLTHROWA(CL_ERR_IO, "IndexInput read past EOF");
...
void BufferedIndexInput::setBufferSize(int32_t newSize) {
    if (newSize != bufferSize) {
        ...
        if (buffer != NULL) {
            ...
            memcpy((void*)newBuffer, (void*)(buffer + bufferPosition),
                   numToCopy);
