...
/** Abstract base class for input from a file in a {@link
 * lucene::store::Directory}.  A random-access input stream.  Used for all
 * Lucene index input operations.
 * @see Directory
 * @see IndexOutput
 */
...
public:
     ...
     * maxLength is the amount read into the buffer, the whole string is still
     * read from the stream returns the amount read
     */
...
protected:
     ...
     * same point as the stream they were cloned from.
     *
