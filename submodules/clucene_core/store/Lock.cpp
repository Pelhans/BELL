...
bool LuceneLock::obtain(int64_t lockWaitTimeout) {
    ...
    if (lockWaitTimeout < 0 && lockWaitTimeout != LOCK_OBTAIN_WAIT_FOREVER) {
        _CLTHROWA(CL_ERR_IllegalArgument,
                  "lockWaitTimeout should be LOCK_OBTAIN_WAIT_FOREVER or a "
                  "non-negative number");
    }
...
std::string NoLock::toString() { return "NoLock"; }
bool NoLock::obtain() { return true; }
void NoLock::release() {}
bool NoLock::isLocked() { return false; }

const char* NoLock::getClassName() { return "NoLock"; }
const char* NoLock::getObjectName() const { return getClassName(); }

SingleInstanceLock::SingleInstanceLock(LocksType* locks,
                                       _LUCENE_THREADMUTEX* locks_LOCK,
                                       const char* lockName) {
    this->locks = locks;
...
const char* SingleInstanceLock::getClassName() { return "SingleInstanceLock"; }
const char* SingleInstanceLock::getObjectName() const { return getClassName(); }
...
FSLock::FSLock(const char* _lockDir, const char* name, int filemode) {
    if (filemode <= 0)
        this->filemode =
            _S_IREAD |
            _S_IWRITE; // must do this or file will be created Read only
    else
...
const char* FSLock::getClassName() { return "FSLock"; }
const char* FSLock::getObjectName() const { return getClassName(); }

bool FSLock::obtain() {
    if (!Misc::dir_Exists(lockDir)) {
        if (_mkdir(lockDir) == -1) {
            char* err = _CL_NEWARRAY(
                char, 34 + strlen(lockDir) +
                          1); // 34: len of "Couldn't create lock directory: "
...
void FSLock::release() { _unlink(lockFile); }

bool FSLock::isLocked() { return Misc::dir_Exists(lockFile); }

string FSLock::toString() { return string("SimpleFSLock@") + lockFile; }
