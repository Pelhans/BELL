...
extern "C" __declspec(dllimport) _cl_dword_t
    __stdcall GetFileSize(HANDLE hFile, _cl_dword_t* lpFileSizeHigh);

extern "C" __declspec(dllimport) bool __stdcall UnmapViewOfFile(
    void* lpBaseAddress);

extern "C" __declspec(dllimport) bool __stdcall CloseHandle(HANDLE hObject);
extern "C" __declspec(dllimport) HANDLE
    __stdcall CreateFileA(const char* lpFileName, _cl_dword_t dwDesiredAccess,
...
extern "C" __declspec(dllimport) HANDLE __stdcall CreateFileMappingA(
    HANDLE hFile, SECURITY_ATTRIBUTES* lpFileMappingAttributes,
    _cl_dword_t flProtect, _cl_dword_t dwMaximumSizeHigh,
    _cl_dword_t dwMaximumSizeLow, const char* lpName);
extern "C" __declspec(dllimport) void* __stdcall MapViewOfFile(
    HANDLE hFileMappingObject, _cl_dword_t dwDesiredAccess,
    _cl_dword_t dwFileOffsetHigh, _cl_dword_t dwFileOffsetLow,
...
#error no mmap implementation set
    ...
    Internal() : data(NULL), pos(0), isClone(false), _length(0) {}
    ~Internal() {}
};

MMapIndexInput::MMapIndexInput(const char* path) : _internal(_CLNEW Internal) {
    // Func - Constructor.
    ...
    _internal->fhandle = CreateFileA(path, GENERIC_READ, FILE_SHARE_READ, 0,
                                     OPEN_EXISTING, 0, 0);
    ...
    if (_internal->_length > 0) {
        _internal->mmaphandle = CreateFileMappingA(_internal->fhandle, NULL,
                                                   PAGE_READONLY, 0, 0, NULL);
        if (_internal->mmaphandle != NULL) {
            void* address =
                MapViewOfFile(_internal->mmaphandle, FILE_MAP_READ, 0, 0, 0);
            if (address != NULL) {
        ...
        cl_sprintf(errstr,
                   "MMapIndexInput::MMapIndexInput failed with error %d: %s",
                   len, errnum, lpMsgBuf);
    ...
    if (_internal->fhandle < 0) {
        ...
        if (::fstat(_internal->fhandle, &sb)) {
            ...
            // mmap the file
            void* address = ::mmap(0, _internal->_length, PROT_READ, MAP_SHARED,
                                   _internal->fhandle, 0);
...
int64_t MMapIndexInput::getFilePointer() const { return _internal->pos; }
void MMapIndexInput::seek(const int64_t pos) { this->_internal->pos = pos; }
    ...
    if (!_internal->isClone) {
        ...
        if (_internal->data != NULL) {
            if (!UnmapViewOfFile(_internal->data)) {
                CND_PRECONDITION(
                    false, "UnmapViewOfFile(data) failed"); // todo: change to
                                                            // rich error
            }
        }
        ...
        if (_internal->fhandle > 0) ::close(_internal->fhandle);
        _internal->fhandle = 0;
#endif
    }
    _internal->data = NULL;
