...
protected:
    typedef CL_NS(util)::CLHashMap<
        char*, RAMFile*, CL_NS(util)::Compare::Char, CL_NS(util)::Equals::Char,
    ...
    FileMap* files; // unlike the java Hashtable, FileMap is not synchronized,
                    // and all access must be protected by a lock
public:
    int64_t sizeInBytes; // todo

    DEFINE_MUTABLE_MUTEX(files_mutex) // mutable: const methods must also be
                                      // able to synchronize properly
    ...
    /**
     * Creates a new <code>RAMDirectory</code> instance from the {@link
     * FSDirectory}.
     *
    ...
    bool openInput(const char* name, IndexInput*& ret, CLuceneError& error,
