...
BitSet::BitSet(const BitSet& copy) : _size(copy._size), _count(-1) {
    int32_t len = (_size >> 3) + 1;
    bits = _CL_NEWARRAY(uint8_t, len);
    memcpy(bits, copy.bits, len);
}

BitSet::BitSet(int32_t size) : _size(size), _count(-1) {
    int32_t len = (_size >> 3) + 1;
    bits = _CL_NEWARRAY(uint8_t, len);
    memset(bits, 0, len);
}
...
void BitSet::write(CL_NS(store)::Directory* d, const char* name) {
    ...
    try {
        if (isSparse()) {
            writeDgaps(
                output); // sparse bit-set more efficiently saved as d-gaps.
        } else {
            writeBits(output);
        }
    }
...
BitSet::~BitSet() { _CLDELETE_ARRAY(bits); }
...
int32_t BitSet::size() const { return _size; }
...
BitSet* BitSet::clone() const { return _CLNEW BitSet(*this); }
...
bool BitSet::isSparse() {
    // note: order of comparisons below set to favor smaller values (no binary
    // range search.) note: adding 4 because we start with ((int) -1) to
    // indicate d-gaps format. note: we write the d-gap for the byte number, and
    // the byte (bits[i]) itself, therefore
...
int32_t BitSet::nextSetBit(int32_t fromIndex) const {
    if (fromIndex < 0) _CLTHROWT(CL_ERR_IndexOutOfBounds, _T("fromIndex < 0"));
