...
void FastCharStream::readChar(TCHAR& c) {
    try {
        ...
        if (err.number() == CL_ERR_IO) input = 0;
        throw err;
    }
}
int FastCharStream::GetNext() {
    ...
    {
        _CLTHROWA(CL_ERR_IO,
                  "warning : FileReader.GetNext : Read TCHAR over EOS.");
    }
...
bool FastCharStream::Eos() const { return input == NULL; }

int32_t FastCharStream::Column() const { return col; }

int32_t FastCharStream::Line() const { return line; }
CL_NS_END
