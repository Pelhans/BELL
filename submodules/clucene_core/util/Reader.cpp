...
StringReader::StringReader(const TCHAR* _value, const int32_t _length,
                           bool copyData) {
    this->m_size = 0;
    this->value = NULL;
    this->ownValue = true;
...
void StringReader::init(const TCHAR* _value, const int32_t _length,
                        bool copyData) {
    ...
    if (copyData) {
        ...
        } else if (length > this->buffer_size ||
                   length < (this->buffer_size / 2)) { // expand, or shrink
...
AStringReader::~AStringReader() {
    if (ownValue) _CLDELETE_ARRAY(this->value);
}

size_t AStringReader::size() { return m_size; }
int32_t AStringReader::read(const signed char*& start, int32_t min,
                            int32_t max) {
    if (m_size == pos) return -1;
    start = this->value + pos;
    int32_t r = (int32_t)cl_min(cl_max(min, max), m_size - pos);
...
int32_t AStringReader::read(const unsigned char*& start, int32_t min,
                            int32_t max) {
    if (m_size == pos) return -1;
...
int64_t AStringReader::position() { return pos; }
...
public:
    ...
    public:
        ...
        JStreamsBuffer(int32_t fhandle, int32_t buffersize) {
            ...
            int32_t bufsize =
                (int32_t)((m_size <= buffersize) ? m_size + 1 : buffersize);
            setMinBufSize(bufsize);
        }
        void _setMinBufSize(int32_t bufsize) { this->setMinBufSize(bufsize); }
    ...
    ~Internal() { delete jsbuffer; }
};

FileInputStream::FileInputStream(const char* path, int32_t buflen) {
    if (buflen == -1) buflen = DEFAULT_BUFFER_SIZE;
    _internal = new Internal(path, buflen);
}

size_t FileInputStream::size() { return _internal->jsbuffer->size(); }

FileInputStream::~FileInputStream() { delete _internal; }

int32_t FileInputStream::read(const signed char*& start, int32_t min,
                              int32_t max) {
    return _internal->jsbuffer->read(start, min, max);
}
int64_t FileInputStream::position() { return _internal->jsbuffer->position(); }
...
FileReader::FileReader(const char* path, const char* enc, int32_t buflen) {
    int encoding;
    if (strcmp(enc, "ASCII") == 0) encoding = ASCII;
#ifdef _UCS2
        ...
        _CLTHROWA(CL_ERR_IllegalArgument,
                  "Unsupported encoding, use jstreams iconv based instead");
...
public:
    ...
    public:
        ...
        virtual ~JStreamsBuffer() { _CLDELETE(input); }
        void _setMinBufSize(int32_t min) { this->setMinBufSize(min); }
    };
...
SimpleInputStreamReader::SimpleInputStreamReader() { _internal = NULL; }
...
SimpleInputStreamReader::~SimpleInputStreamReader() { delete _internal; }

int32_t SimpleInputStreamReader::read(const TCHAR*& start, int32_t min,
                                      int32_t max) {
    return _internal->jsbuffer->read(start, min, max);
}
...
size_t SimpleInputStreamReader::size() { return _internal->jsbuffer->size(); }
...
public:
    ...
    protected:
        int32_t fillBuffer(TCHAR* start, int32_t space) {
            ...
            if (r > 0) _tcsncpy(start, buffer, r);
            return r;
        }
    ...
    public:
        ...
        ~JStreamsFilteredBuffer() {
            if (deleteInput) _CLDELETE(input);
        }
        void _setMinBufSize(int32_t min) { this->setMinBufSize(min); }
    };
...
FilteredBufferedReader::FilteredBufferedReader(Reader* reader,
                                               bool deleteReader) {
    _internal = new Internal(reader, deleteReader);
}
FilteredBufferedReader::~FilteredBufferedReader() { delete _internal; }
int32_t FilteredBufferedReader::read(const TCHAR*& start, int32_t min,
                                     int32_t max) {
    return _internal->jsbuffer->read(start, min, max);
}
...
size_t FilteredBufferedReader::size() { return _internal->jsbuffer->size(); }
...
FilteredBufferedInputStream::FilteredBufferedInputStream(InputStream* input,
                                                         bool deleteInput) {
...
int32_t FilteredBufferedInputStream::read(const signed char*& start,
