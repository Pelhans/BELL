...
typedef CL_NS(
    util)::CLHashMap<TCHAR*, int, CL_NS(util)::Compare::TChar,
                     CL_NS(util)::Equals::TChar, CL_NS(util)::Deletor::tcArray,
                     CL_NS(util)::Deletor::DummyInt32> __wcsintrntype;
typedef CL_NS(
    util)::CLHashMap<char*, int, CL_NS(util)::Compare::Char,
                     CL_NS(util)::Equals::Char, CL_NS(util)::Deletor::acArray,
                     CL_NS(util)::Deletor::DummyInt32> __strintrntype;
...
void CLStringIntern::_shutdown() {
    ...
    if (StringIntern_stringaPool.size() > 0) {
        printf("WARNING: stringaPool still contains intern'd strings "
               "(refcounts):\n");
    ...
    if (StringIntern_stringPool.size() > 0) {
        printf("WARNING: stringPool still contains intern'd strings "
               "(refcounts):\n");
...
const TCHAR* CLStringIntern::intern(const TCHAR* str) {
    ...
    if (str[0] == 0) return LUCENE_BLANK_STRING;
...
bool CLStringIntern::unintern(const TCHAR* str) {
    ...
    if (str[0] == 0)
        return false; // warning: a possible memory leak, since str may be never
                      // freed!
...
const char* CLStringIntern::internA(const char* str, const int8_t count,
                                    const bool use_provided) {
    if (str == NULL) return NULL;
    if (str[0] == 0) return _LUCENE_BLANK_ASTRING;
    ...
    if (itr == StringIntern_stringaPool.end()) {
        ...
        if (use_provided)
            _CLDELETE_LCaARRAY(
                (char*)str); // delete the provided string if already exists
