...
/*
 * The concept of ThreadLocal is that a ThreadLocal class stores specific values
 * for each unique thread. Several thread-end detection techniques are used to
 * delete the thread data if the thread dies before the ThreadLocal class is
 * shut.
 *
...
bool __stdcall DllMain(unsigned short hinstDLL, // DLL module handle
                       ...
                       void*)                   // reserved
{
    if (fdwReason == 3) _ThreadLocal::UnregisterCurrentThread();
...
void pthread_threadlocal_make_key() {
    (void)pthread_key_create(&pthread_threadlocal_key,
                             &pthread_threadlocal_destructor);
}
#endif
...
typedef CL_NS(util)::CLMultiMap<
    _LUCENE_THREADID_TYPE, ThreadLocals*, CL_NS(util)::CLuceneThreadIdCompare,
...
// we don't use STATIC_DEFINE_MUTEX, because then the initialization order will
// be undefined.
...
public:
    typedef CL_NS(util)::CLSet<
        _LUCENE_THREADID_TYPE, void*, CL_NS(util)::CLuceneThreadIdCompare,
        CL_NS(util)::Deletor::ConstNullVal<_LUCENE_THREADID_TYPE>,
    ...
    Internal(AbstractDeletor* _deletor) : locals(false, false) {
...
void _ThreadLocal::set(void* t) {
    ...
    // drop a reference to this ThreadLocal in ThreadData
    {
        ...
        if (threadData == NULL) threadData = _CLNEW ThreadDataType(false, true);

        ThreadLocals* threadLocals = threadData->get(id);
        if (threadLocals == NULL) {
            threadLocals = _CLNEW ThreadLocals;
            threadData->insert(
                std::pair<const _LUCENE_THREADID_TYPE, ThreadLocals*>(
                    id, threadLocals));
        }
...

    {
        ...
        if (t != NULL) _internal->locals.put(id, t);
    }
}
...
void _ThreadLocal::RemoveThreadLocal(_ThreadLocal* tl) {
    ...
    for (ThreadDataType::iterator itr = threadData->begin();
         itr != threadData->end(); itr++) {
        ThreadLocals* threadLocals = itr->second;
...
void ThreadLocals::UnregisterThread() {
    // this should only be accessed from its own thread... if this changes, then
    // this access has to be locked.
