...
public:
    ...
    virtual ~__CLList() { clear(); }

    __CLList(const bool deleteValue) : dv(deleteValue) {}
    ...
    void set(size_t i, _kt val) {
        if (dv && i < base::size()) _valueDeletor::doDelete((*this)[i]);
    ...
    void delete_back() {
        if (base::size() > 0) {
            ...
            if (itr != base::begin()) itr--;
            _kt key = *itr;
            base::erase(itr);
            if (dv) _valueDeletor::doDelete(key);
        }
    }
    ...
    void remove(size_t i, bool dontDelete = false) {
        if (i < base::size()) {
            ...
            _kt key = *itr;
            base::erase(itr);
            if (dv && !dontDelete) _valueDeletor::doDelete(key);
        }
    }
...
// it grows in chunks... todo: check jlucene for initial size of array, and
// growfactors
template <typename _kt, typename _valueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLVector
    : public __CLList<_kt, CL_NS_STD(vector) < _kt>,
      _valueDeletor > {
public:
    CLVector(const bool deleteValue = true)
        : __CLList<_kt, CL_NS_STD(vector) < _kt>,
          _valueDeletor > (deleteValue) {}
};
...
// implementation of the List interface, provides access to the first and last
// list elements in O(1) no comparator is required... and so can contain
// duplicates a simple list with no comparator
//*** a very simple list - use <list>
#ifdef LUCENE_DISABLE_HASHING
#define CLHashList CLSetList
#else

template <typename _kt, typename _Comparator = CL_NS(util)::Compare::TChar,
          typename _valueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLHashList
    : public __CLList<_kt, CL_NS_HASHING(_CL_HASH_SET) < _kt, _Comparator>,
      _valueDeletor > {
public:
    CLHashList(const bool deleteValue = true)
        : __CLList<_kt, CL_NS_HASHING(_CL_HASH_SET) < _kt, _Comparator>,
          _valueDeletor > (deleteValue) {}
};
#endif

template <typename _kt, typename _valueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLLinkedList
    : public __CLList<_kt, CL_NS_STD(list) < _kt>,
      _valueDeletor > {
public:
    CLLinkedList(const bool deleteValue = true)
        : __CLList<_kt, CL_NS_STD(list) < _kt>,
          _valueDeletor > (deleteValue) {}
};
template <typename _kt, typename _Comparator = CL_NS(util)::Compare::TChar,
          typename _valueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLSetList
    : public __CLList<_kt, CL_NS_STD(set) < _kt, _Comparator>,
      _valueDeletor > {
public:
    CLSetList(const bool deleteValue = true)
        : __CLList<_kt, CL_NS_STD(set) < _kt, _Comparator>,
          _valueDeletor > (deleteValue) {}
};
