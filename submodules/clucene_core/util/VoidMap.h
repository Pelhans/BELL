 ...
 * @internal
 */
template <typename _kt, typename _vt, typename _base,
...
public:
    ...
    ///\param deleteKey if true then the key variable is deleted when an object
    /// is deleted \param keyDelType delete the key variable using the specified
    /// type \param deleteValue if true then the value variable is deleted when
    /// an object is deleted \param valueDelType delete the value variable using
    /// the specified type
    ...
    void removeitr(iterator itr, const bool dontDeleteKey = false,
                   const bool dontDeleteValue = false) {
        if (itr == base::end()) return;

        // delete key&val first. This prevents potential loops (deleting object
        // removes itself)
        _kt key = itr->first;
        _vt val = itr->second;
        base::erase(itr);

        // keys & vals need to be deleted after erase, because the hashvalue is
        // still needed
        if (dk && !dontDeleteKey) _KeyDeletor::doDelete(key);
        if (dv && !dontDeleteValue) _ValueDeletor::doDelete(val);
    }
    /// delete and optionally delete the specified key and associated value
    void remove(_kt key, const bool dontDeleteKey = false,
                const bool dontDeleteValue = false) {
        iterator itr = base::find(key);
        if (itr != base::end()) removeitr(itr, dontDeleteKey, dontDeleteValue);
    }
...
#else
                if (dk) _KeyDeletor::doDelete(itr->first);
                if (dv) _ValueDeletor::doDelete(itr->second);
                ++itr;
...
template <typename _kt, typename _vt, typename _Compare, typename _EqualDummy,
          typename _KeyDeletor = CL_NS(util)::Deletor::Dummy,
          typename _ValueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLHashMap
    : public __CLMap<_kt, _vt, CL_NS_STD(map) < _kt, _vt, _Compare>,
      _KeyDeletor,
      _ValueDeletor > {
    typedef typename CL_NS_STD(map)<_kt, _vt, _Compare> _base;
    typedef __CLMap<_kt, _vt, CL_NS_STD(map) < _kt, _vt, _Compare>, _KeyDeletor,
        _ValueDeletor > _this;
...
public:
    ...
    virtual void put(_kt k, _vt v) {
        ...
        if (_this::dk || _this::dv) _this::remove(k);
...
// unsynchronized
template <typename _kt, typename _vt, typename _Hasher, typename _Equals,
          typename _KeyDeletor = CL_NS(util)::Deletor::Dummy,
          typename _ValueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLHashMap
    : public __CLMap<_kt, _vt, CL_NS_HASHING(_CL_HASH_MAP) < _kt, _vt, _Hasher,
                     _Equals>,
      _KeyDeletor,
      _ValueDeletor > {
...
template <typename _kt, typename _vt, typename _Compare,
          typename _KeyDeletor = CL_NS(util)::Deletor::Dummy,
          typename _ValueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLSet
    : public __CLMap<_kt, _vt, CL_NS_STD(map) < _kt, _vt, _Compare>,
      _KeyDeletor,
      _ValueDeletor > {
    typedef typename CL_NS_STD(map)<_kt, _vt, _Compare> _base;
...
template <typename _kt, typename _vt, typename _Compare,
          typename _KeyDeletor = CL_NS(util)::Deletor::Dummy,
          typename _ValueDeletor = CL_NS(util)::Deletor::Dummy>
class CLUCENE_INLINE_EXPORT CLMultiMap
    : public __CLMap<_kt, _vt, CL_NS_STD(multimap) < _kt, _vt>,
      _KeyDeletor,
      _ValueDeletor > {
    typedef typename CL_NS_STD(multimap)<_kt, _vt> _base;
    typedef __CLMap<_kt, _vt, CL_NS_STD(multimap) < _kt, _vt>, _KeyDeletor,
        _ValueDeletor > _this;
